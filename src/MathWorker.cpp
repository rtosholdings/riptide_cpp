...
which provides the threading + work dispatch mechanism used by the rest of riptide.

Implementation notes
====================
Darwin / macOS (OSX) / XNU kernel
---------------------------------
* XNU doesn't support ``pthread_getaffinity_np()``, ``pthread_setaffinity_np()``, ``cpu_set_t`` as supported by Linux and FreeBSD.
  In fact, it explicitly does not provide the ability to bind threads to specific processor cores. However, it does provide
  a thread affinity API which can be used to provide affinity *hints* for application threads to the OS scheduler.
...
* macOS libpthread implementation; useful to see which threading APIs macOS supports -- particularly any _np (non-POSIX) APIs:
...
TODO
====
* Consider linking in the hwloc library here and using it to detect system topology and set thread affinity. It's portable,
  available through the libhwloc conda package, and already handles more-complex cases around NUMA and systems with
  >64 logical processors.
  For example, we'd use the ``hwloc_set_proc_cpubind()`` and ``hwloc_set_thread_cpubind()`` functions from the library,
  as documented here:
