...
void PrintCPUInfo() {
  ...
  if (g_bmi2 == 0 || g_avx2 == 0) {
    printf("!!!NOTE: this system does not support AVX2 or BMI2 instructions, "
           "and will not work!\n");
  }
}
...
void Sleep(unsigned int dwMilliseconds) { usleep(dwMilliseconds * 1000); }

bool CloseHandle(THANDLE hObject) { return true; }

pid_t GetCurrentThread() { return gettid(); }
...
HANDLE GetCurrentProcess() { return NULL; }
...
HMODULE LoadLibraryW(const wchar_t *lpLibFileName) { return NULL; }

FARPROC GetProcAddress(HMODULE hModule, const char *lpProcName) { return NULL; }
}
