...
extern bool GetUpcastType(int numpyInType1, int numpyInType2, int &convertType1,
                          int &convertType2, int64_t funcNumber);
extern int GetStridesAndContig(PyArrayObject const *inArray, int &ndim,
                               int64_t &stride);
 ...
 * valid.
 */
extern PyArrayObject *AllocateNumpyArray(int ndim, npy_intp *dims,
                                         int32_t numpyType,
                                         int64_t itemsize = 0,
 ...
 * valid.
 */
extern PyArrayObject *AllocateNumpyArrayForData(int ndim, npy_intp *dims,
                                                int32_t numpyType,
                                                int64_t itemsize, char *data,
                                                int array_flags,
                                                npy_intp *strides = nullptr);

extern PyArrayObject *AllocateLikeNumpyArray(PyArrayObject const *inArr,
                                             int32_t numpyType);
extern PyArrayObject *AllocateLikeResize(PyArrayObject *inArr,
                                         npy_intp rowSize);

extern int32_t GetArrDType(PyArrayObject *inArr);
extern bool ConvertScalarObject(PyObject *inObject1, _m256all *pDest,
                                int16_t numpyType, void **pDataIn,
                                int64_t *pItemSize);
extern bool ConvertSingleItemArray(void *pInput, int16_t numpyInType,
                                   _m256all *pDest, int16_t numpyType);
...
static inline bool IsFastArrayOrNumpy(PyArrayObject *pArray) {
  ...
  return ((pto == g_FastArrayType) || (pto == &PyArray_Type) ||
          PyType_IsSubtype(pto, &PyArray_Type));
}
...
template <typename T> static void *GetInvalid() {
