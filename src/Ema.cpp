        ...
        else
        {
            ...
            else
            {
                ...
                for ( int i = 0; i < totalInputRows; i++ )
                {
                    ...
                    if ( location >= GB_BASE_INDEX )
                    {
                        // printf("inputs: %lf  %lf  %lf  %lf  %lf\n", (double)pSrc[i], (double)pLastEma[location],
                        // (double)-decayRate, (double)pTime[i], (double)pLastTime[location] );
                        pLastEma[location] = pSrc[i] + pLastEma[location] * exp(-decayRate * (pTime[i] - pLastTime[location]));
...
#define EMA_WEIGHTED_FUNC \
    ...
    static EMA_BY_TWO_FUNC GetFunc(EMA_FUNCTIONS func)
    {
        switch ( func )
        {
        case EMA_DECAY: return EmaDecay;
        case EMA_NORMAL: return EmaNormal;
        case EMA_WEIGHTED: return EmaWeighted;
        default: break;
        }
        return NULL;
    }
...
PyObject * EmaAll32(PyObject * self, PyObject * args)
{
    ...
    CASE_NPY_INT64:
        ...
        break;
    default: PyErr_Format(PyExc_ValueError, "EmaAll32 key param must int8, int16, int32, int64"); return NULL;
    }
    ...
    default: PyErr_Format(PyExc_ValueError, "EmaAll32 cannot parse arguments.  tuple size %lld\n", tupleSize); return NULL;
    }
