...
typedef void (*ROLLING_FUNC)(void *pDataIn, void *pDataOut, int64_t len,
                             int64_t windowSize);
...
//=========================================================================================================================
typedef void (*EMA_BY_TWO_FUNC)(void *pKey, void *pAccumBin, void *pColumn,
                                int64_t numUnique, int64_t totalInputRows,
                                void *pTime, int8_t *pIncludeMask,
                                int8_t *pResetMask, double decayRate);
...
template <typename T, typename U> class EmaBase {
public:
  EmaBase(){};
  ~EmaBase(){};
  ...
  static void RollingSum(void *pDataIn, void *pDataOut, int64_t len,
                         int64_t windowSize) {
  ...
  static void RollingNanSum(void *pDataIn, void *pDataOut, int64_t len,
                            int64_t windowSize) {
  ...
  static void RollingMean(void *pDataIn, void *pDataOut, int64_t len,
                          int64_t windowSize) {
  ...
  static void RollingNanMean(void *pDataIn, void *pDataOut, int64_t len,
                             int64_t windowSize) {
  ...
  static void RollingVar(void *pDataIn, void *pDataOut, int64_t len,
                         int64_t windowSize) {
  ...
  static void RollingStd(void *pDataIn, void *pDataOut, int64_t len,
                         int64_t windowSize) {
  ...
  static void RollingNanVar(void *pDataIn, void *pDataOut, int64_t len,
                            int64_t windowSize) {
  ...
  static void RollingNanStd(void *pDataIn, void *pDataOut, int64_t len,
                            int64_t windowSize) {
...
PyObject *Rolling(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!LL", &PyArray_Type, &inArrObject, &func,
                        &param1)) {
...
static void CumSum(void *pKeyT, void *pAccumBin, void *pColumn,
                   int64_t numUnique, int64_t totalInputRows,
                   void *pTime1, // not used
                   int8_t *pIncludeMask, int8_t *pResetMask,
                   double windowSize1) {
  ...
  LOGGING("cumsum %lld  %lld  %lld  %p  %p\n", numUnique, totalInputRows,
          (int64_t)Invalid, pIncludeMask, pResetMask);
...
template <typename T, typename U, typename K>
static void CumProd(void *pKeyT, void *pAccumBin, void *pColumn,
                    int64_t numUnique, int64_t totalInputRows,
                    void *pTime1, // not used
                    int8_t *pIncludeMask, int8_t *pResetMask,
                    double windowSize1) {
...
static void FindNth(void *pKeyT, void *pAccumBin, void *pColumn,
                    int64_t numUnique, int64_t totalInputRows,
                    void *pTime1, // not used
                    int8_t *pIncludeMask, int8_t *pResetMask,
                    double windowSize1) {
...
template <typename T, typename U, typename V, typename K> class EmaByBase {
public:
  EmaByBase(){};
  ~EmaByBase(){};
  ...
  //    mask when to reset
  static void EmaDecay(void *pKeyT, void *pAccumBin, void *pColumn,
                       int64_t numUnique, int64_t totalInputRows, void *pTime1,
                       int8_t *pIncludeMask, int8_t *pResetMask,
                       double decayRate) {
          ...
          if (location >= GB_BASE_INDEX) {
            ...
            if (pIncludeMask[i] != 0) {
              ...
              pLastEma[location] =
                  value +
                  pLastEma[location] *
                      exp(-decayRate * (pTime[i] - pLastTime[location]));
...
#define EMA_NORMAL_FUNC                                                        \
  ...
  if (timeDelta < 0)                                                           \
    decayedWeight = 0;                                                         \
  pLastEma[location] =                                                         \
      value * (1 - decayedWeight) + pLastEma[location] * decayedWeight;        \
  pLastTime[location] = pTime[i];                                              \
  pDest[i] = pLastEma[location];
  ...
  // pResetMask might be NULL
  //    mask when to reset
  static void EmaNormal(void *pKeyT, void *pAccumBin, void *pColumn,
                        int64_t numUnique, int64_t totalInputRows, void *pTime1,
                        int8_t *pIncludeMask, int8_t *pResetMask,
                        double decayRate) {
...
#define EMA_WEIGHTED_FUNC                                                      \
  ...
  // pResetMask might be NULL
  //    mask when to reset
  static void EmaWeighted(void *pKeyT, void *pAccumBin, void *pColumn,
                          int64_t numUnique, int64_t totalInputRows,
                          void *pTime1, int8_t *pIncludeMask,
                          int8_t *pResetMask, double decayedWeight) {
...
template <typename K>
static EMA_BY_TWO_FUNC GetEmaByFunction(int inputType, int *outputType,
                                        int timeType, EMA_FUNCTIONS func) {
...
void EmaByCall(void *pEmaBy, int64_t i) {
  ...
  if (outArray && pFunction) {
    ...
    if (pFunctionX) {
      pFunctionX(pstEma32->pKey, (char *)pDataOut, (char *)pDataIn, uniqueRows,
                 pstEma32->totalInputRows,

                 // params
                 pstEma32->pTime, pstEma32->inIncludeMask,
                 pstEma32->inResetMask, pstEma32->doubleParam);
...
PyObject *EmaAll32(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "OO!LLO", &inList1, &PyArray_Type, &iKey,
                        &unique_rows, &funcNum, &params)) {
  ...
  if (!PyTuple_Check(params)) {
    PyErr_Format(PyExc_ValueError,
                 "EmaAll32 params argument needs to be a tuple");
    return NULL;
  }
  ...
  default:
    PyErr_Format(PyExc_ValueError,
                 "EmaAll32 key param must int8, int16, int32, int64");
    return NULL;
  }
  ...
  default:
    PyErr_Format(PyExc_ValueError,
                 "EmaAll32 cannot parse arguments.  tuple size %lld\n",
                 tupleSize);
    return NULL;
  }
  ...
  if (inResetMask != NULL && (PyArray_TYPE(inResetMask) != 0 ||
                              ArrayLength(inResetMask) != totalArrayLength)) {
    PyErr_Format(PyExc_ValueError,
                 "EmaAll32 inResetMask must be a bool mask of same size");
    return NULL;
  }

  if (inIncludeMask != NULL &&
      (PyArray_TYPE(inIncludeMask) != 0 ||
       ArrayLength(inIncludeMask) != totalArrayLength)) {
    PyErr_Format(PyExc_ValueError,
                 "EmaAll32 inIncludeMask must be a bool mask of same size");
    return NULL;
  }

  if (inTime != NULL && (PyArray_TYPE(inTime) < NPY_INT ||
                         PyArray_TYPE(inTime) > NPY_LONGDOUBLE ||
                         ArrayLength(inTime) != totalArrayLength)) {
    PyErr_Format(PyExc_ValueError,
                 "EmaAll32 inTime must be a 32 or 64 bit value of same size");
    return NULL;
  }
  ...
  ArrayInfo *aInfo =
      BuildArrayInfo(inList1, (int64_t *)&tupleSize, &totalItemSize);
  ...
  // being produced
  stEma32 *pstEma32 = (stEma32 *)WORKSPACE_ALLOC(
      (sizeof(stEma32) + 8 + sizeof(stEmaReturn)) * tupleSize);
  ...
  pstEma32->inIncludeMask =
      inIncludeMask == NULL ? NULL : (int8_t *)PyArray_BYTES(inIncludeMask);
  pstEma32->inResetMask =
      inResetMask == NULL ? NULL : (int8_t *)PyArray_BYTES(inResetMask);
  pstEma32->typeOfFunctionCall = TYPE_OF_FUNCTION_CALL::ANY_GROUPBY_FUNC;

  LOGGING("Ema unique %lld  total: %lld  arrays: %p %p %p\n", unique_rows,
          totalArrayLength, pstEma32->pTime, pstEma32->inIncludeMask,
          pstEma32->inResetMask);
  ...
  for (int i = 0; i < tupleSize; i++) {
    ...
    switch (iKeyType) {
    case NPY_INT8:
      pFunction = GetEmaByFunction<int8_t>(
          aInfo[i].NumpyDType, &numpyOutType,
          inTime == NULL ? -1 : PyArray_TYPE(inTime), (EMA_FUNCTIONS)funcNum);
      break;
    case NPY_INT16:
      pFunction = GetEmaByFunction<int16_t>(
          aInfo[i].NumpyDType, &numpyOutType,
          inTime == NULL ? -1 : PyArray_TYPE(inTime), (EMA_FUNCTIONS)funcNum);
      break;
    CASE_NPY_INT32:
      pFunction = GetEmaByFunction<int32_t>(
          aInfo[i].NumpyDType, &numpyOutType,
          inTime == NULL ? -1 : PyArray_TYPE(inTime), (EMA_FUNCTIONS)funcNum);
      break;
    CASE_NPY_INT64:

      pFunction = GetEmaByFunction<int64_t>(
          aInfo[i].NumpyDType, &numpyOutType,
          inTime == NULL ? -1 : PyArray_TYPE(inTime), (EMA_FUNCTIONS)funcNum);
      break;
    }
    ...
    if (pFunction) {
      ...
      outArray =
          AllocateNumpyArray(1, (npy_intp *)&totalArrayLength, numpyOutType);
      LOGGING("[%d] Allocated output array size %lld for type %d  ptr:%p\n", i,
              totalArrayLength, numpyOutType, PyArray_BYTES(outArray, 0));
    } else {
...
template <typename T>
void mat_interp_extrap(void *xT, void *xpT, void *ypT, void *outT, int64_t N,
                       int64_t M, int32_t clip) {
      ...
      if (xi == xi) {
        if (xi > xp[0]) {
          ...
          if (j == M) {
            ...
            // middle slope
            result =
                (yp[j] - yp[j - 1]) * (xi - xp[j - 1]) / (xp[j] - xp[j - 1]) +
                yp[j - 1];
          }
        } else {
...
template <typename T>
void mat_interp(void *xT, void *xpT, void *ypT, void *outT, int64_t N,
                int64_t M, int32_t clip) {
...
bool InterpolateExtrap(void *callbackArgT, int core, int64_t start,
                       int64_t length) {
  ...
  if (pInterp->mode == 2) {
    ...
    if (pInterp->itemsize == 8) {
      mat_interp_extrap<double>(pInterp->x + fixup, pInterp->xp + fixup2d,
                                pInterp->yp + fixup2d, pInterp->out + fixup,
                                length, M, clip);
    } else {
      mat_interp_extrap<float>(pInterp->x + fixup, pInterp->xp + fixup2d,
                               pInterp->yp + fixup2d, pInterp->out + fixup,
                               length, M, clip);
    }
  } else {
    if (pInterp->itemsize == 8) {
      mat_interp<double>(pInterp->x + fixup, pInterp->xp, pInterp->yp,
                         pInterp->out + fixup, length, M, clip);
    } else {
      mat_interp<float>(pInterp->x + fixup, pInterp->xp, pInterp->yp,
                        pInterp->out + fixup, length, M, clip);
    }
  }
  return true;
...
PyObject *InterpExtrap2d(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!|i", &PyArray_Type, &arr, &PyArray_Type,
                        &xp, &PyArray_Type, &yp, &clip)) {
  ...
  if (PyArray_NDIM(arr) > 1) {
    PyErr_Format(PyExc_ValueError,
                 "The 1st argument must be 1 dimensional arrays");
    return NULL;
  }

  if ((PyArray_NDIM(xp) != PyArray_NDIM(yp)) || PyArray_NDIM(yp) > 2) {
    PyErr_Format(PyExc_ValueError,
                 "The 2nd and 3rd argument must be the same dimensions");
    return NULL;
  }
  ...
  if (!(PyArray_FLAGS(xp) & PyArray_FLAGS(yp) & NPY_ARRAY_C_CONTIGUOUS)) {
    PyErr_Format(PyExc_ValueError, "The 2nd and 3rd argument must be row "
  ...
  if (mode == 2) {
    if ((N != PyArray_DIM(xp, 0)) || (N != PyArray_DIM(yp, 0))) {
      PyErr_Format(
          PyExc_ValueError,
          "The arrays must be the same size on the first dimension: %lld", N);
      return NULL;
    }
    M = PyArray_DIM(xp, 1);
  ...
  if (dtype != PyArray_TYPE(xp) || dtype != PyArray_TYPE(yp)) {
    PyErr_Format(PyExc_ValueError, "The arrays must all be the same type: %d",
                 dtype);
    return NULL;
  }
  ...
  if (dtype != NPY_FLOAT64 && dtype != NPY_FLOAT32) {
    PyErr_Format(PyExc_ValueError,
                 "The arrays must all be float32 or float64 not type: %d",
                 dtype);
    return NULL;
  }
