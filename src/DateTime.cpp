...
#define ParseSingleNonNumber(pStartX, pEndX)                                   \
  ((pStartX < pEndX && (*pStartX < '0' || *pStartX > '9')) ? ++pStartX : 0)
...
// pStart updated on return
template <typename T>
FORCE_INLINE int64_t ParseNumber(const T **ppStart, const T *pEnd,
                                 int64_t maxplaces) {
  const T *pStart = *ppStart;
...
//
template <typename T>
void ParseTimeString(int64_t *pOutNanoTime, int64_t arrayLength,
                     const T *pString, int64_t itemSize) {
  ...
  for (int64_t i = 0; i < arrayLength; i++) {
    ...
    if (hour < 24 && ParseSingleNonNumber(pStart, pEnd)) {
      ...
      if (minute < 60) {
        ...
        pOutNanoTime[i] =
            1000000000LL * ((hour * 3600) + (minute * 60) + seconds);
...
int64_t MONTH_SPLITS[12] = {0,   31,  59,  90,  120, 151,
...
int64_t YearToEpochNano(int64_t year, int64_t month, int64_t day) {
  ...
  if (year >= 1970 && year <= 2040 && month >= 1 && month <= 12 && day >= 1 &&
      day <= 31) {
...
//
template <typename T>
void ParseDateString(int64_t *pOutNanoTime, int64_t arrayLength,
                     const T *pString, int64_t itemSize) {
...
template <typename T>
void ParseDateTimeString(int64_t *pOutNanoTime, int64_t arrayLength,
                         const T *pString, int64_t itemSize) {
...
PyObject *TimeStringToNanos(PyObject *self, PyObject *args) {
  ...
  if (dType != NPY_STRING && dType != NPY_UNICODE) {
    PyErr_Format(PyExc_ValueError, "TimeStringToNanos first argument must be a "
  ...
  if (outArray) {
    ...
    if (dType == NPY_STRING) {
      ...
      ParseTimeString<uint32_t>(pNanoTime, arrayLength, (uint32_t *)pString,
                                itemSize / 4);
    }
...
PyObject *DateStringToNanos(PyObject *self, PyObject *args) {
  ...
  // TODO: Check to make sure inArr and timeArr sizes are the same
  if (dType != NPY_STRING && dType != NPY_UNICODE) {
    PyErr_Format(PyExc_ValueError, "DateStringToNanos first argument must be a "
                                   "bytes or unicode string array");
    return NULL;
  }
  ...
  if (outArray) {
    ...
    if (dType == NPY_STRING) {
      ...
      ParseDateString<uint32_t>(pNanoTime, arrayLength, (uint32_t *)pString,
                                itemSize / 4);
    }
    return (PyObject *)outArray;
  }
...
PyObject *DateTimeStringToNanos(PyObject *self, PyObject *args) {
  ...
  // TODO: Check to make sure inArr and timeArr sizes are the same
  if (dType != NPY_STRING && dType != NPY_UNICODE) {
    PyErr_Format(PyExc_ValueError, "DateTimeStringToNanos first argument must "
  ...
  if (outArray) {
    ...
    if (dType == NPY_STRING) {
      ...
      ParseDateTimeString<uint32_t>(pNanoTime, arrayLength, (uint32_t *)pString,
                                    itemSize / 4);
    }
    return (PyObject *)outArray;
  }
...
// Anything invalid is set to 0
template <typename T>
void ParseStrptime(int64_t *pOutNanoTime, int64_t arrayLength, const T *pString,
                   int64_t itemSize, const char *pFmt) {
  ...
  for (int64_t i = 0; i < arrayLength; i++) {
    ...
    if (pStop && timeBack.tm_year != 0) {
      ...
      yearresult = YearToEpochNano(timeBack.tm_year + 1900, timeBack.tm_mon + 1,
                                   timeBack.tm_mday);

      // addin hours min secs
      yearresult +=
          (1000000000LL * ((timeBack.tm_hour * 3600LL) +
                           (timeBack.tm_min * 60LL) + timeBack.tm_sec));
...
PyObject *StrptimeToNanos(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!y#", &PyArray_Type, &inArr, &strTimeFormat,
                        &strTimeFormatSize)) {
  ...
  // TODO: Check to make sure inArr and timeArr sizes are the same
  if (dType != NPY_STRING && dType != NPY_UNICODE) {
    PyErr_Format(PyExc_ValueError, "StrptimeToNanos first argument must be a "
  ...
  if (outArray) {
    ...
    if (dType == NPY_STRING) {
      ParseStrptime<char>(pNanoTime, arrayLength, pString, itemSize,
                          strTimeFormat);
    } else {

      ParseStrptime<uint32_t>(pNanoTime, arrayLength, (uint32_t *)pString,
                              itemSize / 4, strTimeFormat);
    }
