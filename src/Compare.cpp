...
// example of stub
// static void Compare32(void* pDataIn, void* pDataIn2, void* pDataOut, int64_t len, int32_t scalarMode) { return
// CompareFloat<_CMP_EQ_OS>(pDataIn, pDataIn2, pDataOut, len, scalarMode); } const int CMP_LUT[6] = { _CMP_EQ_OS, _CMP_NEQ_OS,
// _CMP_LT_OS, _CMP_GT_OS, _CMP_LE_OS, _CMP_GE_OS };
                                 ...
                                 int * wantedOutType)
{
    ...
    *wantedOutType = NPY_BOOL;
    ...
    // The unordered relationship is true when at least one of the two source operands being compared is a NaN; the ordered
    // relationship is true when neither source operand is a NaN. A subsequent computational instruction that uses the mask result
    // in the destination operand as an input operand will not generate an exception, because a mask of all 0s corresponds to a
    // floating - point value of + 0.0 and a mask of all 1s corresponds to a QNaN.
