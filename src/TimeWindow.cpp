...
typedef void (*TIMEWINDOW_FUNC)(void *pDataIn, void *pTimeIn, void *pDataOut,
...
template <typename T, typename U, typename V> class TimeWindowBase {
public:
  TimeWindowBase(){};
  ~TimeWindowBase(){};

  static void TimeWindowSum(void *pDataIn, void *pTimeIn, void *pDataOut,
                            int64_t start, int64_t length, int64_t timeDelta) {
  ...
  static void TimeWindowProd(void *pDataIn, void *pTimeIn, void *pDataOut,
                             int64_t start, int64_t length, int64_t timeDelta) {
...
TIMEWINDOW_FUNC GeTimeWindowFunction(int64_t func, int32_t inputType,
                                     int32_t *outputType) {
  switch (inputType) {
  case NPY_BOOL:
  ...
  case NPY_LONGDOUBLE:
    *outputType = NPY_LONGDOUBLE;
    return TimeWindowBase<long double, long double,
                          int64_t>::GeTimeWindowFunction(func);
  case NPY_INT8:
    *outputType = NPY_INT64;
    return TimeWindowBase<int8_t, int64_t, int64_t>::GeTimeWindowFunction(func);
  case NPY_INT16:
    *outputType = NPY_INT64;
    return TimeWindowBase<int16_t, int64_t, int64_t>::GeTimeWindowFunction(
        func);
  CASE_NPY_INT32:
    *outputType = NPY_INT64;
    return TimeWindowBase<int32_t, int64_t, int64_t>::GeTimeWindowFunction(
        func);
  CASE_NPY_INT64:

    *outputType = NPY_INT64;
    return TimeWindowBase<int64_t, int64_t, int64_t>::GeTimeWindowFunction(
        func);
  case NPY_UINT8:
    *outputType = NPY_UINT64;
    return TimeWindowBase<uint8_t, uint64_t, int64_t>::GeTimeWindowFunction(
        func);
  case NPY_UINT16:
    *outputType = NPY_UINT64;
    return TimeWindowBase<uint16_t, uint64_t, int64_t>::GeTimeWindowFunction(
        func);
  CASE_NPY_UINT32:
    *outputType = NPY_UINT64;
    return TimeWindowBase<uint32_t, uint64_t, int64_t>::GeTimeWindowFunction(
        func);
  CASE_NPY_UINT64:

    *outputType = NPY_UINT64;
    return TimeWindowBase<uint64_t, uint64_t, int64_t>::GeTimeWindowFunction(
        func);
  }
...
PyObject *TimeWindow(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!LL", &PyArray_Type, &inArr, &PyArray_Type,
                        &timeArr, &func, &param1)) {
  ...
  if (arrayLength != arrayLengthTime) {
    PyErr_Format(
        PyExc_ValueError,
        "TimeWindow array and time array must have same length: %lld  %lld",
        arrayLength, arrayLengthTime);
    return NULL;
  }
  ...
  if (pTimeWindowFunc) {
    ...
    if (outArray) {
      ...
      auto lambdaTWCallback = [](void *callbackArgT, int core, int64_t start,
                                 int64_t length) -> bool {
        TWCallbackStruct *callbackArg = (TWCallbackStruct *)callbackArgT;

        // printf("[%d] TW %lld %lld\n", core, start, length);

        callbackArg->func(callbackArg->pDataIn, callbackArg->pTimeIn,
      ...
      g_cMathWorker->DoMultiThreadedChunkWork(arrayLength, lambdaTWCallback,
                                              &stTWCallback);
