...
//
void FillInNumpyHeader(NUMPY_HEADERSIZE *pstNumpyHeader, int32_t dtype,
                       int32_t ndim, int64_t *dims, int32_t flags,
                       int64_t itemsize) {
  if (pstNumpyHeader) {
...
NUMPY_HEADERSIZE *AllocCompressedMemory(int64_t arraylength, int32_t dtype,
                                        int32_t ndim, int64_t *dims,
                                        int32_t flags, int64_t itemsize) {

  int64_t source_size = arraylength * itemsize;
  int64_t dest_size = ZSTD_compressBound(source_size);

  // Include enough for our header
  NUMPY_HEADERSIZE *pstNumpyHeader =
      (NUMPY_HEADERSIZE *)WORKSPACE_ALLOC(dest_size + sizeof(NUMPY_HEADERSIZE));
...
bool DecompressOneArray(void *pstCompressArraysV, int core, int64_t t) {
  COMPRESS_NUMPY_TO_NUMPY *pstCompressArrays =
      (COMPRESS_NUMPY_TO_NUMPY *)pstCompressArraysV;
  ArrayInfo *aInfo = pstCompressArrays->aInfo;

  if (pstCompressArrays->pNumpyHeaders[t]) {
    NUMPY_HEADERSIZE *pNumpyHeader =
        (NUMPY_HEADERSIZE *)pstCompressArrays->aInfo[t].pData;

    void *source = &pNumpyHeader[1];
    uint64_t dest_size = (uint64_t)ZSTD_getDecompressedSize(
        source, pNumpyHeader->compressedSize);
    ...
    size_t cSize =
        ZSTD_decompress(dest, dest_size, source, pNumpyHeader->compressedSize);

    if (ZSTD_isError(cSize)) {
      PyErr_Format(PyExc_ValueError, "Decompression error: %s",
...
bool CompressMemoryArray(void *pstCompressArraysV, int core, int64_t t) {
  ...
  if (pstCompressArrays->compMode == COMPRESSION_MODE_COMPRESS &&
    ...
    size_t cSize = ZSTD_compress(&pstNumpyHeader[1], dest_size, aInfo[t].pData,
                                 source_size, pstCompressArrays->compLevel);

    pstNumpyHeader->compressedSize = cSize;

    LOGGING("[%d] compressed %d%% %llu from %lld\n", (int)t,
            (int)((cSize * 100) / source_size), cSize, source_size);
...
PyObject *CompressDecompressArrays(PyObject *self, PyObject *args) {
  ...
  if (aInfo) {
    ...
    COMPRESS_NUMPY_TO_NUMPY *pstCompressArrays =
        (COMPRESS_NUMPY_TO_NUMPY *)WORKSPACE_ALLOC(
            sizeof(COMPRESS_NUMPY_TO_NUMPY) +
            (tupleSize * sizeof(NUMPY_HEADERSIZE *)));
    ...
    case COMPRESSION_MODE_COMPRESS: {
      ...
      for (int t = 0; t < tupleSize; t++) {
        ...
        pstCompressArrays->pNumpyHeaders[t] = AllocCompressedMemory(
            aInfo[t].ArrayLength, aInfo[t].NumpyDType, aInfo[t].NDim,
      ...
      g_cMathWorker->DoMultiThreadedWork((int)tupleSize, CompressMemoryArray,
                                         pstCompressArrays);
    } break;
    ...
    case COMPRESSION_MODE_DECOMPRESS: {
      for (int t = 0; t < tupleSize; t++) {
        ...
        uint64_t dest_size = (uint64_t)ZSTD_getDecompressedSize(
            &pNumpyHeader[1], pNumpyHeader->compressedSize);
        ...
        pstCompressArrays->pNumpyArray[t] = AllocateNumpyArray(
            pNumpyHeader->ndim, (npy_intp *)pNumpyHeader->dimensions,
            pNumpyHeader->dtype, pNumpyHeader->itemsize);
        CHECK_MEMORY_ERROR(pstCompressArrays->pNumpyArray[t]);
      }

      g_cMathWorker->DoMultiThreadedWork((int)tupleSize, DecompressOneArray,
                                         pstCompressArrays);
    } break;
    }
    ...
    if (mode == COMPRESSION_MODE_COMPRESS) {
      ...
      for (int t = 0; t < tupleSize; t++) {
        ...
        if (pstCompressArrays->pNumpyHeaders[t]) {
          ...
          int64_t totalCompressedBytes =
              pstNumpyHeader->compressedSize + sizeof(NUMPY_HEADERSIZE);
          int64_t uncomp = (int64_t)ZSTD_getDecompressedSize(
              pCompressedData, (size_t)totalCompressedBytes);

          uncompressedSize += uncomp;
          compressedSize += totalCompressedBytes;

          item = (PyObject *)AllocateNumpyArray(
              1, (npy_intp *)&totalCompressedBytes, NPY_UINT8);
      ...
      LOGGING("%d%% comp ratio    compressed size: %lld    uncompressed size: "
              "%lld\n",
              (int)((compressedSize * 100) / uncompressedSize), compressedSize,
              uncompressedSize);
    }
...
PyObject *CompressString(PyObject *self, PyObject *args) {
  ...
  if (source_size > 0) {
    ...
    if (ZSTD_isError(cSize)) {
      PyErr_Format(PyExc_ValueError, "Compression error: %s",
                   ZSTD_getErrorName(cSize));
      Py_CLEAR(result);
      return NULL;
    }
