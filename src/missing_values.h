...
{
     /**
      * Type trait for getting an invalid value (by the riptide definition) for a given C++ type.
      */
     template<typename T>
     ...
     struct invalid_for_type<bool>
     {
         ...
         {
             // Return true (always) here until/if we really support invalid/null values for bools.
             return true;
         }
     };
     ...
     {
         // NOTE: We don't use std::numeric_limits<int8_t>::min() here, because if the value
         //       differs between platforms (for whatever reason), it'd mean that different systems
         //       would interpret the same data values differently. Hard-coding the constant value
         //       here avoids that.
     ...
     /**
      * @brief Conversion function which understands riptide invalid/NA values and preserves them when converting.
      *
    ...
    {
       return
          invalid_for_type<SourceType>::is_valid
          // The value is valid, so perform the conversion normally.
          // NOTE: Because of the need to preserve values as closely as possible, this function
          //       isn't able to handle the case where a valid value (such as 255.f) is converted
          //       to another type and happens to be the invalid value for that type.
          //       This problem isn't really solvable without having a flag at the numpy C array level
          //       indicating whether the array should be considered as having sentinel values or not.
          //       Having that flag (or a bitmask indicating which values are valid) would allow the
          //       user to decide how to handle these values.
          ? TargetType{ value }
          // The input value is invalid, so return the/an invalid value for the target type.
