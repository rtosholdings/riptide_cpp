...
static const inline __m256i MIN_OPi8(  __m256i x, __m256i y) { return _mm256_min_epi8(x, y); }
static const inline __m256i MIN_OPu8(  __m256i x, __m256i y) { return _mm256_min_epu8(x, y); }
static const inline __m256i MIN_OPu16( __m256i x, __m256i y) { return _mm256_min_epu16(x, y); }
...
static const inline __m256i MAX_OPu16(__m256i x, __m256i y) { return _mm256_max_epu16(x, y); }
static const inline __m256i MAX_OPi16(__m256i x, __m256i y) { return _mm256_max_epi16(x, y); }
static const inline __m256i MAX_OPi32(__m256i x, __m256i y) { return _mm256_max_epi32(x, y); }
...
template<typename T> static const inline bool AddOp(bool x, bool y) { return x | y; }
template<typename T> static const inline T SubOp(T x, T y) { return x - y; }
template<typename T> static const inline bool SubOp(bool x, bool y) { return (x == true) && (y== false) ; }
template<typename T> static const inline T MulOp(T x, T y) { return x * y; }

// nan != nan --> True
// nan > anynumber --> False
// nan < anynumber --> False
template<typename T> static const inline float MinOp(float x, float y) { if (x > y || y != y) return y; else return x; }
template<typename T> static const inline double MinOp(double x, double y) { if (x > y || y != y) return y; else return x; }
template<typename T> static const inline T MinOp(T x, T y) { if (x > y ) return y; else return x; }

template<typename T> static const inline float MaxOp(float x, float y) { if (x < y || y != y) return y; else return x; }
template<typename T> static const inline double MaxOp(double x, double y) { if (x < y || y != y) return y; else return x; }
...
template<typename T> static const inline float ModOp(T x, T y) { return fmodf(x,y); }
template<typename T> static const inline double ModOp(T x, T y) { return fmod(x,y); }
template<typename T> static const inline bool ModOp(bool x, bool y) { return (x ^ y); }

template<typename T> static const inline T PowerOp(T x, T y) { return (T)pow(x, y); }
template<typename T> static const inline float PowerOp(float x, float y) { return powf(x, y); }
template<typename T> static const inline double PowerOp(double x, double y) { return pow(x, y); }
template<typename T> static const inline long double PowerOp(long double x, long double y) { return powl(x, y); }
template<typename T> static const inline bool PowerOp(bool x, bool y) { if (!x & y) return 0; else return 1; }

// numpy np.remainder is not 'C' remainder but there is a new math function
//template<typename T> static const inline long double REMAINDER_OP(long double x, long double y) { return remainderl(x, y); }
//template<typename T> static const inline double REMAINDER_OP(double x, double y) { return remainder(x, y); }
//template<typename T> static const inline float REMAINDER_OP(float x, float y) { return remainderf(x, y); }

template<typename T> static const inline long double REMAINDER_OP(long double x, long double y) { return x - (y*floorl(x / y)); }
template<typename T> static const inline double REMAINDER_OP(double x, double y) { return x - (y*floor(x / y)); }
template<typename T> static const inline float REMAINDER_OP(float x, float y) { return x - (y*floorf(x / y)); }

template<typename T> static const inline long double FMOD_OP(long double x, long double y) { return fmodl(x, y); }
template<typename T> static const inline double FMOD_OP(double x, double y) { return fmod(x, y); }
template<typename T> static const inline float FMOD_OP(float x, float y) { return fmodf(x, y); }

template<typename T> static const inline long double LogOp(long double x) { return logl(x); }
template<typename T> static const inline double LogOp(double x) { return log(x); }
template<typename T> static const inline float LogOp(float x) { return logf(x); }
...
static ANY_TWO_FUNC GetSimpleMathOpFast(int func, int scalarMode, int numpyInType1, int numpyInType2, int numpyOutType, int* wantedOutType) {
   ...
   case MATH_OPERATION::DIV:
      *wantedOutType = NPY_DOUBLE;
      if (scalarMode == SCALAR_MODE::FIRST_ARG_SCALAR) {
         if (numpyInType2 == NPY_FLOAT) *wantedOutType = NPY_FLOAT;
      }
      else {
         if (numpyInType1 == NPY_FLOAT)  *wantedOutType = NPY_FLOAT;
      }
   ...
   if (numpyInType1 <= NPY_LONGDOUBLE && numpyInType2 <= NPY_LONGDOUBLE) {
      ...
      case MATH_OPERATION::SUBDATES:
         if (scalarMode == SCALAR_MODE::NO_SCALARS && numpyInType1 != numpyInType2) {
            ...
            // Additional check to see if dtypes really are different (this path is rare)
...
public:
   ...
   PyObject* PossiblyConvertString(int newType, int oldType, PyObject* inObject) {
      ...
      if (newType == NPY_STRING && oldType == NPY_UNICODE) {
   ...
   BOOL CheckInputs(PyObject* args, int defaultScalarType, INT64 funcNumber) {
      ...
      //// For three inputs, the third inputs cannot be a scalar and must be same array length and type
   ...
   BOOL CheckInputsInternal(int defaultScalarType, INT64 funcNumber) {
      ...
      if (isScalar1) {
         ...
         if (!isScalar2) {
            // for most basic math, the two inputs must be same dtype, so check other dtype
      ...
      if (scalarMode == SCALAR_MODE::NO_SCALARS) {
         ...
         if (len1 != len2) {
            ...
            if (len1==1 && len2 >= 1) {
               ...
               // Check if we can upcast the scalar or if the scalar causes the array to upcast
   ...
   PyArrayObject* CheckOutputs(int wantOutType) {
      ...
      // If the number of args is 3, the third argument can be considered the output argument
      // outputObject will be set if came from inplace opeation like x += 3
      ...
      if (outputObject) {
         ...
         if (PyArray_TYPE((PyArrayObject*)outputObject) != wantOutType) {
            ...
            // z=rc.addi32(a,b,c)
            // Now z and c point to the same object, thus we must increment the ref count
            Py_INCREF(outputObject);
         }
      }

      // Check if they specified their own output array
      // NOTE: the output array is specified for += operations (in place operations)
      if (outputObject) {
         ...
         // TODO: when handling 2 dim arrays that are contiguous and same ORDER, allocate an array with the flags for order 'F' or 'C'
         if (len1 <= 0 && len2 <=0 ) {
            // Handle empty
...
static PyObject* PossiblyAddUnicode(CTwoInputs&  twoInputs) {
   ...
   if (twoInputs.scalarMode == SCALAR_MODE::SECOND_ARG_SCALAR && (twoInputs.numpyInType1 == NPY_STRING || twoInputs.numpyInType1 == NPY_UNICODE)) {
   ...
   if (inType != -1) {
      if (inType == NPY_STRING) {
         ...
         return ConcatTwoStrings(
            ...
            (char*)twoInputs.pDataIn1,
            (char*)twoInputs.pDataIn2,
            twoInputs.itemSize1,
            twoInputs.itemSize2,
...
TwoInputsInternal(CTwoInputs&  twoInputs, INT64 funcNumber) {
   ...
   if (outputArray && pTwoFunc && wantedOutputType != -1) {
      ...
      if (twoInputs.len1 > 0 || twoInputs.len2 > 0) {
         ...
         if (twoInputs.len1 > 1) fl.Input1Strides = PyArray_STRIDE(twoInputs.inArr, 0);
         if (twoInputs.len2 > 1) fl.Input2Strides = PyArray_STRIDE(twoInputs.inArr2, 0);

         fl.InputItemSize = twoInputs.len1 >= twoInputs.len2 ? PyArray_ITEMSIZE(twoInputs.inArr) : PyArray_ITEMSIZE(twoInputs.inArr2);
         ...
         LOGGING("Calling func %p %p %p %llu %llu %d\n", twoInputs.pDataIn1, twoInputs.pDataIn2, pDataOut, twoInputs.len1, twoInputs.len2, twoInputs.scalarMode);
         //pTwoFunc(twoInputs.pDataIn1, twoInputs.pDataIn2, pDataOut, twoInputs.len1 >= twoInputs.len2 ? twoInputs.len1 : twoInputs.len2, twoInputs.scalarMode);
         WorkTwoStubCall(&fl, twoInputs.pDataIn1, twoInputs.pDataIn2, pDataOut, twoInputs.len1 >= twoInputs.len2 ? twoInputs.len1 : twoInputs.len2, twoInputs.scalarMode);

         // Not sure why this final step is required to call fastarrayview
         // otherwise, the view is not set properly from a new array (recycled arrays do work though)
...
BOOL WhereCallback(void* callbackArgT, int core, INT64 start, INT64 length) {
   ...
   LOGGING("[%d] Where start: %lld  length: %lld   scalar: %d  strides1: %lld  strides2: %lld\n", core, start, length, pCallback->scalarMode, strides1, strides2);
