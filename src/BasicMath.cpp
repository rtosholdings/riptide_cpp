...
// static const inline __m256d DIV_OP_256(const int32_t z, __m128i x, __m128i y) { return _mm256_div_pd(_mm256_cvtepi32_pd(x),
// _mm256_cvtepi32_pd(y)); } static const inline __m256d DIV_OP_256(const int64_t z, __m128i x, __m128i y) { return
...
//_asm cvt2q
// CVTDQ2PD
// inline __m256d DIV_OP_256(const int64_t z, __m256i x, __m256i y) { return _mm256_div_pd(_mm256_cvtepi64_pd(x),
...
public:
    ...
    PyArrayObject * CheckOutputs(int wantOutType)
    {
        ...
        if (outputObject)
        {
            ...
            if (PyArray_TYPE((PyArrayObject *)outputObject) != wantOutType)
            {
                LOGGING("BasicMath does not match output type %d vs %d\n", PyArray_TYPE((PyArrayObject *)outputObject),
                        wantOutType);
                // PyErr_Format(PyExc_ValueError, "BasicMath does not match output type %d vs %d",
        ...
        else
        {
            ...
            // TODO: when handling 2 dim arrays that are contiguous and same ORDER, allocate an array with the flags for order 'F'
            // or 'C'
            if (len1 <= 0 && len2 <= 0)
            {
                // Handle empty
...
PyObject * TwoInputsInternal(CTwoInputs & twoInputs, int64_t funcNumber)
{
    ...
    if (outputArray && pTwoFunc && wantedOutputType != -1)
    {
        ...
        if (twoInputs.len1 > 0 || twoInputs.len2 > 0)
        {
            ...
            // pTwoFunc(twoInputs.pDataIn1, twoInputs.pDataIn2, pDataOut, twoInputs.len1 >= twoInputs.len2 ? twoInputs.len1 :
            // twoInputs.len2, twoInputs.scalarMode);
...
bool WhereCallback(void * callbackArgT, int core, int64_t start, int64_t length)
{
    ...
    LOGGING("[%d] Where start: %lld  length: %lld   scalar: %d  strides1: %lld  strides2: %lld\n", core, start, length,
            pCallback->scalarMode, strides1, strides2);
...
PyObject * Where(PyObject * self, PyObject * args)
{
    ...
    if (result)
    {
        LOGGING("Where input: scalarmode: %d  in1:%d in2:%d  out:%d  length: %lld  itemsize: %lld\n", twoInputs.scalarMode,
                twoInputs.numpyInType1, twoInputs.numpyInType2, twoInputs.numpyOutType, length, numpyItemSize);
