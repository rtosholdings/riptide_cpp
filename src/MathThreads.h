...
typedef bool (*DOWORK_CALLBACK)(struct stMATH_WORKER_ITEM *pstWorkerItem,
                                int32_t core, int64_t workIndex);

// Callback routine from multithreaded worker thread (items just count up from
// 0,1,2,...)
typedef bool (*MTWORK_CALLBACK)(void *callbackArg, int32_t core,
                                int64_t workIndex);

// Callback routine from multithreaded chunk thread (0, 65536, 130000, etc.)
typedef bool (*MTCHUNK_CALLBACK)(void *callbackArg, int32_t core, int64_t start,
                                 int64_t length);
...
struct stWorkerRing {
  ...
  FORCE_INLINE void Cancel() { Cancelled = 1; }
  ...
  FORCE_INLINE void SetWorkItem(int32_t maxThreadsToWake) {
    ...
    // Linux thread wakeup
    int s =
        futex((int *)&WorkIndex, FUTEX_WAKE, maxThreadsToWake, NULL, NULL, 0);
    if (s == -1)
