...
stScalarType NpTypeObjects[MAX_NUMPY_TYPE] = {
    {NULL, 1, NPY_BOOL},
    {NULL, 1, NPY_BYTE},
    {NULL, 1, NPY_UBYTE},
    {NULL, 2, NPY_SHORT},
...
const char *gNumpyTypeToString[NUMPY_LAST_TYPE] = {
    "NPY_BOOL",       "NPY_BYTE",      "NPY_UBYTE",   "NPY_SHORT",
    "NPY_USHORT",     "NPY_INT",       "NPY_UINT",
#if defined(RT_OS_WINDOWS)
    ...
    "NPY_LONG64",         // not on linux
#endif
    "NPY_INT64",      "NPY_UINT64",    "NPY_FLOAT",   "NPY_DOUBLE",
...
PyArrayObject *EnsureContiguousArray(PyArrayObject *inObject) {
  ...
  if (!(arrFlags & (NPY_ARRAY_C_CONTIGUOUS | NPY_ARRAY_F_CONTIGUOUS))) {
    ...
    inObject = (PyArrayObject *)PyArray_FromAny((PyObject *)inObject, NULL, 0,
                                                0, NPY_ARRAY_ENSURECOPY, NULL);
    if (!inObject) {
      PyErr_Format(PyExc_ValueError,
                   "RipTide: Error converting non-contiguous array");
      return NULL;
    }
  }
  return inObject;
...
PyObject *SetFastArrayType(PyObject *self, PyObject *args) {
  ...
  if (g_FastArrayType == NULL) {
    ...
    LOGGING("dealloc  %p %p %p %p\n", g_FastArrayDeallocate,
            g_FastArrayInstanceDeallocate, FastArrayDestructor,
            g_FastArrayType->tp_dealloc);
...
PyObject *LedgerFunction(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (tupleSize < 2) {
    PyErr_Format(PyExc_TypeError,
                 "LedgerFunction requires two args instead of %llu args",
                 tupleSize);
    return NULL;
  }
...
//-----------------------------------------------------------------------------------
PyArrayObject *AllocateNumpyArray(int ndim, npy_intp *dims, int32_t numpyType,
                                  int64_t itemsize, bool fortran_array,
                                  npy_intp *strides) {
  ...
  if (itemsize == 0 && !PyTypeNum_ISFLEXIBLE(numpyType)) {
    ...
    if (returnObject) {
      LOGGING("got recycled  len:%lld  ref cnt %llu!\n", len,
              returnObject->ob_base.ob_refcnt);
  ...
  PyTypeObject *const allocType =
      g_FastArrayType ? g_FastArrayType : &PyArray_Type;

  if (commonArray) {
    // NOTE: We now directly allocate a FastArray
    returnObject =
        (PyArrayObject *)PyArray_New(allocType, ndim, dims, numpyType, NULL,
                                     nullptr, 0, array_flags, nullptr);
...
PyArrayObject *AllocateNumpyArrayForData(int ndim, npy_intp *dims,
                                         int32_t numpyType, int64_t itemsize,
                                         char *data, int array_flags,
                                         npy_intp *strides) {
  ...
  // NOTE: this path taken when we already have data in our own memory
  returnObject =
      (PyArrayObject *)PyArray_New(allocType, ndim, dims, numpyType,
                                   strides, // Strides
...
//-----------------------------------------------------------------------------------
// Check recycle pool
PyArrayObject *AllocateLikeNumpyArray(PyArrayObject const *inArr,
                                      int32_t numpyType) {
  ...
  if ((PyArray_ISNUMBER(const_cast<PyArrayObject *>(inArr)) ||
       PyArray_ISBOOL(const_cast<PyArrayObject *>(inArr))) &&
  ...
  PyArrayObject *returnObject = (PyArrayObject *)PyArray_NewLikeArray(
      const_cast<PyArrayObject *>(inArr), NPY_KEEPORDER, descr, 1);
...
PyObject *Empty(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!iLO!", &PyList_Type, &inDimensions, &dtype,
                        &itemsize, &PyBool_Type, &isFortran))
    return NULL;
...
#error Unable to determine how to parse PyLong to npy_intp with overflow-checking.
  ...
  int overflow = 0;
  npy_intp dimension1 = static_cast<npy_intp>(PyLong_AsNPY_INTPAndOverflow(
      PyList_GET_ITEM(inDimensions, 0), &overflow));
  if (overflow) {
    return PyErr_Format(PyExc_ValueError,
                        "Dimension is too large for this system.");
  }

  PyObject *result = (PyObject *)AllocateNumpyArray(1, &dimension1, dtype,
                                                    itemsize, is_fortran);
  CHECK_MEMORY_ERROR(result);
  return result;
}
...
// NPY_DOUBLE     12
// NPY_LONGDOUBLE 13
bool ConvertSingleItemArray(void *pInput, int16_t numpyInType, _m256all *pDest,
                            int16_t numpyOutType) {
...
bool ConvertScalarObject(PyObject *inObject1, _m256all *pDest,
                         int16_t numpyOutType, void **ppDataIn,
                         int64_t *pItemSize) {
  ...
  *ppDataIn = pDest;
  ...
  if (isPyBool || PyArray_IsScalar((inObject1), Bool)) {
    ...
    if (PyLong_Check(inObject1) || isNumpyScalarInteger) {
    ...
    } else if (PyFloat_Check(inObject1) ||
               PyArray_IsScalar((inObject1), Floating)) {
      ...
      *ppDataIn = PyUnicode_AsUCS4Copy(inObject1);
      ...
      if (PyArray_IsScalar((inObject1), Integer)) {
        ...
        printf("!!unknown numpy scalar type in convertScalarObject %d --",
               numpyOutType);
        return false;
      }
      ...
      LOGGING("!!unknown scalar type in convertScalarObject %d --",
              numpyOutType);
...
static PyObject *TestNumpy(PyObject *self, PyObject *args) {
  ...
  if (PyArray_Check(scalarObject)) {
    ...
    printf("array itemsize=%lld, strides=%lld, flags=%d, length=%lld:\n",
           (int64_t)PyArray_ITEMSIZE(arr), (int64_t)(*PyArray_STRIDES(arr)),
           PyArray_FLAGS(arr), ArrayLength(arr));
...
static PyMethodDef CSigMathUtilMethods[] = {
    ...
    {"RecycleSetGarbageCollectTimeout", RecycleSetGarbageCollectTimeout,
     METH_VARARGS,
     "RecycleSetGarbageCollectTimeout wrapper.  arg1 is long integer timespan"},
    {"RecycleDump", RecycleDump, METH_VARARGS,
     "Dump all objects in reclaimed memory"},
    {"SetRecycleMode", SetRecycleMode, METH_VARARGS,
     "Change recycling mode 0=on  1=off"},
    {"Empty", Empty, METH_VARARGS,
     "similar to np.empty, but tries to allocate from recycler"},

    {"TryRecycleNumpy", TryRecycleNumpy, METH_VARARGS,
     "Try to allocate from recycled array"},
    ...
    {"CompressDecompressArrays", CompressDecompressArrays, METH_VARARGS,
     "CompressDecompressArrays one or more arrays"},
    {"CompressFile", (PyCFunction)CompressFile, METH_VARARGS | METH_KEYWORDS,
     "CompressFile one or more arrays"},
    {"DecompressFile", (PyCFunction)DecompressFile,
     METH_VARARGS | METH_KEYWORDS, "DecompressFile one or more arrays"},
    {"MultiStackFiles", (PyCFunction)MultiStackFiles,
     METH_VARARGS | METH_KEYWORDS,
     "MultiStackFiles one or more files with auto hstack .. arrays"},
    {"MultiDecompressFiles", (PyCFunction)MultiDecompressFiles,
     METH_VARARGS | METH_KEYWORDS,
     "MultiDecompressFile one or more files (returns tuple)"},
    {"MultiPossiblyStackFiles", (PyCFunction)MultiPossiblyStackFiles,
     METH_VARARGS | METH_KEYWORDS,
     "MultiPossiblyStackFiles one or more files (returns tuple)"},
    {"MultiConcatFiles", (PyCFunction)MultiConcatFiles,
     METH_VARARGS | METH_KEYWORDS,
     "MultiConcatFiles one or more files with output=filename"},
    {"SetLustreGateway", SetLustreGateway, METH_VARARGS,
     "SetLustreGateway - give alternate samba servers"},
    ...
    {"IsMemberCategorical", IsMemberCategorical, METH_VARARGS,
     "IsMemberCategorical calculation"},
    ...
    {"MultiKeyGroupBy32", (PyCFunction)MultiKeyGroupBy32,
     METH_VARARGS | METH_KEYWORDS, "MultiKeyGroupBy32 calculation"},
    {"MultiKeyGroupBy32Super", MultiKeyGroupBy32Super, METH_VARARGS,
     "MultiKeyGroupBy32Super calculation"},
    {"MultiKeyUnique32", MultiKeyUnique32, METH_VARARGS,
     "MultiKeyUnique32 calculation"},
    {"MultiKeyIsMember32", MultiKeyIsMember32, METH_VARARGS,
     "MultiKeyIsMember32 calculation"},
    {"MultiKeyAlign32", MultiKeyAlign32, METH_VARARGS,
     "MultiKeyAlign32 calculation"},
    {"MultiKeyRolling", MultiKeyRolling, METH_VARARGS,
     "MultiKeyRolling calculation"},
    ...
    {"MergeBinnedAndSorted", MergeBinnedAndSorted, METH_VARARGS,
     "MergeBinnedAndSorted calculation"},

    {"GroupByPack32", GroupByPack32, METH_VARARGS,
     "GroupByPack32 data from int to float, etc"},
    //{ "GroupByOp32", GroupByOp32, METH_VARARGS, "GroupByOp32 data from int to
    //float, etc" },
    {"GroupByAll32", GroupByAll32, METH_VARARGS,
     "GroupByAll32 data from int to float, etc"},
    {"GroupByAll64", GroupByAll64, METH_VARARGS,
     "GroupByAll64 data from int to float, etc"},
    {"GroupByAllPack32", GroupByAllPack32, METH_VARARGS,
     "GroupByAllPack32 data from int to float, etc"},
    ...
    {"InterpExtrap2d", InterpExtrap2d, METH_VARARGS,
     "Interpolation routine see: np.interp"},

    {"ThreadingMode", ThreadingMode, METH_VARARGS, "Change Threading Mode"},

    {"ConvertSafe", ConvertSafe, METH_VARARGS,
     "Convert data from int to float, etc, preserving invalids"},
    {"ConvertUnsafe", ConvertUnsafe, METH_VARARGS,
     "Convert data from int to float, NOT preserving invalids"},
    {"CombineFilter", CombineFilter, METH_VARARGS,
     "Combine an index and a bool filter"},
    {"CombineAccum2Filter", CombineAccum2Filter, METH_VARARGS,
     "Combine an index and a bool filter"},
    {"CombineAccum1Filter", CombineAccum1Filter, METH_VARARGS,
     "Combine an index and a bool filter"},
    {"MakeiFirst", MakeiFirst, METH_VARARGS,
     "Combine an index and a bool filter, returns iFirst"},
    ...
    {"GetUpcastNum", GetUpcastNum, METH_VARARGS,
     "GetUpcastNum - pass in list of numpy arrays, get dtype num"},
    {"HomogenizeArrays", HomogenizeArrays, METH_VARARGS,
     "HomogenizeArrays - pass in list of numpy arrays, get dtype num"},
    {"ShiftArrays", ShiftArrays, METH_VARARGS,
     "ShiftArrays - pass in list of numpy arrays, shift amoount"},
    ...
    {"BooleanToFancy", (PyCFunction)BooleanToFancy,
     METH_VARARGS | METH_KEYWORDS, "BooleanToFancy functionality"},
    {"ReIndexGroups", ReIndexGroups, METH_VARARGS,
     "reindex categoricals from multistack"},
    {"ReverseShuffle", ReverseShuffle, METH_VARARGS,
     "ReverseShuffle for indexes algo: out[in[i]] = i"},

    {"Reduce", Reduce, METH_VARARGS, "Reduce add,min,max etc"},

    {"AsAnyArray", (PyCFunction)AsAnyArray, METH_VARARGS | METH_KEYWORDS,
     docstring_asarray},
    {"AsFastArray", (PyCFunction)AsFastArray, METH_VARARGS | METH_KEYWORDS,
     docstring_asfastarray},

    {"BasicMathUnaryOp", (PyCFunction)BasicMathUnaryOp,
     METH_VARARGS | METH_KEYWORDS, "BasicMathUnaryOp functionality"},
    {"BasicMathOneInput", BasicMathOneInput, METH_VARARGS,
     "BasicMathOneInput functionality"},
    {"BasicMathTwoInputs", BasicMathTwoInputs, METH_VARARGS,
     "BasicMathTwoInputs functionality"},

    // for low level python hooks
    {"BasicMathHook", BasicMathHook, METH_VARARGS,
     "BasicMathHook functionality (pass in fastarray class, FA, np.ndarray)"},

    {"LedgerFunction", (PyCFunction)LedgerFunction,
     METH_VARARGS | METH_KEYWORDS, "LedgerFunction calculation"},
    {"SetFastArrayType", SetFastArrayType, METH_VARARGS, "SetFastArrayType"},

    {"Sort", Sort, METH_VARARGS,
     "return sorted array, second argument 1=QuickSort, 2=Merge, 3=Heap"},
    {"SortInPlace", SortInPlace, METH_VARARGS,
     "SortInPlace second argument 1=QuickSort, 2=Merge, 3=Heap"},
    {"SortInPlaceIndirect", SortInPlaceIndirect, METH_VARARGS,
     "SortInPlaceIndirect"},
    {"ReIndex", ReIndex, METH_VARARGS, "returns data rearranged by index"},

    {"RemoveTrailingSpaces", RemoveTrailingSpaces, METH_VARARGS,
     "in place removal of trailing spaces (matlab)"},

    {"LexSort64", (PyCFunction)LexSort64, METH_VARARGS | METH_KEYWORDS,
     "LexSort64 returns int64 indexing"},
    {"LexSort32", (PyCFunction)LexSort32, METH_VARARGS | METH_KEYWORDS,
     "LexSort32 returns int32 indexing"},
    {"GroupFromLexSort", (PyCFunction)GroupFromLexSort,
     METH_VARARGS | METH_KEYWORDS,
    ...
    {"RecordArrayToColMajor", RecordArrayToColMajor, METH_VARARGS,
     "Convert record arrays to col major"},

    {"NanInfCountFromSort", NanInfCountFromSort, METH_VARARGS,
     "NanInfCountFromSort"},
    ...
    {"GetNanoTime", GetNanoTime, METH_NOARGS,
     "Get int64 nano time since unix epoch"},
    {"GetTSC", GetTSC, METH_NOARGS, "Get int64 time stamp counter from CPU"},
    {"GetThreadWakeUp", GetThreadWakeUp, METH_NOARGS,
     "Get how many threads wake up to do work."},
    {"SetThreadWakeUp", SetThreadWakeUp, METH_VARARGS,
     "Set how many threads to wake up.  Return the previous value"},

    {"TimeStringToNanos", TimeStringToNanos, METH_VARARGS,
     "Parse string in HH:MM:SS or HH:MM:SS.mmm format to nanos from midnight"},
    {"DateStringToNanos", DateStringToNanos, METH_VARARGS,
     "Parse string in YYYYMMDD or YYYY-MM-DD format to UTC epoch nanos"},
    {"DateTimeStringToNanos", DateTimeStringToNanos, METH_VARARGS,
     ...
     "epoch nanos"},
    {"StrptimeToNanos", StrptimeToNanos, METH_VARARGS,
     "Parse string in strptime  then .mmm to UTC epoch nanos"},
...
PyMODINIT_FUNC PyInit_riptide_cpp() {
  ...
  for (int i = 0; i < NUMPY_LAST_TYPE; i++) {
    NpTypeObjects[i].type =
        (PyTypeObject *)PyArray_TypeObjectFromType(NpTypeObjects[i].typenum);
  }
    ...
    for (int convertType2 = 0; convertType2 < 14; convertType2++) {
      ...
      if (convertType1 == 0) {
        ...
        if (convertType1 == NPY_ULONGLONG) {
          ...
          if (convertType1 < NPY_ULONGLONG && (convertType1 & 1) == 0 &&
              (convertType2 & 1) == 1) {
        ...
        if (convertType1 == convertType2) {
          ...
          if (convertType2 == NPY_ULONGLONG) {
            ...
            if (convertType2 < NPY_ULONGLONG && (convertType2 & 1) == 0 &&
                (convertType1 & 1) == 1) {
...
int GetNumpyType(bool value) { return NPY_BOOL; }
int GetNumpyType(int8_t value) { return NPY_INT8; }
int GetNumpyType(int16_t value) { return NPY_INT16; }
int GetNumpyType(int32_t value) { return NPY_INT32; }
int GetNumpyType(int64_t value) { return NPY_INT64; }
int GetNumpyType(uint8_t value) { return NPY_UINT8; }
int GetNumpyType(uint16_t value) { return NPY_UINT16; }
int GetNumpyType(uint32_t value) { return NPY_UINT32; }
int GetNumpyType(uint64_t value) { return NPY_UINT64; }
int GetNumpyType(float value) { return NPY_FLOAT; }
int GetNumpyType(double value) { return NPY_DOUBLE; }
int GetNumpyType(long double value) { return NPY_LONGDOUBLE; }
int GetNumpyType(char *value) { return NPY_STRING; }

//-------------------------------------
// Returns false if cannot upcast
// Input: numpyInType1, numpyInType2
// Output: convertType1, convertType2
bool GetUpcastType(int numpyInType1, int numpyInType2, int &convertType1,
                   int &convertType2, int64_t funcNumber) {
  if (numpyInType1 == numpyInType2) {
    convertType1 = numpyInType1;
    convertType2 = numpyInType1;
    return true;
  }
  if (numpyInType1 >= 0 && numpyInType1 <= NPY_LONGDOUBLE &&
      numpyInType2 >= 0 && numpyInType2 <= NPY_LONGDOUBLE) {
    stUpCast *pUpcast;
    if (funcNumber >= MATH_OPERATION::CMP_EQ &&
        funcNumber <= MATH_OPERATION::CMP_GTE) {
      pUpcast = &gUpcastTableComparison[numpyInType1 * 14 + numpyInType2];
      LOGGING("special comparison upcast %d %d  to   %d %d\n", numpyInType1,
              numpyInType2, pUpcast->dtype1, pUpcast->dtype2);
    } else {
...
//
int GetStridesAndContig(PyArrayObject const *inArray, int &ndim,
                        int64_t &stride) {
      ...
      if (lastStride == stride) {
        ...
        while (currentdim != 0) {
          ...
          LOGGING("'C' %lld vs %lld  dim: %lld  stride: %lld \n", curStrideLen,
                  PyArray_STRIDE(inArray, currentdim - 1),
        ...
        while (currentdim != (ndims - 1)) {
          ...
          LOGGING("'F' %lld vs %lld  dim:  %lld   stride: %lld \n",
                  ...
                  PyArray_DIM(inArray, currentdim + 1), stride);
          if (PyArray_STRIDE(inArray, currentdim + 1) != curStrideLen)
            break;
          currentdim++;
        }
