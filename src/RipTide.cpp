...
static PyObject * TestNumpy(PyObject * self, PyObject * args)
{
    ...
    // There is no error checking so scalar must be an array - scalar object, and ctypeptr must have enough space to hold the
    // correct type. For flexible - sized types, a pointer to the data is copied into the memory of ctypeptr, for all other types,
...
static PyMethodDef CSigMathUtilMethods[] = {
    ...
    { "DateTimeStringToNanos", DateTimeStringToNanos, METH_VARARGS,
      "Parse string in YYYYMMDD or YYYY-MM-DD format  then HH:MM:SS.mmm to UTC epoch nanos" },
