...
static PyStructSequence_Field SDSArrayCutoffs_fields[] = {
    ...
    {"arrays", "A sequence of arrays created by stacking the columns of one or "
    ...
    {"first_file_header", "A dictionary created from the SDS file header in "
...
static PyStructSequence_Field SDSFileInfo_fields[] = {
    ...
    {"columns", "A sequence of ndarrays (or derived array types) stored in the "
    ...
    {"container_items", "A sequence of objects, each describing an item stored "
                        "within the SDS file."},
    // bytes
...
static PyObject *Create_SDSArrayCutoffs(PyObject *const returnArrayTuple,
...
static PyObject *Create_SDSArrayInfo(PyObject *const arrayShapeTuple,
                                     const int32_t dtype, const int32_t flags,
...
static PyObject *Create_SDSContainerItem(const char *const arrayName,
                                         const unsigned char sdsFlags) {
...
static PyObject *Create_SDSFileInfo(PyObject *const columnMetadataBytes,
...
bool RegisterSdsPythonTypes(PyObject *module_dict) {
  ...
  if (PyStructSequence_InitType2(&PyType_SDSArrayCutoffs,
                                 &SDSArrayCutoffs_desc) < 0) {
    return false;
  }
  if (PyDict_SetItemString(module_dict, "SDSArrayCutoffs",
                           (PyObject *)&PyType_SDSArrayCutoffs) < 0) {
    return false;
  }
  ...
  if (PyDict_SetItemString(module_dict, "SDSFileInfo",
                           (PyObject *)&PyType_SDSFileInfo) < 0) {
    return false;
  }
  ...
  if (PyStructSequence_InitType2(&PyType_SDSContainerItem,
                                 &SDSContainerItem_desc) < 0) {
    return false;
  }
  if (PyDict_SetItemString(module_dict, "SDSContainerItem",
                           (PyObject *)&PyType_SDSContainerItem) < 0) {
    return false;
  }

  if (PyStructSequence_InitType2(&PyType_SDSArrayInfo, &SDSArrayInfo_desc) <
      0) {
    return false;
  }
  if (PyDict_SetItemString(module_dict, "SDSArrayInfo",
                           (PyObject *)&PyType_SDSArrayInfo) < 0) {
    return false;
  }
...
static void ClearErrors() { g_lastexception = 0; }
...
//--------------------------------------------------
//
void *BeginAllowThreads() { return PyEval_SaveThread(); }
...
static SDS_ARRAY_BLOCK *GetArrayBlock(char *baseOffset, int64_t arrayNum) {
  SDS_ARRAY_BLOCK *pArrayBlock = (SDS_ARRAY_BLOCK *)GetMemoryOffset(
      baseOffset, ((SDS_FILE_HEADER *)baseOffset)->ArrayBlockOffset);
...
int64_t BuildListInfo(PyListObject *inListNames, OUT char *pListNames) {
  ...
  for (int i = 0; i < listNameCount; i++) {
    ...
    if (pTuple && PyTuple_Check(pTuple)) {
      ...
      if (PyBytes_Check(pBytes)) {
        int overflow = 0;
        int64_t value = PyLong_AsLongLongAndOverflow(
            PyTuple_GET_ITEM(pTuple, 1), &overflow);
        ...
        *pListNames++ = (uint8_t)value;
        ...
        printf("!!internal error processing, check that list is in bytes "
               "instead of unicode\n");
      }
 ...
 */
PyObject *MakeListNames(const char *pArrayNames, int64_t nameBlockCount,
                        const int64_t nameSize) {
...
// old versions of GCC.
/*constexpr*/ static /*NPY_TYPES*/ int FixupDType(const /*NPY_TYPES*/ int dtype,
...
int SetStringLong(PyObject *pDict, const char *strkey, long long value) {
  return PyDict_SetItemString(pDict, strkey,
                              (PyObject *)PyLong_FromLongLong(value));
}

//----------------------------------------------------
//
PyObject *GetFileHeaderDict(const SDS_FILE_HEADER *const pFileHeader,
                            SDS_FINAL_CALLBACK *const pSDSFinalCallback) {
  ...
  if (pFileHeader && pDict) {
    ...
    SetStringLong(pDict, "TotalArrayCompressedSize",
                  (long long)(pFileHeader->TotalArrayCompressedSize));
    SetStringLong(pDict, "TotalArrayUncompressedSize",
                  (long long)(pFileHeader->TotalArrayUncompressedSize));

    SetStringLong(pDict, "ArrayBlockSize",
                  (long long)(pFileHeader->ArrayBlockSize));
    SetStringLong(pDict, "ArrayBlockOffset",
                  (long long)(pFileHeader->ArrayBlockOffset));

    SetStringLong(pDict, "ArraysWritten",
                  (long long)(pFileHeader->ArraysWritten));
    SetStringLong(pDict, "ArrayFirstOffset",
                  (long long)(pFileHeader->ArrayFirstOffset));

    SetStringLong(pDict, "NameBlockSize",
                  (long long)(pFileHeader->NameBlockSize));
    SetStringLong(pDict, "NameBlockOffset",
                  (long long)(pFileHeader->NameBlockOffset));
    SetStringLong(pDict, "NameBlockCount",
                  (long long)(pFileHeader->NameBlockCount));

    SetStringLong(pDict, "BandBlockSize",
                  (long long)(pFileHeader->BandBlockSize));
    SetStringLong(pDict, "BandBlockOffset",
                  (long long)(pFileHeader->BandBlockOffset));
    SetStringLong(pDict, "BandBlockCount",
                  (long long)(pFileHeader->BandBlockCount));
    SetStringLong(pDict, "BandSize", (long long)(pFileHeader->BandSize));

    SetStringLong(pDict, "SectionBlockSize",
                  (long long)(pFileHeader->SectionBlockSize));
    SetStringLong(pDict, "SectionBlockOffset",
                  (long long)(pFileHeader->SectionBlockOffset));
    SetStringLong(pDict, "SectionBlockCount",
                  (long long)(pFileHeader->SectionBlockCount));
    SetStringLong(pDict, "SectionBlockReservedSize",
                  (long long)(pFileHeader->SectionBlockReservedSize));

    SetStringLong(pDict, "FileOffset", (long long)(pFileHeader->FileOffset));
    SetStringLong(pDict, "TimeStampUTCNanos",
                  (long long)(pFileHeader->TimeStampUTCNanos));
    if (pSDSFinalCallback && pSDSFinalCallback->strFileName) {
      PyDict_SetItemString(
          pDict, "Filename",
          PyUnicode_FromString(pSDSFinalCallback->strFileName));
      ...
      if (pSection && pSection->SectionCount) {
        ...
        for (int64_t n = 0; n < pSection->SectionCount; n++) {
          PyList_SetItem(pSectionList, n,
                         PyUnicode_FromString(pSection->pSectionNames[n]));
          PyList_SetItem(
              pSectionListOffset, n,
              (PyObject *)PyLong_FromLongLong(pSection->pSectionOffsets[n]));
        }
      }
    }
  }
 ...
 */
PyObject *
GetSDSFileInfo(PyObject *const pListName, PyObject *const pystring,
  ...
  for (int64_t i = 0; i < arrayCount; i++) {
    ...
    for (int64_t j = 0; j < pArrayBlock->NDim; j++) {
      PyTuple_SET_ITEM(shapeTuple, j,
                       PyLong_FromLongLong(pArrayBlock->Dimensions[j]));
    }
    ...
    //       across OSes.
    const int FixedUpDtype =
        FixupDType(pArrayBlock->DType, pArrayBlock->ItemSize);

    // Build the SDSArrayInfo struct sequence (C-API namedtuple) instance for
    // this array.
    PyObject *array_info = Create_SDSArrayInfo(
        shapeTuple, FixedUpDtype, pArrayBlock->Flags, pArrayBlock->ItemSize);
  ...
  PyObject *returnFileInfo =
      Create_SDSFileInfo(pystring, numpyArrayTuple, pListName, pDict);
...
void *ReadFromSharedMemory(SDS_SHARED_MEMORY_CALLBACK *pSMCB) {
  ...
  if (mode == COMPRESSION_MODE_INFO) {
    return GetSDSFileInfo(pListName, pystring, arrayCount,
                          GetArrayBlock(baseOffset, 0), pFileHeader);
  }
  ...
  for (int64_t i = 0; i < arrayCount; i++) {
    ...
    PyArrayObject *pArray = AllocateNumpyArrayForData(
        pArrayBlock->NDim, (npy_intp *)pArrayBlock->Dimensions, dtype,
        pArrayBlock->ItemSize, data, pArrayBlock->Flags,
  ...
  PyObject *returnTupleTuple =
      Create_SDSFileInfo(pystring, returnTuple, pListName, pDict);
  return returnTupleTuple;
}

//--------------------------------------------
// Wrap arrays
PyObject *ReadFinalStackArrays(SDS_STACK_CALLBACK *pSDSFinalCallback,
  ...
  for (int t = 0; t < arraysWritten; t++) {
    ...
    PyObject *sdsContainerItem = Create_SDSContainerItem(
        pSDSFinalCallback[t].ArrayName, pSDSFinalCallback[t].ArrayEnum);

    // pylist_append will add a reference count but setitem will not
    PyList_SET_ITEM(pyListName, t, sdsContainerItem);

    //==============
    PyArrayObject *pOffsetArray =
        AllocateNumpyArray(1, (npy_intp *)&fileCount, NPY_LONGLONG);
    if (pOffsetArray) {
      ...
      memcpy(pOffsets, pSDSFinalCallback[t].pArrayOffsets + 1,
             fileCount * sizeof(int64_t));
  ...
  for (int f = 0; f < fileCount; f++) {
    ...
    PyList_SET_ITEM(pyMeta, f,
                    PyBytes_FromStringAndSize(pSDSFileInfo[f].MetaData,
                                              pSDSFileInfo[f].MetaDataSize));
  }
...
PyObject *ReadFinalWrap(SDS_FINAL_CALLBACK *pSDSFinalCallback) {
  ...
  PyObject *pListName =
      ReadListNamesPython(pSDSFinalCallback->nameData, pFileHeader);
  PyObject *pystring =
      GetMetaData(pSDSFinalCallback->metaData, pSDSFinalCallback->metaSize);
  ...
  if (mode == COMPRESSION_MODE_INFO) {
    ...
    PyObject *returnObject =
        GetSDSFileInfo(pListName, pystring, arraysWritten, pArrayBlocks,
                       pFileHeader, pSDSFinalCallback);
    return returnObject;
  }
  ...
  // Create and return an SDSFileInfo namedtuple.
  PyObject *returnWrap =
      Create_SDSFileInfo(pystring, returnArrayTuple, pListName, pDict);
...
void *ReadFinalStack(SDS_STACK_CALLBACK *pSDSFinalCallback, int64_t finalCount,
                     SDS_STACK_CALLBACK_FILES *pSDSFileInfo,
                     SDS_FILTER *pSDSFilter, int64_t fileCount) {
  ...
  if (finalCount <= 0) {
    ...
    PyObject *returnArrayTuple = ReadFinalStackArrays(
        pSDSFinalCallback, finalCount, pSDSFileInfo, pSDSFilter, fileCount);
...
void CopyUnicodeString(PyObject *pUnicode, char **returnString,
                       int64_t *returnSize) {
...
int64_t GetStringFromDict(const char *dictstring, PyObject *kwargs,
                          char **returnString, int64_t *returnSize) {
  if (!kwargs)
    return 0;
...
PyObject *CompressFile(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (!PyArg_ParseTuple(args, "y#y#OO!iii|y#", &fileName, &fileNameSize,
                        &metaData, &metaDataSize, &inListArrays, &PyList_Type,
                        &inListNames, &compType, &level, &fileType, &shareName,
                        &shareNameSize)) {
  ...
  char *pListNames =
      (char *)WORKSPACE_ALLOC((SDS_MAX_FILENAME * listNameCount) + 8);
  if (!pListNames) {
    return NULL;
  }
  ...
  ArrayInfo *aInfo =
      BuildArrayInfo(inListArrays, &arrayCount, &totalItemSize, false);
  ...
  if (aInfo) {
    ...
    SDSWriteInfo.aInfo =
        (SDSArrayInfo *)WORKSPACE_ALLOC(sizeof(SDSArrayInfo) * arrayCount);
    ...
    for (int64_t i = 0; i < arrayCount; i++) {
      ...
      if (pSrc->pObject) {
        ...
        if (!(pDest->Flags &
              (SDS_ARRAY_C_CONTIGUOUS | SDS_ARRAY_F_CONTIGUOUS))) {
...
void AllocateArrayCallback(SDS_ALLOCATE_ARRAY *pAllocateArray) {
  ...
  if (true) {
    ...
    if (ndim) {
      ...
      LOGGING("Allocating ndim:%d  dim0:%lld  type:%d  itemsize:%lld  flags:%d "
              " strde0:%lld",
              pAllocateArray->ndim, dims[0], pAllocateArray->numpyType,
              pAllocateArray->itemsize, pAllocateArray->numpyFlags, strides[0]);
      ...
      if (pAllocateArray->data) {
        pDestInfo->pArrayObject = AllocateNumpyArrayForData(
            pAllocateArray->ndim, (npy_intp *)dims, pAllocateArray->numpyType,
      ...
      if (pDestInfo->pArrayObject) {
        ...
        // Fill in pDestInfo
        pDestInfo->pData = (char *)PyArray_GETPTR1(
            (PyArrayObject *)(pDestInfo->pArrayObject), 0);
      }
    }
  }
}
  ...
  if (kwargs) {
    ...
    if (maskItem && PyArray_Check(maskItem)) {
      if (PyArray_TYPE((PyArrayObject *)maskItem) == NPY_BOOL) {
        ...
        pRCB->Filter.pBoolMask =
            (bool *)PyArray_GETPTR1((PyArrayObject *)maskItem, 0);
        // Needed
        pRCB->Filter.BoolMaskTrueCount = SumBooleanMask(
            (int8_t *)pRCB->Filter.pBoolMask, pRCB->Filter.BoolMaskLength);
...
PyObject *DecompressFile(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (!PyArg_ParseTuple(args, "y#i|y#", &fileName, &fileNameSize, &mode,
  ...
  //==============================================
  void *result = SDSReadFile(fileName, shareName, folderName, sectionsName,
                             &sdsRI, &sdsRCB);
  ...
  if (!result) {
    PyErr_Format(
        PyExc_ValueError,
        "NULL is returned from SDSReadFile but no error string was found");
  }
...
// names/enums>
//
PyObject *InternalDecompressFiles(PyObject *self, PyObject *args,
                                  PyObject *kwargs, int multiMode) {
  ...
  if (multiMode == SDS_MULTI_MODE_READ_MANY &&
      mode != COMPRESSION_MODE_DECOMPRESS_FILE) {
    multiMode = SDS_MULTI_MODE_READ_MANY_INFO;
  }
  if (multiMode == SDS_MULTI_MODE_STACK_MANY &&
      mode != COMPRESSION_MODE_DECOMPRESS_FILE) {
  ...
  if (multiMode == SDS_MULTI_MODE_CONCAT_MANY) {
    ...
    if (!pOutputName) {
      PyErr_Format(PyExc_ValueError,
                   "The output= must be a filename when concatenating files");
    }
  }
  ...
  if (fileCount) {
    ...
    SDS_MULTI_READ *pMultiRead =
        (SDS_MULTI_READ *)WORKSPACE_ALLOC(sizeof(SDS_MULTI_READ) * fileCount);
    ...
    if (pMultiRead) {
      ...
      result = SDSReadManyFiles(pMultiRead, pInclusionList, pFolderList,
                                pSectionsList, fileCount, multiMode, &sdsRCB);
...
// array names/enums>
//
PyObject *MultiDecompressFiles(PyObject *self, PyObject *args,
                               PyObject *kwargs) {
...
PyObject *MultiPossiblyStackFiles(PyObject *self, PyObject *args,
                                  PyObject *kwargs) {
  return InternalDecompressFiles(self, args, kwargs,
                                 SDS_MULTI_MODE_STACK_OR_READMANY);
}
...
PyObject *SetLustreGateway(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "y#O!", &fileName, &fileNameSize, &PyTuple_Type,
                        &tuple)) {
