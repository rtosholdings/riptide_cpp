...
static PyStructSequence_Field SDSArrayCutoffs_fields[] = {
    ...
    { "array_infos",
      "A sequence of the same length as the sequence provided in the 'arrays' component, where each element of the sequence "
      "contains an object with the name and SDS flags of the corresponding stacked array in 'arrays'." },
    ...
    // typing.Sequence[bytes]
    { "filenames",
      "A sequence containing the ASCII-encoded filenames of SDS files from which data was read from during the stacking "
      "operation." },
...
static PyStructSequence_Field SDSFileInfo_fields[] = {
    // bytes
    { "column_metadata",
      "JSON-formatted metadata for columns (arrays) within the Dataset which have types subclassing FastArray (such as "
      "Categorical or DateTimeNano)." },
    ...
    { "columns",
      "A sequence of ndarrays (or derived array types) stored in the file; or, if reading an SDS file in info-only mode, a "
      "sequence of objects describing the type, shape, etc. of each of the columns stored in the file." },
