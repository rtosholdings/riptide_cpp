...
bool CFileReadWrite::FlushCache(CHAR driveLetter) {
  ...
  HANDLE tempHandle =
      CreateFile(szVolumeName, GENERIC_READ | GENERIC_WRITE,
...
bool CFileReadWrite::Open(const char *fileName, bool writeOption,
...
bool CFileReadWrite::CancelIO() { return CancelIo(Handle); }
...
DWORD CFileReadWrite::ReadChunkAsync(void *buffer, uint32_t count,
                                     DWORD *lastError,
...
bool CFileReadWrite::WaitForLastRead(DWORD *lastError, OVERLAPPED *pos) {
  ...
  if (*lastError == ERROR_IO_PENDING) {
    ...
    if (!bReadDone) {
      *lastError = GetLastError();
      if (*lastError == ERROR_INVALID_PARAMETER) {
        ...
        LogError("!! Error 998, invalid access to memory location %s %d",
                 FileName, *lastError);
      } else {
        LogError("!!Read failed %s %d", FileName, *lastError);
      }
    ...
    LogError("!!! Not calling GetOverlapped because data available? %d",
             *lastError);
  }
  return true;
}
...
DWORD CFileReadWrite::WriteChunkAsync(void *buffer, uint32_t count,
                                      DWORD *lastError, OVERLAPPED *pOverlapped,
                                      bool bWaitOnIO) {
  DWORD n = 0;
  if (!Overlapped) {
...
CFileReadWrite *ReadWriteOpen(const char *fullFileName,
                              bool writeOption = false, bool overlapped = true,
                              bool directIO = false) {
...
DWORD ReadChunkAsync(CFileReadWrite *pReadWrite, void *buffer, uint32_t count,
                     DWORD *lastError, OVERLAPPED *pOverlapped) {

  return pReadWrite->ReadChunkAsync(buffer, count, lastError, pOverlapped);
}

bool ReadWriteClose(CFileReadWrite *pReadWrite) { return pReadWrite->Close(); }
