            ...
            if (isString)
            {
                ...
                auto lambdaBSearchCallback = [](void * callbackArgT, int core, int64_t start, int64_t length) -> bool {
                    ...
                    else
                    {
                        // switch (callbackArg->binmode) {
                        // case 0:
                        //   MakeBinsBSearchUnicode<INT8>(callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                        //   (UINT32*)callbackArg->pBin1, callbackArg->binSize, callbackArg->inputItemSize,
                        //   callbackArg->searchItemSize); break;
                        // case 1:
                        //   MakeBinsBSearchUnicode<INT16>(callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                        //   (UINT32*)callbackArg->pBin1, callbackArg->binSize, callbackArg->inputItemSize,
                        //   callbackArg->searchItemSize); break;
                        // case 2:
                        //   MakeBinsBSearchUnicode<INT32>(callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                        //   (UINT32*)callbackArg->pBin1, callbackArg->binSize, callbackArg->inputItemSize,
                        //   callbackArg->searchItemSize); break;
                        // case 3:
                        //   MakeBinsBSearchUnicode<INT64>(callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                        //   (UINT32*)callbackArg->pBin1, callbackArg->binSize, callbackArg->inputItemSize,
                        //   callbackArg->searchItemSize); break;
                        //}
                    }
