...
//--------------------------------------------------------------------------
typedef void (*REINDEXCALLBACK)(void *pDataOut, void *pDataIn, void *pIndex1,
                                int64_t indexLen, int64_t itemSize);
...
void ReIndexData(void *pDataOut, void *pDataIn, void *pIndex1, int64_t size,
                 int64_t itemSize) {
  U *pOut = (U *)pDataOut;
  U *pIn = (U *)pDataIn;
  T *pIndex = (T *)pIndex1;
...
//--------------------------------------------------------
// T is the type of the index -- usually int32_t or INT64
template <typename T> REINDEXCALLBACK ReIndexDataStep1(int64_t itemSize) {
  switch (itemSize) {
  case 1:
...
REINDEXCALLBACK ReIndexer(int64_t itemSize, int dtypeIndex) {
  ...
  PyErr_Format(PyExc_ValueError, "ReIndexing failed on unknown indexer %d",
               dtypeIndex);
  return NULL;
}
...
static bool ReIndexThreadCallback(struct stMATH_WORKER_ITEM *pstWorkerItem,
  ...
  while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
    ...
    int64_t offsetAdj =
        pstWorkerItem->BlockSize * workBlock * OldCallback->itemSize;
    int64_t indexAdj =
        pstWorkerItem->BlockSize * workBlock * OldCallback->indexTypeSize;

    OldCallback->reIndexCallback(pDataOut + offsetAdj, pDataIn,
                                 pDataIndex + indexAdj, lenX,
                                 stRICallback.itemSize);
...
PyObject *ReIndex(PyObject *self, PyObject *args) {
  ...
  if (mlp.aInfo && mlp.tupleSize > 1) {
    ...
    PyArrayObject *result =
        AllocateLikeResize(mlp.aInfo[1].pObject, arraySize1);
    ...
    if (result) {
      ...
      if (reIndexCallback != NULL) {
        ...
        if (pWorkItem == NULL) {
          ...
          // main thread
          reIndexCallback(pDataOut, pDataIn, pIndex, arraySize1,
                          mlp.aInfo[1].ItemSize);
    ...
    PyErr_Format(PyExc_ValueError, "ReIndex: Must pass in at least two params, "
...
//=================================================================================================================
typedef void (*REMAP_INDEX)(void *pInput1, void *pOutput1, int64_t length,
                            int32_t *pMapper, int64_t maplength);

//--------------------------------------------------------------------------------
template <typename T, typename U>
void ReMapIndex(void *pInput1, void *pOutput1, int64_t length, int32_t *pMapper,
                int64_t maplength) {
...
//--------------------------------------------------------
// T is the type of the index -- usually int32_t or INT64
template <typename T> REMAP_INDEX ReMapIndexStep1(int numpyOutputType) {
...
PyObject *ReMapIndex(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O", &PyArray_Type, &inArr1, &PyArray_Type,
                        &outArr1, &PyArray_Type, &remapArr1))
    return NULL;

  if (PyArray_TYPE(remapArr1) != NPY_INT32) {

    PyErr_Format(PyExc_ValueError,
                 "third arg array must be NPY_int32_t -- not %d",
                 PyArray_TYPE(remapArr1));
  } else {
...
//=================================================================================================================
typedef PyObject *(*NAN_INF_COUNT)(void *pDataIn1, void *pIndex,
                                   int64_t arraySize1, int numpyInType);
...
//-----------------------------------------------------------------------------------------------
template <typename T, typename U>
PyObject *NanInfCount(void *pDataIn1, void *pIndex1, int64_t arraySize1,
                      int numpyInType) {
  ...
  if (numpyInType == NPY_FLOAT || numpyInType == NPY_DOUBLE ||
      numpyInType == NPY_LONGDOUBLE) {
...
template <typename U> NAN_INF_COUNT GetNanInfCount(int numpyInType) {
...
PyObject *NanInfCountFromSort(PyObject *self, PyObject *args) {
  ...
  if (mlp.aInfo && mlp.tupleSize == 2) {
    ...
    if (mlp.aInfo[0].ArrayLength == mlp.aInfo[1].ArrayLength) {
      ...
      PyErr_Format(PyExc_ValueError,
                   "NanInfCountFromSort: Array sizes must match");
    }
...
typedef void (*MAKE_BINS_SORTED)(void *pDataIn1, void *pIndex1, void *pOut1,
                                 int64_t length, double *pBin1, int64_t maxbin,
                                 int64_t nancount, int64_t infcount,
                                 int64_t neginfcount);
typedef void (*MAKE_BINS_BSEARCH)(void *pDataIn1, void *pOut1, int64_t start,
                                  int64_t length, void *pBin1, int64_t maxbin,
                                  int numpyInType);
...
void MakeBinsSorted(void *pDataIn1, void *pSortIndex1, void *pOut1,
                    int64_t length, double *pBin1, int64_t maxbin,
  ...
  LOGGING(
      "Array length %lld   bin length %lld   nancount %lld  neginfcount%lld\n",
...
PyObject *BinsToCutsSorted(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!O!i", &PyArray_Type, &inArr1,
                        &PyArray_Type, &binArr1, &PyArray_Type, &indexArr1,
  ...
  if (!PyArg_ParseTuple((PyObject *)counts, "LLL", &nancount, &infcount,
                        &neginfcount))
    return NULL;
  ...
  if (PyArray_TYPE(binArr1) != NPY_DOUBLE) {
    ...
    PyErr_Format(PyExc_ValueError, "bin array must be float64 -- not %d",
    ...
    if (func != NULL) {
      ...
      if (result) {
        ...
        func(pDataIn1, pIndex1, pDataOut, ArrayLength(inArr1), (double *)pBin1,
             binSize, nancount, infcount, neginfcount);
...
void SearchSortedRight(void *pDataIn1, void *pOut1, const int64_t start,
                       const int64_t length, void *pBin1T, int64_t maxbin1,
                       int numpyInType) {
...
// int64_t nanos
template <typename T, typename V, typename BINTYPE>
void SearchSortedLeft(void *pDataIn1, void *pOut1, const int64_t start,
                      const int64_t length, void *pBin1T, int64_t maxbin1,
                      int numpyInType) {
  const T *pData = (const T *)pDataIn1;
  pData = &pData[start];
...
NPY_INLINE static int STRING_LTEQ(const char *s1, const char *s2, int64_t len1,
                                  int64_t len2) {
...
NPY_INLINE static int STRING_LT(const char *s1, const char *s2, int64_t len1,
                                int64_t len2) {
  const unsigned char *c1 = (unsigned char *)s1;
  const unsigned char *c2 = (unsigned char *)s2;
  int64_t i;
...
NPY_INLINE static int STRING_GTEQ(const char *s1, const char *s2, int64_t len1,
                                  int64_t len2) {
  const unsigned char *c1 = (unsigned char *)s1;
  const unsigned char *c2 = (unsigned char *)s2;
  int64_t i;
...
NPY_INLINE static int STRING_GT(const char *s1, const char *s2, int64_t len1,
                                int64_t len2) {
  const unsigned char *c1 = (unsigned char *)s1;
  const unsigned char *c2 = (unsigned char *)s2;
  int64_t i;
...
void SearchSortedLeftString(void *pDataIn1, void *pOut1, const int64_t start,
                            const int64_t length, char *pBin1T, int64_t maxbin1,
                            int64_t itemSizeValue, int64_t itemSizeBin) {
  ...
  for (int64_t i = 0; i < length; i++) {
    ...
    if (STRING_GT(value, veryfirst, itemSizeValue, itemSizeBin) &&
      ...
      if (STRING_LTEQ(value, &pBin1[first * itemSizeBin], itemSizeValue,
                      itemSizeBin)) {
        pOut[i] = first;
      } else {
        // edge test
...
// int64_t nanos
template <typename OUT_TYPE>
void MakeBinsBSearchString(void *pDataIn1, void *pOut1, const int64_t start,
                           const int64_t length, char *pBin1T, int64_t maxbin1,
                           int64_t itemSizeValue, int64_t itemSizeBin) {
  const char *pData = (const char *)pDataIn1;
  pData = &pData[start];
  ...
  LOGGING("MakeBinsBSearchString Array length %lld   bin length %lld    "
  ...
  for (int64_t i = 0; i < length; i++) {
    ...
    if (*value != 0 &&
        STRING_GTEQ(value, veryfirst, itemSizeValue, itemSizeBin) &&
        STRING_LTEQ(value, verylast, itemSizeValue, itemSizeBin)) {
...
void MakeBinsBSearchFloat(void *pDataIn1, void *pOut1, const int64_t start,
                          const int64_t length, void *pBin1T, int64_t maxbin1,
                          int numpyInType) {
  const T *pData = (const T *)pDataIn1;
  pData = &pData[start];
  ...
  LOGGING("MakeBinsBSearchFloat Array length %lld   bin length %lld    bintype "
          "%d\n",
...
// int64_t nanos
template <typename T, typename OUT_TYPE, typename BINTYPE>
void MakeBinsBSearch(void *pDataIn1, void *pOut1, const int64_t start,
                     const int64_t length, void *pBin1T, int64_t maxbin1,
                     int numpyInType) {
  const T *pData = (const T *)pDataIn1;
  pData = &pData[start];
...
template <typename OUT_TYPE>
MAKE_BINS_BSEARCH GetMakeBinsBSearch(int numpyInType, int binType,
                                     int searchMode) {
  MAKE_BINS_BSEARCH result = NULL;
  ...
  case NPY_INT16:
    result =
        GetMakeBinsBSearchPart2<OUT_TYPE, int16_t>(numpyInType, searchMode);
    break;
  CASE_NPY_INT32:
    result =
        GetMakeBinsBSearchPart2<OUT_TYPE, int32_t>(numpyInType, searchMode);
    break;
    // NO UINT8/16/32?
  CASE_NPY_INT64:

    result =
        GetMakeBinsBSearchPart2<OUT_TYPE, int64_t>(numpyInType, searchMode);
    break;
  CASE_NPY_UINT64:

    result =
        GetMakeBinsBSearchPart2<OUT_TYPE, uint64_t>(numpyInType, searchMode);
    break;
  case NPY_FLOAT:
...
PyObject *BinsToCutsBSearch(PyObject *self, PyObject *args) {
  ...
  if (func != NULL || isString) {
    ...
    if (result) {
      ...
      if (isString) {
        ...
        auto lambdaBSearchCallback = [](void *callbackArgT, int core,
                                        int64_t start, int64_t length) -> bool {
          BSearchCallbackStruct *callbackArg =
              (BSearchCallbackStruct *)callbackArgT;
          ...
          if (callbackArg->numpyInType == NPY_STRING) {
            ...
            case 0:
              MakeBinsBSearchString<int8_t>(
                  callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                  (char *)callbackArg->pBin1, callbackArg->binSize,
                  callbackArg->inputItemSize, callbackArg->searchItemSize);
              break;
            case 1:
              MakeBinsBSearchString<int16_t>(
                  callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                  (char *)callbackArg->pBin1, callbackArg->binSize,
                  callbackArg->inputItemSize, callbackArg->searchItemSize);
              break;
            case 2:
              MakeBinsBSearchString<int32_t>(
                  callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                  (char *)callbackArg->pBin1, callbackArg->binSize,
                  callbackArg->inputItemSize, callbackArg->searchItemSize);
              break;
            case 3:
              MakeBinsBSearchString<int64_t>(
                  callbackArg->pDataIn1, callbackArg->pDataOut, start, length,
                  (char *)callbackArg->pBin1, callbackArg->binSize,
                  callbackArg->inputItemSize, callbackArg->searchItemSize);
              break;
        ...
        g_cMathWorker->DoMultiThreadedChunkWork(
            lengthData, lambdaBSearchCallback, &stBSearchCallback);
        ...
        auto lambdaBSearchCallback = [](void *callbackArgT, int core,
          ...
          callbackArg->func(callbackArg->pDataIn1, callbackArg->pDataOut, start,
                            length, callbackArg->pBin1, callbackArg->binSize,
                            callbackArg->numpyInType);
          return true;
        };
  ...
  PyErr_Format(PyExc_ValueError,
               "bin array must be int32/int64/float32/float64 -- not %d",
               PyArray_TYPE(binArr1));
