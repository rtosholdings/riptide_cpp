...
PyObject* ProcessOneInput(
   PyArrayObject* inArray, 
   PyArrayObject* outObject1,
   int funcNumber, 
   int numpyInType1, 
   int numpyOutType = -1,
   int axis = 0) {
   ...
   if (directionIn == 0 && numpyOutType == -1) {
      ...
      if (pUnaryFunc) {
         ...
         PyArrayObject* outputArray= 
          (ndim <= 1) ?  AllocateNumpyArray(1, (npy_intp*)&len, wantedOutputType) :
...
BasicMathOneInput(PyObject *self, PyObject *args) {
   ...
   if (tupleSize == 2) {
      ...
      if (!IsFastArrayOrNumpy(outObject1)) {
         PyErr_Format(PyExc_ValueError, "BasicMathOneInput out argument is not an array but type %s", outObject1->ob_base.ob_type->tp_name);
         return NULL;
      }
   ...
   return
   ProcessOneInput((PyArrayObject*)inObject1, outObject1, (int)funcNumber, numpyInType1, (int)numpyOutType);
}
...
BasicMathOneInputFromNumber(PyArrayObject* inObject1, INT64 funcNumber, BOOL inplace) {
   ...
   // returns None when punting
   return
   ProcessOneInput((PyArrayObject*)inObject1, outObject1, (int)funcNumber, numpyInType1, numpyOutType);
}
...
BasicMathUnaryOp(PyObject *self, PyObject *args, PyObject *kwargs) {
   ...
   if (kwargs) {
      ...
      if (dtypekwarg) {
         ...
         if (!IsFastArrayOrNumpy(outObject1)) {
            PyErr_Format(PyExc_ValueError, "BasicMathUnaryOp out= kwarg must be an array not type %s", (PyObject*)dtypekwarg->ob_type->tp_name);
            return NULL;
         }
      }
   }
   ...
   if (Py_SIZE(args) > 1) {
      ...
      if (PyLong_CheckExact(funcNumber)) {
         ...
         if (IsFastArrayOrNumpy(inObject)) {
            PyObject* result =
               ProcessOneInput(inObject, NULL, (int)funcNum, PyArray_TYPE(inObject), -1);
            return result;
         }
         else {

            PyErr_Format(PyExc_ValueError, "BasicMathUnaryOp first argument must be an array not type %s", ((PyObject*)inObject)->ob_type->tp_name);
         }
      }
      else {
         PyErr_Format(PyExc_ValueError, "BasicMathUnaryOp second argument must be an integer not type %s", funcNumber->ob_type->tp_name);
      }
   }
   else {
