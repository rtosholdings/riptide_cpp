...
#define SDS_FLAGS_META                                                         \
  16 // This array is string or unicode that contains meta data (from onefile)
...
#define SDS_STACK_TYPE_CONCAT                                                  \
  1 // When sds_concat is called and reader wants to stack
...
struct SDS_FILE_HEADER {
  ...
  //----- offset 208 -----
  char
      Reserved[512 - 208]; // BUGBUG This should be done using align statements,
  ...
  int64_t AddArrayCompressedSize(int64_t cSize) {
    ...
    int64_t fileOffset =
        InterlockedAdd64(&TotalArrayCompressedSize, padSize) - padSize;
...
public:
  ...
  void MakeListSections(const int64_t sectionBlockCount,
...
typedef void *(*SDS_READ_FINAL_CALLBACK)(SDS_FINAL_CALLBACK *pSDSFinalCallback,
                                         int64_t finalCount);

// Called at the end of reading a stacked file
typedef void *(*SDS_STACK_FINAL_CALLBACK)(
    SDS_STACK_CALLBACK *pSDSFinalCallback, int64_t finalCount,
    SDS_STACK_CALLBACK_FILES *pSDSFileInfo, SDS_FILTER *pSDSFilter,
    int64_t fileCount);

// Only called when reading from shared memory
typedef void *(*SDS_READ_SHARED_MEMORY_CALLBACK)(
    SDS_SHARED_MEMORY_CALLBACK *pSDSMemoryCallback);
...
typedef void (*SDS_ALLOCATE_ARRAY_CALLBACK)(
    SDS_ALLOCATE_ARRAY *pSDSAllocateArray);
...
DllExport void *SDSReadFile(
    const char *fileName, const char *shareName, SDS_STRING_LIST *pFolderName,
    SDS_STRING_LIST *pSectionsName, // can be NULL
    SDS_READ_INFO
        *pReadInfo, // has mode= COMPRESSION_MODE_DECOMPRESS_FILE OR INFO MODE
...
typedef bool (*SDS_WRITE_FILE)(const char *, const char *, SDS_WRITE_INFO *,
                               SDS_WRITE_CALLBACKS *);
typedef void *(*SDS_READ_FILE)(const char *, const char *, SDS_READ_INFO *,
                               SDS_READ_CALLBACKS *);
typedef void *(*SDS_READ_MANY_FILES)(SDS_MULTI_READ *, SDS_STRING_LIST *,
                                     int64_t, int, SDS_READ_CALLBACKS *);
...
//===================================================================
void *_SDSReadFile(
    const char *dllLoadPath, const char *fileName, const char *shareName,
...
void *_SDSReadManyFiles(const char *dllLoadPath, SDS_MULTI_READ *pMultiRead,
  ...
  if (g_hModule != NULL) {
    ...
    if (g_fpReadManyFiles == NULL) {
      g_fpReadManyFiles =
          (SDS_READ_MANY_FILES)GetProcAddress(g_hModule, "SDSReadManyFiles");
    }

    if (g_fpReadManyFiles != NULL) {
      return g_fpReadManyFiles(pMultiRead, pInclusionList, fileCount, multiMode,
                               pReadCallbacks);
    }
  }
  return NULL;
...
char *_SDSGetLastError(const char *dllLoadPath) {
  ...
  if (g_hModule != NULL) {
    ...
    if (g_fpGetLastError == NULL) {
      g_fpGetLastError =
          (SDS_GET_LAST_ERROR)GetProcAddress(g_hModule, "SDSGetLastError");
    }
...
void _SDSClearBuffers(const char *dllLoadPath) {
  ...
  if (g_hModule != NULL) {
    ...
    if (g_fpClearBuffers == NULL) {
      g_fpClearBuffers =
          (SDS_CLEAR_BUFFERS)GetProcAddress(g_hModule, "SDSClearBuffers");
    }
