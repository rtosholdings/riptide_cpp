void mat_interp_extrap(double * x, double * xp, double * yp, double * out, int N, int M)
{
    for ( int i = 0; i < N; ++i )
    {
        ...
        {
            out[i] = (yp[j + offset] - yp[j - 1 + offset]) * (x[i] - xp[j - 1 + offset]) / (xp[j + offset] - xp[j - 1 + offset]) +
                     yp[j - 1 + offset];
        }
    }
}
...
static PyObject * interp_extrap_2d(PyObject * self, PyObject * args)
{
    ...
    if ( PyArray_Check(arr) == 0 || PyArray_Check(xp) == 0 || PyArray_Check(yp) == 0 || PyArray_Check(out) == 0 )
        return NULL;
    ...
    if ( PyArray_NDIM(xp) != 2 || PyArray_NDIM(yp) != 2 )
        return NULL;
    if ( PyArray_TYPE(arr) != NPY_DOUBLE || PyArray_TYPE(xp) != NPY_DOUBLE || PyArray_TYPE(yp) != NPY_DOUBLE ||
         PyArray_TYPE(out) != NPY_DOUBLE )
        return NULL;
    int N = static_cast<int>(PyArray_DIM(arr, 0));
    if ( N != static_cast<int>(PyArray_DIM(xp, 0)) || N != static_cast<int>(PyArray_DIM(yp, 0)) )
        return NULL;
    int M = static_cast<int>(PyArray_DIM(xp, 1));
    if ( M != static_cast<int>(PyArray_DIM(yp, 1)) )
        return NULL;
    if ( ! (PyArray_FLAGS(xp) & NPY_ARRAY_C_CONTIGUOUS > 0) || ! (PyArray_FLAGS(yp) & NPY_ARRAY_C_CONTIGUOUS > 0) )
        return NULL;
