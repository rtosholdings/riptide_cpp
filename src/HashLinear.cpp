...
template <typename T> struct numpy_type_code {
  static constexpr int value =
      -1; // -1 is an invalid code, because if something uses this
          // specialization we want it to break.
};

template <> struct numpy_type_code<int32_t> {
  static constexpr int value = NPY_INT32;
};

template <> struct numpy_type_code<int64_t> {
...
int64_t PRIME_NUMBERS[] = {
    ...
    805306457,   1610612741,  3002954501, 4294967291, 8589934583, 17179869143,
    34359738337, 68719476731, 0};
...
FORCE_INLINE
bool UNICODE_MATCH2(const char *str1T, const char *str2T, int64_t strWidth1,
                    int64_t strWidth2) {
  const uint32_t *str1 = (const uint32_t *)str1T;
  const uint32_t *str2 = (const uint32_t *)str2T;
  strWidth1 /= 4;
...
FORCE_INLINE
bool STRING_MATCH2(const char *str1, const char *str2, int64_t strWidth1,
                   int64_t strWidth2) {
  while (1) {
...
template <typename T, typename U>
void *CHashLinear<T, U>::AllocMemory(int64_t numberOfEntries,
                                     int64_t sizeofStruct,
                                     int64_t sizeofExtraArray, bool isFloat) {
  ...
  LOGGING(
      "Hash size selected %llu for NumEntries %llu -- allocation size %llu\n",
...
template <typename T, typename U>
FORCE_INLINE void
CHashLinear<T, U>::InternalGetLocation(U i, HashLocation *pLocation,
                                       int8_t *pBoolOutput, U *pLocationOutput,
...
// Returns two arrays
template <typename T, typename U>
FORCE_INLINE void CHashLinear<T, U>::InternalGetLocationCategorical(
    U i, HashLocation *pLocation, U *pLocationOutput, T item, uint64_t hash,
    int64_t *missed) {
...
template <typename T, typename U>
FORCE_INLINE void
CHashLinear<T, U>::InternalSetLocation(U i, HashLocation *pLocation, T item,
                                       uint64_t hash) {
...
//
template <typename T, typename U>
void CHashLinear<T, U>::MakeHashLocationMK(int64_t arraySize, T *pInput,
  ...
  LOGGING("in mkhashlocationmk asize: %llu   isize: %llu  HashSize:%lld  "
          "sizeofU:%d\n",
...
void FindLastMatchCategorical(int64_t arraySize1, int64_t arraySize2, U *pKey1,
                              U *pKey2, V *pVal1, V *pVal2, U *pLocationOutput,
...
template <typename T, typename U>
template <typename V>
void CHashLinear<T, U>::FindLastMatchMK(int64_t arraySize1, int64_t arraySize2,
                                        T *pKey1, T *pKey2, V *pVal1, V *pVal2,
                                        U *pLocationOutput,
                                        int64_t totalItemSize,
                                        bool allowExact) {
...
template <typename T, typename U>
template <typename V>
void CHashLinear<T, U>::FindNextMatchMK(int64_t arraySize1, int64_t arraySize2,
                                        T *pKey1, T *pKey2, V *pVal1, V *pVal2,
                                        U *pLocationOutput,
                                        int64_t totalItemSize,
                                        bool allowExact) {
...
//
template <typename T, typename U>
static void IsMemberMK(void *pHashLinearVoid, int64_t arraySize, void *pInputT,
                       void *pInput2T, int8_t *pBoolOutput,
  ...
  LOGGING("in ismembermk  asize: %llu   isize: %llu  %p   %p  %p\n", arraySize,
          totalItemSize, pBitFields, pLocationOutput, pBoolOutput);
...
//(HashSize-1);
#define HASH_int32_t                                                           \
  uint64_t h = _mm_crc32_u32(0, (uint32_t)item) & (HashSize - 1);
...
#define HASH_INT128                                                            \
  uint64_t h = fasthash64_16((uint64_t *)&pInput[i]) & (HashSize - 1);
...
//
template <typename T, typename U>
void CHashLinear<T, U>::MakeHashLocation(int64_t arraySize, T *pHashList,
                                         int64_t hintSize) {
  ...
  if (sizeof(T) <= 2) {
    ...
    if (sizeof(T) == 4) {
      ...
      printf(
          "!!! MakeHashLocation -- hash item size is not 1,2,4, or 8!  %zu\n",
          sizeof(T));
    }
  }
  LOGGING("%llu entries had %llu collisions   %llu unique\n", arraySize,
...
//
template <typename T, typename U>
int64_t CHashLinear<T, U>::IsMemberCategorical(int64_t arraySize, T *pHashList,
                                               U *pLocationOutput) {
  ...
  if (sizeof(T) <= 2) {
    ...
    for (U i = 0; i < arraySize; i++) {
      T item = pHashList[i];
      InternalGetLocationCategorical(i, pLocation, pLocationOutput, item, item,
                                     &missed);
    }
  } else {
    ...
    if (sizeof(T) == 4) {
      ...
      if (HashMode == HASH_MODE_PRIME) {
        for (U i = 0; i < arraySize; i++) {
          T item = pHashList[i];
          HASH_INT1;
          InternalGetLocationCategorical(i, pLocation, pLocationOutput, item, h,
                                         &missed);
        }
      } else {
...
//
template <typename T, typename U>
void IsMember(void *pHashLinearVoid, int64_t arraySize, void *pHashListT,
              int8_t *pBoolOutput, void *pLocationOutputU) {
  ...
  LOGGING("IsMember arraySize %lld   get index size is %zu    output size is "
          "%zu  HashSize is %llu\n",
...
//
template <typename T, typename U>
void CHashLinear<T, U>::MakeHashLocationFloat(int64_t arraySize, T *pHashList,
                                              int64_t hintSize) {
...
//
template <typename T, typename U>
void IsMemberFloat(void *pHashLinearVoid, int64_t arraySize, void *pHashListT,
                   int8_t *pBoolOutput, void *pLocationOutputU) {
...
//
template <typename T, typename U>
int64_t CHashLinear<T, U>::IsMemberFloatCategorical(int64_t arraySize,
                                                    T *pHashList,
                                                    U *pLocationOutput) {
...
typedef PyArrayObject *(COPY_TO_SMALLER_ARRAY)(void *pFirstArrayIndex,
                                               int64_t numUnique,
                                               int64_t totalRows);
//-----------------------------------------------------------------------------------------
...
PyArrayObject *CopyToSmallerArray(void *pFirstArrayIndex, int64_t numUnique,
                                  int64_t totalRows,
  ...
  LOGGING("after alloc numpy copy to smaller %p  %lld %lld %lld\n", pFirstArray,
          numUnique, totalRows, numUnique * sizeof(INDEX_TYPE));
...
uint64_t CHashLinear<T, U>::GroupByFloat(
    int64_t totalRows, int64_t totalItemSize, T *pInput,
...
uint64_t CHashLinear<T, U>::GroupByItemSize(
  ...
  LOGGING("GroupByItem: hintSize %lld   HashSize %llu  totalRows %lld   "
...
uint64_t CHashLinear<T, U>::GroupBy(
    int64_t totalRows, int64_t totalItemSize, const char *pInput,
      ...
      while (1) {
        if (pBitFieldsX[hash >> 6] & (1LL << (hash & 63))) {
          ...
          if (numCollisions < 0) {
            ...
            printf("!!! error in groupby collisions too high -- trying to "
                   "match size %lld\n",
                   totalItemSize);
            printf("%llu entries   hashSize %llu  had %llu collisions   %llu "
                   "unique\n",
...
template <typename T, typename U>
uint64_t CHashLinear<T, U>::Unique(int64_t totalRows, int64_t totalItemSize,
                                   const char *pInput,
  ...
  LOGGING("Unique: hintSize:%lld   HashSize:%llu  sizeoftypeU:%lld   "
          "sizeoftypeT:%lld\n",
  ...
  LOGGING("%llu entries   hashSize %lld    %lld unique\n", totalRows,
          (int64_t)HashSize, (int64_t)NumUnique);
...
void CHashLinear<T, U>::MultiKeyRolling(
    ...
    // Return values
    U *pIndexArray, U *pRunningCountArray, HASH_MODE hashMode,
...
//
template <typename T, typename U>
void CHashLinear<T, U>::MakeHashLocationString(int64_t arraySize,
                                               const char *pHashList,
                                               int64_t strWidth,
                                               int64_t hintSize,
                                               bool isUnicode) {
...
template <typename T, typename U>
FORCE_INLINE void
CHashLinear<T, U>::InternalSetLocationString(U i, HashLocation *pLocation,
                                             const char *strValue,
...
template <typename T, typename U>
FORCE_INLINE void
CHashLinear<T, U>::InternalSetLocationUnicode(U i, HashLocation *pLocation,
                                              const char *strValue,
                                              int64_t strWidth, uint64_t hash) {
  ...
  while (IsBitSet(hash)) {
    // Check if we have a match from before
    if (UNICODE_MATCH((const char *)pLocation[hash].value, strValue,
                      strWidth)) {
      return;
    }
...
// Returns two arrays
template <typename T, typename U>
FORCE_INLINE void CHashLinear<T, U>::InternalGetLocationString(
    int64_t i, HashLocation *pLocation, int8_t *pBoolOutput, U *pLocationOutput,
...
// Returns two arrays
template <typename T, typename U>
FORCE_INLINE void CHashLinear<T, U>::InternalGetLocationUnicode(
    int64_t i, HashLocation *pLocation, int8_t *pBoolOutput, U *pLocationOutput,
    const char *strValue, int64_t strWidth, uint64_t hash) {
...
// Returns two arrays
template <typename T, typename U>
FORCE_INLINE void CHashLinear<T, U>::InternalGetLocationString2(
    int64_t i, HashLocation *pLocation, int8_t *pBoolOutput, U *pLocationOutput,
  ...
  while (IsBitSet(hash)) {
    ...
    if (STRING_MATCH2((const char *)pLocation[hash].value, strValue, strWidth,
                      strWidth2)) {
      // printf("match\n");
      // return the first location
      pLocationOutput[i] = pLocation[hash].Location;
...
// Returns two arrays
template <typename T, typename U>
FORCE_INLINE void CHashLinear<T, U>::InternalGetLocationUnicode2(
    int64_t i, HashLocation *pLocation, int8_t *pBoolOutput, U *pLocationOutput,
    const char *strValue, int64_t strWidth, int64_t strWidth2, uint64_t hash) {
  ...
  while (IsBitSet(hash)) {
    ...
    // if ((i % 1000) == 0) printf("%d  Comparing2 %s to %s\n", (int)i, (const
    // char*)pLocation[hash].value, strValue);
    if (UNICODE_MATCH2((const char *)pLocation[hash].value, strValue, strWidth,
                       strWidth2)) {
      // printf("match\n");
      // return the first location
      pLocationOutput[i] = pLocation[hash].Location;
...
// Returns two arrays
template <typename T, typename U>
FORCE_INLINE void CHashLinear<T, U>::InternalGetLocationStringCategorical(
    int64_t i, HashLocation *pLocation, U *pLocationOutput,
    const char *strValue, int64_t strWidth, uint64_t hash, int64_t *missed) {
...
// Returns two arrays
template <typename T, typename U>
FORCE_INLINE void CHashLinear<T, U>::InternalGetLocationString2Categorical(
    int64_t i, HashLocation *pLocation, U *pLocationOutput,
    const char *strValue, int64_t strWidth, int64_t strWidth2, uint64_t hash,
    int64_t *missed) {
...
template <typename T, typename U>
static int64_t
IsMemberStringCategorical(void *pHashLinearVoid, int64_t arraySize,
                          int64_t strWidth, int64_t strWidth2,
                          const char *pHashList, void *pLocationOutputU,
                          bool isUnicode) {
    ...
    if (isUnicode) {
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        while (1) {
          ...
          if (pBitFields[index] & (1LL << (hash & 63))) {
            // Check if we have a match from before
            // Check if we have a match from before
            if (UNICODE_MATCH((const char *)pLocation[hash].value, strStart,
                              strWidth)) {
              // return the first location
              pLocationOutput[i] = pLocation[hash].Location + 1;
              break;
      ...
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        while (1) {
          ...
          if (pBitFields[index] & (1LL << (hash & 63))) {
            // Check if we have a match from before
            if (STRING_MATCH((const char *)pLocation[hash].value, strStart,
                             strWidth)) {
              // return the first location
              pLocationOutput[i] = pLocation[hash].Location + 1;
              break;
    ...
    if (isUnicode) {
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        while (1) {
          ...
          if (pBitFields[index] & (1LL << (hash & 63))) {
            // Check if we have a match from before
            // Check if we have a match from before
            if (UNICODE_MATCH2((const char *)pLocation[hash].value, strStart,
                               strWidth2, strWidth)) {
              // return the first location
              pLocationOutput[i] = pLocation[hash].Location + 1;
              break;
      ...
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        while (1) {
          ...
          if (pBitFields[index] & (1LL << (hash & 63))) {
            // Check if we have a match from before
            if (STRING_MATCH2((const char *)pLocation[hash].value, strStart,
                              strWidth2, strWidth)) {
              // return the first location
              pLocationOutput[i] = pLocation[hash].Location + 1;
              break;
...
template <typename T, typename U>
void CHashLinear<T, U>::IsMemberString(int64_t arraySize, int64_t strWidth,
                                       int64_t strWidth2, const char *pHashList,
                                       int8_t *pBoolOutput, U *pLocationOutput,
                                       bool isUnicode) {
  ...
  if (strWidth == strWidth2) {
    ...
    if (isUnicode) {
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        hash = hash & (HashSize - 1);
        // printf("**uni hash %lld\n", (long long)hash);
        InternalGetLocationUnicode(i, pLocation, pBoolOutput, pLocationOutput,
                                   strStart, strWidth, hash);
      }
    } else {
      ...
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        // Use and mask to strip off high bits
        hash = hash & (HashSize - 1);
        InternalGetLocationString(i, pLocation, pBoolOutput, pLocationOutput,
                                  strStart, strWidth, hash);
      }
    }
  } else {
    ...
    if (isUnicode) {
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        // Use and mask to strip off high bits
        hash = hash & (HashSize - 1);
        InternalGetLocationUnicode2(i, pLocation, pBoolOutput, pLocationOutput,
                                    strStart, strWidth2, strWidth, hash);
      }
    } else {
      ...
      for (int64_t i = 0; i < arraySize; i++) {
        ...
        // Use and mask to strip off high bits
        hash = hash & (HashSize - 1);
        InternalGetLocationString2(i, pLocation, pBoolOutput, pLocationOutput,
                                   strStart, strWidth2, strWidth, hash);
      }
    }
  }
...
typedef void (*ISMEMBER_MT)(void *pHashLinearVoid, int64_t arraySize,
                            void *pHashList, int8_t *pBoolOutput,
...
static bool IMMTThreadCallback(struct stMATH_WORKER_ITEM *pstWorkerItem,
                               int core, int64_t workIndex) {
  ...
  while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
    ...
    int64_t inputAdj =
        pstWorkerItem->BlockSize * workBlock * Callback->typeSizeIn;
    int64_t outputAdj =
        pstWorkerItem->BlockSize * workBlock * Callback->typeSizeOut;
    int64_t boolAdj = pstWorkerItem->BlockSize * workBlock;

    Callback->anyIMMTCallback(
        Callback->pHashLinearVoid, lenX, pInput1 + inputAdj,
        Callback->pBoolOutput + boolAdj, pOutput + outputAdj);
...
static void IsMemberMultiThread(ISMEMBER_MT pFunction, void *pHashLinearVoid,
                                int64_t arraySize, void *pHashList,
                                int8_t *pBoolOutput, void *pLocationOutputU,
                                int64_t sizeInput, int64_t sizeOutput) {
  ...
  if (pWorkItem == NULL) {
    ...
    // thread
    pFunction(pHashLinearVoid, arraySize, pHashList, pBoolOutput,
              pLocationOutputU);
  } else {
...
void *IsMemberHash32(int64_t size1, void *pInput1,
                     ...
                     int8_t *pBoolOutput, int32_t sizeType, HASH_MODE hashMode,
                     int64_t hintSize) {
  ...
  switch (sizeType) {
  case 1: {
    CHashLinear<uint8_t, U> *pHashLinear =
        new CHashLinear<uint8_t, U>(HASH_MODE_MASK);
    pHashLinear->MakeHashLocation(size2, (uint8_t *)pInput2, 256 / 2);
    IsMemberMultiThread(IsMember<uint8_t, U>, pHashLinear, size1,
                        (uint8_t *)pInput1, pBoolOutput, (U *)pOutput,
                        sizeof(uint8_t), sizeof(U));
    delete pHashLinear;
    return NULL;
  } break;
  case 2: {
    CHashLinear<uint16_t, U> *pHashLinear =
        new CHashLinear<uint16_t, U>(HASH_MODE_MASK);
    pHashLinear->MakeHashLocation(size2, (uint16_t *)pInput2, 65536 / 2);
    IsMemberMultiThread(IsMember<uint16_t, U>, pHashLinear, size1,
                        (uint16_t *)pInput1, pBoolOutput, (U *)pOutput,
  ...
  case 4: {
    CHashLinear<uint32_t, U> *pHashLinear =
        new CHashLinear<uint32_t, U>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint32_t *)pInput2, hintSize);
    IsMemberMultiThread(IsMember<uint32_t, U>, pHashLinear, size1,
                        (uint32_t *)pInput1, pBoolOutput, (U *)pOutput,
                        sizeof(uint32_t), sizeof(U));
    delete pHashLinear;
    return NULL;
  } break;
  case 8: {
    CHashLinear<uint64_t, U> *pHashLinear =
        new CHashLinear<uint64_t, U>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint64_t *)pInput2, hintSize);
    IsMemberMultiThread(IsMember<uint64_t, U>, pHashLinear, size1,
                        (uint64_t *)pInput1, pBoolOutput, (U *)pOutput,
  ...
  case 104: {
    ...
    IsMemberMultiThread(IsMemberFloat<float, U>, pHashLinear, size1,
                        (float *)pInput1, pBoolOutput, (U *)pOutput,
  ...
  case 108: {
    ...
    IsMemberMultiThread(IsMemberFloat<double, U>, pHashLinear, size1,
                        (double *)pInput1, pBoolOutput, (U *)pOutput,
...
void *IsMemberHash64(int64_t size1, void *pInput1, int64_t size2, void *pInput2,
                     int64_t *pOutput, int8_t *pBoolOutput, int32_t sizeType,
                     HASH_MODE hashMode, int64_t hintSize) {

  switch (sizeType) {
  case 1: {
    CHashLinear<uint8_t, int64_t> *pHashLinear =
        new CHashLinear<uint8_t, int64_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint8_t *)pInput2, hintSize);
    IsMember<uint8_t, int64_t>(pHashLinear, size1, (uint8_t *)pInput1,
                               pBoolOutput, (int64_t *)pOutput);
    delete pHashLinear;
    return NULL;
  } break;
  case 2: {
    CHashLinear<uint16_t, int64_t> *pHashLinear =
        new CHashLinear<uint16_t, int64_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint16_t *)pInput2, hintSize);
    IsMember<uint16_t, int64_t>(pHashLinear, size1, (uint16_t *)pInput1,
                                pBoolOutput, (int64_t *)pOutput);
    delete pHashLinear;
    return NULL;
  } break;
  ...
  case 4: {
    CHashLinear<uint32_t, int64_t> *pHashLinear =
        new CHashLinear<uint32_t, int64_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint32_t *)pInput2, hintSize);
    IsMember<uint32_t, int64_t>(pHashLinear, size1, (uint32_t *)pInput1,
                                pBoolOutput, (int64_t *)pOutput);
    delete pHashLinear;
    return NULL;
  } break;
  case 8: {
    CHashLinear<uint64_t, int64_t> *pHashLinear =
        new CHashLinear<uint64_t, int64_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint64_t *)pInput2, hintSize);
    IsMember<uint64_t, int64_t>(pHashLinear, size1, (uint64_t *)pInput1,
                                pBoolOutput, (int64_t *)pOutput);
    delete pHashLinear;
    return NULL;
  } break;
  case 104: {
    CHashLinear<float, int64_t> *pHashLinear =
        new CHashLinear<float, int64_t>(hashMode);
    pHashLinear->MakeHashLocationFloat(size2, (float *)pInput2, hintSize);
    IsMemberFloat<float, int64_t>(pHashLinear, size1, (float *)pInput1,
                                  pBoolOutput, (int64_t *)pOutput);
    delete pHashLinear;
    return NULL;
  } break;
  case 108: {
    CHashLinear<double, int64_t> *pHashLinear =
        new CHashLinear<double, int64_t>(hashMode);
    pHashLinear->MakeHashLocationFloat(size2, (double *)pInput2, hintSize);
    IsMemberFloat<double, int64_t>(pHashLinear, size1, (double *)pInput1,
                                   pBoolOutput, (int64_t *)pOutput);
    delete pHashLinear;
    return NULL;
  } break;
...
// Returns in pOutput: index location of second arg -- where first arg found in
// second arg Returns in pBoolOutput: True if found, False otherwise
int64_t IsMemberHashCategorical(int64_t size1, void *pInput1, int64_t size2,
                                void *pInput2, int32_t *pOutput,
                                int32_t sizeType, HASH_MODE hashMode,
                                int64_t hintSize) {
  ...
  switch (sizeType) {
  case 1: {
    CHashLinear<uint8_t, int32_t> *pHashLinear =
        new CHashLinear<uint8_t, int32_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint8_t *)pInput2, hintSize);
    missed =
        pHashLinear->IsMemberCategorical(size1, (uint8_t *)pInput1, pOutput);
    delete pHashLinear;
    return missed;
  } break;

  case 2: {
    CHashLinear<uint16_t, int32_t> *pHashLinear =
        new CHashLinear<uint16_t, int32_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint16_t *)pInput2, hintSize);
    missed =
        pHashLinear->IsMemberCategorical(size1, (uint16_t *)pInput1, pOutput);
    delete pHashLinear;
    return missed;
  } break;

  case 4: {
    CHashLinear<uint32_t, int32_t> *pHashLinear =
        new CHashLinear<uint32_t, int32_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint32_t *)pInput2, hintSize);
    missed =
        pHashLinear->IsMemberCategorical(size1, (uint32_t *)pInput1, pOutput);
    delete pHashLinear;
    return missed;
  } break;
  case 8: {
    CHashLinear<uint64_t, int32_t> *pHashLinear =
        new CHashLinear<uint64_t, int32_t>(hashMode);
    pHashLinear->MakeHashLocation(size2, (uint64_t *)pInput2, hintSize);
    missed =
        pHashLinear->IsMemberCategorical(size1, (uint64_t *)pInput1, pOutput);
    delete pHashLinear;
    return missed;
  } break;
  case 104: {
    CHashLinear<float, int32_t> *pHashLinear =
        new CHashLinear<float, int32_t>(hashMode);
    pHashLinear->MakeHashLocationFloat(size2, (float *)pInput2, hintSize);
    missed =
        pHashLinear->IsMemberFloatCategorical(size1, (float *)pInput1, pOutput);
    delete pHashLinear;
    return missed;
  } break;
  case 108: {
    CHashLinear<double, int32_t> *pHashLinear =
        new CHashLinear<double, int32_t>(hashMode);
    pHashLinear->MakeHashLocationFloat(size2, (double *)pInput2, hintSize);
    missed = pHashLinear->IsMemberFloatCategorical(size1, (double *)pInput1,
                                                   pOutput);
    delete pHashLinear;
    return missed;
  } break;
  case 116: {
    CHashLinear<long double, int32_t> *pHashLinear =
        new CHashLinear<long double, int32_t>(hashMode);
    pHashLinear->MakeHashLocationFloat(size2, (long double *)pInput2, hintSize);
    missed = pHashLinear->IsMemberFloatCategorical(
        size1, (long double *)pInput1, pOutput);
    delete pHashLinear;
    return missed;
  } break;
  }
...
// Returns in pOutput: index location of second arg -- where first arg found in
// second arg Returns in pBoolOutput: True if found, False otherwise
int64_t IsMemberHashCategorical64(int64_t size1, void *pInput1, int64_t size2,
                                  void *pInput2, int64_t *pOutput,
                                  int32_t sizeType, HASH_MODE hashMode,
                                  int64_t hintSize) {
  ...
  case 116: {
    CHashLinear<long double, int64_t> *pHashLinear =
        new CHashLinear<long double, int64_t>(hashMode);
    pHashLinear->MakeHashLocationFloat(size2, (long double *)pInput2, hintSize);
    missed = pHashLinear->IsMemberFloatCategorical(
        size1, (long double *)pInput1, pOutput);
    delete pHashLinear;
...
typedef int64_t (*ISMEMBER_STRING)(void *pHashLinearVoid, int64_t arraySize,
                                   int64_t strWidth1, int64_t strWidth2,
                                   const char *pHashList,
                                   void *pLocationOutputU, bool isUnicode);
...
//------------------------------------------------------------------------------
//  Concurrent callback from multiple threads
static bool IMSThreadCallback(struct stMATH_WORKER_ITEM *pstWorkerItem,
                              int core, int64_t workIndex) {
  bool didSomeWork = false;
  ...
  while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
    ...
    int64_t inputAdj =
        pstWorkerItem->BlockSize * workBlock * Callback->strWidth1;
    int64_t outputAdj =
        pstWorkerItem->BlockSize * workBlock * Callback->typeSizeOut;

    int64_t missed = Callback->anyIMSCallback(
        Callback->pHashLinearVoid, lenX, Callback->strWidth1,
        Callback->strWidth2, pInput1 + inputAdj, pOutput + outputAdj,
        isUnicode);
...
//  Based on the sizeType it will call different hash functions
template <typename U>
int64_t IsMemberHashStringCategorical(int64_t size1, int64_t strWidth1,
                                      const char *pInput1, int64_t size2,
                                      int64_t strWidth2, const char *pInput2,
                                      U *pOutput, HASH_MODE hashMode,
                                      int64_t hintSize, bool isUnicode) {
  ...
  pHashLinear->MakeHashLocationString(size2, pInput2, strWidth2, hintSize,
                                      isUnicode);
  ...
  if (pWorkItem == NULL) {
    ...
    // Threading not allowed for this work item, call it directly from main
    // thread
    missed = pFunction(pHashLinear, size1, strWidth1, strWidth2, pInput1,
                       pOutput, isUnicode);
...
//  Based on the sizeType it will call different hash functions
template <typename U>
void IsMemberHashString32(int64_t size1, int64_t strWidth1, const char *pInput1,
                          int64_t size2, int64_t strWidth2, const char *pInput2,
                          U *pOutput, int8_t *pBoolOutput, HASH_MODE hashMode,
                          int64_t hintSize, bool isUnicode) {
  ...
  // Second pass find matches
  // We can multithread it
  pHashLinear->IsMemberString(size1, strWidth1, strWidth2, pInput1, pBoolOutput,
                              pOutput, isUnicode);
...
//-----------------------------------------------------------------------------------------
//  Based on the sizeType it will call different hash functions
void IsMemberHashString64(int64_t size1, int64_t strWidth1, const char *pInput1,
                          int64_t size2, int64_t strWidth2, const char *pInput2,
                          int64_t *pOutput, int8_t *pBoolOutput,
                          HASH_MODE hashMode, int64_t hintSize,
                          bool isUnicode) {
...
bool MergePreBinned(int64_t size1, KEY_TYPE *pKey1, void *pInVal1,
                    int64_t size2, KEY_TYPE *pKey2, void *pInVal2,
                    KEY_TYPE *pOutput, int64_t totalUniqueSize,
                    HASH_MODE hashMode, int32_t dtype) {
  ...
  CASE_NPY_INT64:
    ...
    FindLastMatchCategorical<KEY_TYPE, int64_t>(
        size1, size2, pKey1, pKey2, (int64_t *)pInVal1, (int64_t *)pInVal2,
        pOutput, totalUniqueSize);
    break;
  CASE_NPY_INT32:
    FindLastMatchCategorical<KEY_TYPE, int32_t>(
        size1, size2, pKey1, pKey2, (int32_t *)pInVal1, (int32_t *)pInVal2,
        pOutput, totalUniqueSize);
    break;
  case NPY_FLOAT64:
    FindLastMatchCategorical<KEY_TYPE, double>(
        size1, size2, pKey1, pKey2, (double *)pInVal1, (double *)pInVal2,
        pOutput, totalUniqueSize);
    break;
  case NPY_FLOAT32:
    FindLastMatchCategorical<KEY_TYPE, float>(
        size1, size2, pKey1, pKey2, (float *)pInVal1, (float *)pInVal2, pOutput,
        totalUniqueSize);
    break;
  default:
    success = false;
...
bool AlignHashMK32(int64_t size1, void *pInput1, void *pInVal1, int64_t size2,
                   void *pInput2, void *pInVal2, int32_t *pOutput,
                   int64_t totalItemSize, HASH_MODE hashMode, int32_t dtype,
                   bool isForward, bool allowExact) {

  bool success = true;
  CHashLinear<char, int32_t> *pHashLinear =
      new CHashLinear<char, int32_t>(hashMode);

  switch (dtype) {
  CASE_NPY_INT64:

    if (isForward) {
      pHashLinear->FindNextMatchMK<int64_t>(
          size1, size2, (char *)pInput1, (char *)pInput2, (int64_t *)pInVal1,
          (int64_t *)pInVal2, pOutput, totalItemSize, allowExact);
    } else {
      pHashLinear->FindLastMatchMK<int64_t>(
          size1, size2, (char *)pInput1, (char *)pInput2, (int64_t *)pInVal1,
          (int64_t *)pInVal2, pOutput, totalItemSize, allowExact);
    }
    break;
  CASE_NPY_INT32:
    if (isForward) {
      pHashLinear->FindNextMatchMK<int32_t>(
          size1, size2, (char *)pInput1, (char *)pInput2, (int32_t *)pInVal1,
          (int32_t *)pInVal2, pOutput, totalItemSize, allowExact);
    } else {
      pHashLinear->FindLastMatchMK<int32_t>(
          size1, size2, (char *)pInput1, (char *)pInput2, (int32_t *)pInVal1,
          (int32_t *)pInVal2, pOutput, totalItemSize, allowExact);
    }
    break;
  case NPY_FLOAT64:
    if (isForward) {
      pHashLinear->FindNextMatchMK<double>(
          size1, size2, (char *)pInput1, (char *)pInput2, (double *)pInVal1,
          (double *)pInVal2, pOutput, totalItemSize, allowExact);
    } else {
      pHashLinear->FindLastMatchMK<double>(
          size1, size2, (char *)pInput1, (char *)pInput2, (double *)pInVal1,
          (double *)pInVal2, pOutput, totalItemSize, allowExact);
    }
    break;
  case NPY_FLOAT32:
    if (isForward) {
      pHashLinear->FindNextMatchMK<float>(
          size1, size2, (char *)pInput1, (char *)pInput2, (float *)pInVal1,
          (float *)pInVal2, pOutput, totalItemSize, allowExact);
    } else {
      pHashLinear->FindLastMatchMK<float>(
          size1, size2, (char *)pInput1, (char *)pInput2, (float *)pInVal1,
          (float *)pInVal2, pOutput, totalItemSize, allowExact);
    }
    break;
...
bool AlignHashMK64(int64_t size1, void *pInput1, void *pInVal1, int64_t size2,
                   void *pInput2, void *pInVal2, int64_t *pOutput,
                   int64_t totalItemSize, HASH_MODE hashMode, int32_t dtype,
                   bool isForward, bool allowExact) {

  bool success = true;
  CHashLinear<char, int64_t> *pHashLinear =
      new CHashLinear<char, int64_t>(hashMode);
...
uint64_t DoLinearHash(int64_t totalRows, int64_t totalItemSize,
                      const char *pInput1, int coreType,
                      INDEX_TYPE *pIndexArray, void **pFirstArrayVoid,
                      void **pHashTableAny, int64_t *hashTableSize,
                      HASH_MODE hashMode, int64_t hintSize, bool *pBoolFilter) {

  uint64_t numUnique = 0;
  CHashLinear<HASH_TYPE, INDEX_TYPE> *pHashLinear =
      new CHashLinear<HASH_TYPE, INDEX_TYPE>(hashMode, false);
  INDEX_TYPE *pFirstArray = (INDEX_TYPE *)pHashLinear->AllocMemory(
      hintSize, -2, sizeof(INDEX_TYPE) * (totalRows + 1), false);

  // Handles any size
  numUnique = pHashLinear->GroupBy(totalRows, totalItemSize, pInput1, coreType,
                                   pIndexArray, pFirstArray, hashMode, hintSize,
                                   pBoolFilter);
...
template <typename HASH_TYPE, typename INDEX_TYPE>
uint64_t
DoLinearHashFloat(int64_t totalRows, int64_t totalItemSize, const char *pInput1,
                  int coreType, INDEX_TYPE *pIndexArray, void **pFirstArrayVoid,
                  void **pHashTableAny, int64_t *hashTableSize,
                  HASH_MODE hashMode, int64_t hintSize, bool *pBoolFilter) {
  ...
  CHashLinear<HASH_TYPE, INDEX_TYPE> *pHashLinear =
      new CHashLinear<HASH_TYPE, INDEX_TYPE>(hashMode, false);
  INDEX_TYPE *pFirstArray = (INDEX_TYPE *)pHashLinear->AllocMemory(
      hintSize, -1, sizeof(INDEX_TYPE) * (totalRows + 1), false);

  numUnique = pHashLinear->GroupByFloat(
      totalRows, totalItemSize, (HASH_TYPE *)pInput1, coreType, pIndexArray,
...
template <typename HASH_TYPE, typename INDEX_TYPE>
uint64_t DoLinearHashItemSize(
    int64_t totalRows, int64_t totalItemSize, const char *pInput1,
    int coreType, // This is the numpy type code, e.g. NPY_FLOAT32
    INDEX_TYPE *pIndexArray, void **pFirstArrayVoid, void **pHashTableAny,
    int64_t *hashTableSize, HASH_MODE hashMode, int64_t hintSize,
    bool *pBoolFilter) {
  ...
  if (pFirstArray) {
    numUnique = pHashLinear->GroupByItemSize(
        totalRows, totalItemSize, (HASH_TYPE *)pInput1, coreType, pIndexArray,
        pFirstArray, hashMode, hintSize, pBoolFilter);
  }
...
//  Based on the sizeType it will call different hash functions
template <typename INDEX_TYPE>
uint64_t GroupByInternal(void **pFirstArray, void **pHashTableAny,
                         int64_t *hashTableSize,

                         int64_t totalRows, int64_t totalItemSize,
                         const char *pInput1, int coreType,
                         INDEX_TYPE *pIndexArray, HASH_MODE hashMode,
  ...
  case NPY_FLOAT32: {
    ...
    numUnique = DoLinearHashFloat<uint32_t, INDEX_TYPE>(
        totalRows, totalItemSize, pInput1, coreType, pIndexArray, pFirstArray,
        pHashTableAny, hashTableSize, hashMode, hintSize, pBoolFilter);
    calculated = true;
  } break;
  case NPY_FLOAT64: {
    // so that nans compare, we tell it is uint64
    numUnique = DoLinearHashFloat<uint64_t, INDEX_TYPE>(
        totalRows, totalItemSize, pInput1, coreType, pIndexArray, pFirstArray,
        pHashTableAny, hashTableSize, hashMode, hintSize, pBoolFilter);
    calculated = true;
  } break;
  ...
  if (calculated == false) {
    ...
    case 1: {
      numUnique = DoLinearHashItemSize<uint8_t, INDEX_TYPE>(
          totalRows, totalItemSize, pInput1, coreType, pIndexArray, pFirstArray,
          pHashTableAny, hashTableSize, hashMode, 256 / 2, pBoolFilter);
      calculated = true;
    } break;
    case 2: {
      numUnique = DoLinearHashItemSize<uint16_t, INDEX_TYPE>(
          totalRows, totalItemSize, pInput1, coreType, pIndexArray, pFirstArray,
          pHashTableAny, hashTableSize, hashMode, 65536 / 2, pBoolFilter);
      calculated = true;
    } break;
    case 4: {
      numUnique = DoLinearHashItemSize<uint32_t, INDEX_TYPE>(
          totalRows, totalItemSize, pInput1, coreType, pIndexArray, pFirstArray,
          pHashTableAny, hashTableSize, hashMode, hintSize, pBoolFilter);
      calculated = true;
    } break;
    case 8: {
      numUnique = DoLinearHashItemSize<uint64_t, INDEX_TYPE>(
          totalRows, totalItemSize, pInput1, coreType, pIndexArray, pFirstArray,
          pHashTableAny, hashTableSize, hashMode, hintSize, pBoolFilter);
      calculated = true;
    } break;
  ...
  if (calculated == false) {
    numUnique = DoLinearHash<uint32_t, INDEX_TYPE>(
        totalRows, totalItemSize, pInput1, coreType, pIndexArray, pFirstArray,
        pHashTableAny, hashTableSize, hashMode, hintSize, pBoolFilter);
  }
...
//-----------------------------------------------------------------------------------------
//  Based on the sizeType it will call different hash functions
uint64_t GroupBy32Super(int64_t totalRows, int64_t totalItemSize,
                        const char *pInput1, int coreType, int32_t *pIndexArray,
                        int32_t *pNextArray, int32_t *pUniqueArray,
                        int32_t *pUniqueCountArray, HASH_MODE hashMode,
                        int64_t hintSize, bool *pBoolFilter) {
  ...
  CHashLinear<uint32_t, int32_t> *pHashLinear =
      new CHashLinear<uint32_t, int32_t>(hashMode);
  numUnique = pHashLinear->GroupBySuper(
      totalRows, totalItemSize, pInput1, coreType, pIndexArray, pNextArray,
...
//-----------------------------------------------------------------------------------------
//  Based on the sizeType it will call different hash functions
uint64_t GroupBy64Super(int64_t totalRows, int64_t totalItemSize,
                        const char *pInput1, int coreType, int64_t *pIndexArray,
                        int64_t *pNextArray, int64_t *pUniqueArray,
                        int64_t *pUniqueCountArray, HASH_MODE hashMode,
                        int64_t hintSize, bool *pBoolFilter) {

  CHashLinear<uint64_t, int64_t> *pHashLinear =
      new CHashLinear<uint64_t, int64_t>(hashMode);
  uint64_t numUnique = pHashLinear->GroupBySuper(
      totalRows, totalItemSize, pInput1, coreType, pIndexArray, pNextArray,
      pUniqueArray, pUniqueCountArray, hashMode, hintSize, pBoolFilter);
...
uint64_t Unique32(int64_t totalRows, int64_t totalItemSize, const char *pInput1,
                  ...
                  int32_t *pIndexArray, int32_t *pCountArray,
                  HASH_MODE hashMode, int64_t hintSize, bool *pBoolFilter) {

  CHashLinear<uint32_t, int32_t> *pHashLinear =
      new CHashLinear<uint32_t, int32_t>(hashMode);
  uint64_t numUnique =
      pHashLinear->Unique(totalRows, totalItemSize, pInput1, pIndexArray,
                          pCountArray, hashMode, hintSize, pBoolFilter);
...
uint64_t Unique64(int64_t totalRows, int64_t totalItemSize, const char *pInput1,
                  ...
                  int64_t *pIndexArray, int64_t *pCountArray,
                  HASH_MODE hashMode, int64_t hintSize, bool *pBoolFilter) {
...
//-----------------------------------------------------------------------------------------
//  Based on the sizeType it will call different hash functions
void *MultiKeyRollingStep2(int64_t totalRows, int64_t totalItemSize,
                           const char *pInput1,

                           int64_t *pIndexArray, int64_t *pRunningCountArray,
                           HASH_MODE hashMode, int64_t hintSize,
  ...
  pHashLinear->MultiKeyRolling(totalRows, totalItemSize, pInput1, pIndexArray,
                               pRunningCountArray, hashMode, hintSize);
...
//-----------------------------------------------------------------------------------------
//  Based on the sizeType it will call different hash functions
void *MultiKeyHash32(int64_t totalRows, int64_t totalItemSize,
                     const char *pInput1,

                     int32_t *pIndexArray, int32_t *pRunningCountArray,
                     int32_t *pPrevArray, int32_t *pNextArray,
                     int32_t *pFirstArray, HASH_MODE hashMode, int64_t hintSize,
                     bool *pBoolFilter) {

  CHashLinear<uint32_t, int32_t> *pHashLinear =
      new CHashLinear<uint32_t, int32_t>(hashMode);
  pHashLinear->MakeHashLocationMultiKey(
      totalRows, totalItemSize, pInput1, pIndexArray, pRunningCountArray,
      pPrevArray, pNextArray, pFirstArray, hashMode, hintSize, pBoolFilter);
  delete pHashLinear;
  return NULL;
}
...
//-----------------------------------------------------------------------------------------
//  Based on the sizeType it will call different hash functions
void *MultiKeyHash64(int64_t totalRows, int64_t totalItemSize,
                     const char *pInput1,

                     int64_t *pIndexArray, int64_t *pRunningCountArray,
                     int64_t *pPrevArray, int64_t *pNextArray,
                     int64_t *pFirstArray, HASH_MODE hashMode, int64_t hintSize,
                     bool *pBoolFilter) {
...
//
void IsMemberHashString32Pre(PyArrayObject **indexArray, PyArrayObject *inArr1,
                             int64_t size1, int64_t strWidth1,
                             const char *pInput1, int64_t size2,
                             int64_t strWidth2, const char *pInput2,
                             int8_t *pBoolOutput, HASH_MODE hashMode,
                             int64_t hintSize, bool isUnicode) {
  ...
  if (size < 100) {
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT8);
    ...
    IsMemberHashString32<int8_t>(size1, strWidth1, (const char *)pInput1, size2,
                                 strWidth2, (const char *)pInput2, pDataOut2,
                                 pBoolOutput, HASH_MODE(hashMode), hintSize,
                                 isUnicode);

  } else if (size < 30000) {
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT16);
    int16_t *pDataOut2 = (int16_t *)PyArray_BYTES(*indexArray);
    IsMemberHashString32<int16_t>(size1, strWidth1, (const char *)pInput1,
                                  size2, strWidth2, (const char *)pInput2,
                                  pDataOut2, pBoolOutput, HASH_MODE(hashMode),
                                  hintSize, isUnicode);
    ...
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT32);
    ...
    IsMemberHashString32<int32_t>(size1, strWidth1, (const char *)pInput1,
                                  size2, strWidth2, (const char *)pInput2,
                                  pDataOut2, pBoolOutput, HASH_MODE(hashMode),
                                  hintSize, isUnicode);
    ...
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT64);
    ...
    IsMemberHashString32<int64_t>(size1, strWidth1, (const char *)pInput1,
                                  size2, strWidth2, (const char *)pInput2,
                                  pDataOut2, pBoolOutput, HASH_MODE(hashMode),
                                  hintSize, isUnicode);
  }
...
int64_t IsMemberCategoricalHashStringPre(
    PyArrayObject **indexArray, PyArrayObject *inArr1, int64_t size1,
    int64_t strWidth1, const char *pInput1, int64_t size2, int64_t strWidth2,
    const char *pInput2, HASH_MODE hashMode, int64_t hintSize, bool isUnicode) {
  ...
  if (size2 < 100) {
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT8);
    int8_t *pDataOut2 = (int8_t *)PyArray_BYTES(*indexArray);
    missed = IsMemberHashStringCategorical<int8_t>(
        size1, strWidth1, (const char *)pInput1, size2, strWidth2,
        (const char *)pInput2, pDataOut2, HASH_MODE(hashMode), hintSize,
        isUnicode);

  } else if (size2 < 30000) {
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT16);
    int16_t *pDataOut2 = (int16_t *)PyArray_BYTES(*indexArray);
    missed = IsMemberHashStringCategorical<int16_t>(
        size1, strWidth1, (const char *)pInput1, size2, strWidth2,
        (const char *)pInput2, pDataOut2, HASH_MODE(hashMode), hintSize,
        isUnicode);
    ...
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT32);
    int32_t *pDataOut2 = (int32_t *)PyArray_BYTES(*indexArray);
    missed = IsMemberHashStringCategorical<int32_t>(
        size1, strWidth1, (const char *)pInput1, size2, strWidth2,
        (const char *)pInput2, pDataOut2, HASH_MODE(hashMode), hintSize,
        isUnicode);
    ...
    *indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT64);
    int64_t *pDataOut2 = (int64_t *)PyArray_BYTES(*indexArray);
    missed = IsMemberHashStringCategorical<int64_t>(
        size1, strWidth1, (const char *)pInput1, size2, strWidth2,
        (const char *)pInput2, pDataOut2, HASH_MODE(hashMode), hintSize,
        isUnicode);
  }
...
typedef void (*ISMEMBER_MK)(void *pHashLinearVoid, int64_t arraySize,
                            void *pInputT, void *pInput2T, int8_t *pBoolOutput,
                            void *pLocationOutputU, int64_t totalItemSize);
...
//------------------------------------------------------------------------------
//  Concurrent callback from multiple threads
static bool IMMKThreadCallback(struct stMATH_WORKER_ITEM *pstWorkerItem,
                               int core, int64_t workIndex) {
  bool didSomeWork = false;
  ...
  while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
    ...
    int64_t inputAdj =
        pstWorkerItem->BlockSize * workBlock * Callback->totalItemSize;
    int64_t outputAdj =
        pstWorkerItem->BlockSize * workBlock * Callback->typeSizeOut;
    int64_t boolAdj = pstWorkerItem->BlockSize * workBlock;

    Callback->anyIMMKCallback(Callback->pHashLinearVoid, lenX,
                              pInput1 + inputAdj, Callback->pInput2,
                              Callback->pBoolOutput + boolAdj,
                              pOutput + outputAdj, Callback->totalItemSize);
...
void IsMemberHashMK(int64_t size1, void *pInput1,
                    int64_t size2, // size of the second argument
                    void *pInput2, int8_t *pBoolOutput, U *pOutput,
                    int64_t totalItemSize, int64_t hintSize,
                    HASH_MODE hashMode) {
  ...
  pHashLinear->MakeHashLocationMK(size2, (char *)pInput2, totalItemSize,
                                  hintSize);
  ...
  if (pWorkItem == NULL) {
    ...
    // Threading not allowed for this work item, call it directly from main
    // thread
    pFunction(pHashLinear, size1, (char *)pInput1, (char *)pInput2, pBoolOutput,
              pOutput, totalItemSize);
...
void IsMemberHashMKPre(PyArrayObject **indexArray, int64_t size1, void *pInput1,
                       int64_t size2, // size of the second argument
                       void *pInput2, int8_t *pBoolOutput,
                       int64_t totalItemSize, int64_t hintSize,
                       HASH_MODE hashMode) {
  ...
  LOGGING(
      "!!! in multikey ismember %lld  %d   size2:%lld  size1:%lld  size:%lld\n",
  ...
  if (size < 100) {
    *indexArray = AllocateNumpyArray(1, (npy_intp *)&size1, NPY_INT8);
    if (*indexArray) {
      ...
      IsMemberHashMK<int8_t>(size1, pInput1, size2, pInput2, pBoolOutput,
                             pOutput, totalItemSize, hintSize, hashMode);
    }
  } else if (size < 30000) {
    *indexArray = AllocateNumpyArray(1, (npy_intp *)&size1, NPY_INT16);
    if (*indexArray) {
      int16_t *pOutput = (int16_t *)PyArray_BYTES(*indexArray);
      IsMemberHashMK<int16_t>(size1, pInput1, size2, pInput2, pBoolOutput,
                              pOutput, totalItemSize, hintSize, hashMode);
    }
  } else if (size < 2000000000) {
    *indexArray = AllocateNumpyArray(1, (npy_intp *)&size1, NPY_INT32);
    if (*indexArray) {
      ...
      IsMemberHashMK<int32_t>(size1, pInput1, size2, pInput2, pBoolOutput,
                              pOutput, totalItemSize, hintSize, hashMode);
    }
  } else {
    *indexArray = AllocateNumpyArray(1, (npy_intp *)&size1, NPY_INT64);
    if (*indexArray) {
      ...
      IsMemberHashMK<int64_t>(size1, pInput1, size2, pInput2, pBoolOutput,
                              pOutput, totalItemSize, hintSize, hashMode);
    }
  }
...
PyObject *IsMember32(PyObject *self, PyObject *args) {
  ...
  if (tupleSize == 2) {
    if (!PyArg_ParseTuple(args, "O!O!", &PyArray_Type, &inArr1, &PyArray_Type,
                          &inArr2))
      return NULL;
  } else if (tupleSize == 3) {
    if (!PyArg_ParseTuple(args, "O!O!i", &PyArray_Type, &inArr1, &PyArray_Type,
                          &inArr2, &hashMode))
      return NULL;
  } else {
    if (!PyArg_ParseTuple(args, "O!O!iL", &PyArray_Type, &inArr1, &PyArray_Type,
                          &inArr2, &hashMode, &hintSize))
      return NULL;
  }
  ...
  LOGGING("IsMember32 %s vs %s   size: %d  %d\n", NpyToString(arrayType1),
          NpyToString(arrayType2), sizeType1, sizeType2);
  ...
  if (arrayType1 != arrayType2) {
    ...
    PyErr_Format(PyExc_ValueError,
                 "IsMember32 needs first arg to match %s vs %s",
                 NpyToString(arrayType1), NpyToString(arrayType2));
    return NULL;
  }
  ...
  if (sizeType1 == 0) {
    // Weird type
    PyErr_Format(PyExc_ValueError,
                 "IsMember32 needs a type it understands %s vs %s",
                 NpyToString(arrayType1), NpyToString(arrayType2));
    return NULL;
  }
  ...
  if (boolArray) {
    ...
    if (arrayType1 >= NPY_STRING) {
      ...
      IsMemberHashString32Pre(
          &indexArray, inArr1, arraySize1, sizeType1, (const char *)pDataIn1,
          arraySize2, sizeType2, (const char *)pDataIn2, pDataOut1,
          HASH_MODE(hashMode), hintSize, arrayType1 == NPY_UNICODE);
    } else {
      ...
      if (indexArray) {
        ...
        switch (dtype) {
        case NPY_INT8:
          IsMemberHash32<int8_t>(arraySize1, pDataIn1, arraySize2, pDataIn2,
                                 (int8_t *)pDataOut2, pDataOut1, sizeType1,
                                 HASH_MODE(hashMode), hintSize);
          break;
        case NPY_INT16:
          IsMemberHash32<int16_t>(arraySize1, pDataIn1, arraySize2, pDataIn2,
                                  (int16_t *)pDataOut2, pDataOut1, sizeType1,
                                  HASH_MODE(hashMode), hintSize);
          break;
        CASE_NPY_INT32:
          IsMemberHash32<int32_t>(arraySize1, pDataIn1, arraySize2, pDataIn2,
                                  (int32_t *)pDataOut2, pDataOut1, sizeType1,
                                  HASH_MODE(hashMode), hintSize);
          break;
        CASE_NPY_INT64:
          IsMemberHash32<int64_t>(arraySize1, pDataIn1, arraySize2, pDataIn2,
                                  (int64_t *)pDataOut2, pDataOut1, sizeType1,
                                  HASH_MODE(hashMode), hintSize);
          break;
        }
      }
...
#endif
static uint64_t
GroupByImpl(const int64_t partitionLength, // may be 0
            int64_t *const pCutOffs,       // may be NULL
            const int64_t totalRows, const int64_t totalItemSize,
            const char *const pInput1, const int coreType,
            _Index *const pIndexArray, PyArrayObject **pFirstArrayObject,
            const HASH_MODE hashMode, const int64_t hintSize,
  ...
  if (partitionLength) {
    ...
    auto lambdaMKGBCallback = [](void *callbackArgT, int core,
                                 int64_t count) -> bool {
      ...
      LOGGING("[%d] MKGB %lld  cutoff:%lld  offset: %lld length:%lld  "
              "hintsize:%lld\n",
              core, count, pCutOffs[count], partOffset, partLength,
              cb->HintSize);
      ...
      pPartition->NumUnique = (int64_t)GroupByInternal<_Index>(
          ...
          reinterpret_cast<void **>(&pPartition->pFirstArray),
          &pPartition->pHashTableAny, &pPartition->HashTableSize,
    ...
    PARTITION_GB *pPartitions =
        (PARTITION_GB *)WORKSPACE_ALLOC(partitionLength * sizeof(PARTITION_GB));
    ...
    g_cMathWorker->DoMultiThreadedWork(static_cast<int>(partitionLength),
                                       lambdaMKGBCallback, &stMKGBCallback);

    // firstArray = *stMKGBCallback.pFirstArray;
    // NOW COLLECT ALL THE RESULTS

    PyArrayObject *cutoffsArray =
        AllocateNumpyArray(1, (npy_intp *)&partitionLength, NPY_INT64);
    ...
    PyArrayObject *firstArray = AllocateNumpyArray(
        1, (npy_intp *)&totalUniques, numpy_type_code<_Index>::value);
    CHECK_MEMORY_ERROR(firstArray);
    if (!firstArray)
      return 0;
    ...
    // Clean up------------------------------
    for (int i = 0; i < partitionLength; i++) {
      memcpy(&pFirstArray[startpos], pPartitions[i].pFirstArray,
             pPartitions[i].NumUnique * sizeof(_Index));
      startpos += pPartitions[i].NumUnique;
    }

    // Clean up------------------------------
    for (int i = 0; i < partitionLength; i++) {
      WorkSpaceFreeAllocLarge(pPartitions[i].pHashTableAny,
                              pPartitions[i].HashTableSize);
    }
    ...
    *pFirstArrayObject = (PyArrayObject *)pyFirstList;
    ...
    uint64_t numUnique = GroupByInternal<_Index>(
        ...
        totalRows, totalItemSize, pInput1, coreType, pIndexArray, hashMode,
    ...
    *pFirstArrayObject =
        CopyToSmallerArray<_Index>(pFirstArray, numUnique, totalRows);
...
uint64_t GroupBy32(int64_t partitionLength, // may be 0
                   ...
                   int64_t totalRows, int64_t totalItemSize,
                   const char *pInput1, int coreType, void *pIndexArray,
                   PyArrayObject **pFirstArrayObject, HASH_MODE hashMode,
                   int64_t hintSize, bool *pBoolFilter) {
  ...
  return GroupByImpl<index_type>(
      ...
      static_cast<index_type *>(pIndexArray), pFirstArrayObject, hashMode,
      hintSize, pBoolFilter);
}
...
PyObject *MergeBinnedAndSorted(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!O!L", &PyArray_Type, &key1, &PyArray_Type,
                        &key2, &PyArray_Type, &pvalArray1, &PyArray_Type,
                        &pvalArray2, &totalUniqueSize)) {
  ...
  if (dtype1 != dtype2) {
    ...
    if (!((dtype1 <= NPY_ULONGLONG && dtype2 <= NPY_ULONGLONG) &&
          ((dtype1 & 1) == (dtype2 & 1)) &&
          PyArray_ITEMSIZE((PyArrayObject *)pvalArray1) ==
              PyArray_ITEMSIZE((PyArrayObject *)pvalArray2))) {
  ...
  if (indexArray) {
    switch (dtype1) {
    case NPY_INT8:
      success = MergePreBinned<int8_t>(
          ArrayLength(key1), (int8_t *)pKey1, pVal1, ArrayLength(key2),
          (int8_t *)pKey2, pVal2, (int8_t *)PyArray_BYTES(indexArray),
          totalUniqueSize, HASH_MODE_MASK, dtype1);
      break;
    case NPY_INT16:
      success = MergePreBinned<int16_t>(
          ArrayLength(key1), (int16_t *)pKey1, pVal1, ArrayLength(key2),
          (int16_t *)pKey2, pVal2, (int16_t *)PyArray_BYTES(indexArray),
          totalUniqueSize, HASH_MODE_MASK, dtype1);
      break;
    CASE_NPY_INT32:
      success = MergePreBinned<int32_t>(
          ArrayLength(key1), (int32_t *)pKey1, pVal1, ArrayLength(key2),
          (int32_t *)pKey2, pVal2, (int32_t *)PyArray_BYTES(indexArray),
          totalUniqueSize, HASH_MODE_MASK, dtype1);
      break;
    CASE_NPY_INT64:

      success = MergePreBinned<int64_t>(
          ArrayLength(key1), (int64_t *)pKey1, pVal1, ArrayLength(key2),
          (int64_t *)pKey2, pVal2, (int64_t *)PyArray_BYTES(indexArray),
          totalUniqueSize, HASH_MODE_MASK, dtype1);
      break;
    }
  }
  ...
  if (!success) {
    PyErr_Format(
        PyExc_ValueError,
        "MultiKeyAlign failed.  Only accepts int32_t,int64_t,float32,float64");
    return NULL;
  }
