...
enum SORT_MODE { SORT_MODE_QSORT = 1, SORT_MODE_MERGE = 2, SORT_MODE_HEAP = 3 };
...
//-----------------------------------------------------------------------------------------------
template <typename T> int heapsort_(T *start, int64_t n);

template <typename T> int quicksort_(T *start, int64_t num);

template <typename T> void mergesort0_(T *pl, T *pr, T *pw);

template <typename T> int mergesort_(T *start, int64_t num);
    ...
    if (aInfo) {
      ...
      for (int64_t i = 0; i < listSize; i++) {
        if (aInfo[i].ArrayLength != totalRows) {
          PyErr_Format(
              PyExc_ValueError,
              "MultiListPrepare all arrays must be same number of rows %llu",
              totalRows);
          totalRows = 0;
        }
      }
