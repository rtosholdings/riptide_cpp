...
PyObject *IsMember64(PyObject *self, PyObject *args) {
  ...
  if (arrayType1 == NPY_OBJECT) {
    PyErr_Format(PyExc_ValueError, "IsMember32 cannot handle unicode strings, "
  ...
  if (boolArray && indexArray) {
    ...
    if (arrayType1 >= NPY_STRING) {
      ...
      IsMemberHashString64(arraySize1, sizeType1, (const char *)pDataIn1,
                           arraySize2, sizeType2, (const char *)pDataIn2,
                           pDataOut2, pDataOut1, HASH_MODE(hashMode), hintSize,
                           arrayType1 == NPY_UNICODE);
    } else {
      if (arrayType1 == NPY_FLOAT32 || arrayType1 == NPY_FLOAT64) {

        IsMemberHash64(arraySize1, pDataIn1, arraySize2, pDataIn2, pDataOut2,
                       pDataOut1, sizeType1 + 100, HASH_MODE(hashMode),
                       hintSize);
      } else {
        IsMemberHash64(arraySize1, pDataIn1, arraySize2, pDataIn2, pDataOut2,
                       pDataOut1, sizeType1, HASH_MODE(hashMode), hintSize);
      }
...
PyObject *IsMemberCategorical(PyObject *self, PyObject *args) {
  ...
  if (arrayType1 != arrayType2) {
    ...
    // Arguments do not match
    PyErr_Format(PyExc_ValueError,
                 "IsMemberCategorical needs first arg to match %s vs %s",
                 NpyToString(arrayType1), NpyToString(arrayType2));
    return NULL;
  }
  ...
  if (sizeType1 == 0) {
    // Weird type
    PyErr_Format(PyExc_ValueError,
                 "IsMemberCategorical needs a type it understands %s vs %s",
                 NpyToString(arrayType1), NpyToString(arrayType2));
    return NULL;
  }
  ...
  if (arrayType1 >= NPY_STRING) {
    ...
    missed = IsMemberCategoricalHashStringPre(
        &indexArray, inArr1, arraySize1, sizeType1, (const char *)pDataIn1,
        arraySize2, sizeType2, (const char *)pDataIn2, HASH_MODE(hashMode),
        hintSize, arrayType1 == NPY_UNICODE);
  } else if (arrayType1 == NPY_FLOAT32 || arrayType1 == NPY_FLOAT64 ||
             arrayType1 == NPY_LONGDOUBLE) {
    ...
    if (arraySize1 < 2100000000) {
      ...
      missed = IsMemberHashCategorical(arraySize1, pDataIn1, arraySize2,
                                       pDataIn2, pDataOut2, sizeType1 + 100,
                                       HASH_MODE(hashMode), hintSize);
    } else {

      indexArray = AllocateLikeNumpyArray(inArr1, NPY_INT64);
      int64_t *pDataOut2 = (int64_t *)PyArray_BYTES(indexArray);
      missed = IsMemberHashCategorical64(arraySize1, pDataIn1, arraySize2,
                                         pDataIn2, pDataOut2, sizeType1 + 100,
                                         HASH_MODE(hashMode), hintSize);
    }
  } else {
...
//--------------------------------
// bin1   0 3 0 4 3 2 0 2 3  (5 uniques)  [b'b' b'a' b'b' b'd' b'a' b'e' b'b'
// b'e' b'a'] bin2   0 2 3 0 1 1 4      (5 uniques)  [b'a' b'c' b'd' b'a' b'e'
// b'e' b'b'] uniq   4 2 1 0 3 revMap 3 2 1 4 0 -1 -1 reindx 6 1 4 0 2
//
// final result 6 0 6 2 0 4 6 4 0
template <typename T>
void FindFirstOccurence(T *pArray2, int32_t *pUnique1, int32_t *pReIndex,
                        int32_t *pReverseMap, int32_t array2Length,
                        int32_t unique1Length, int32_t unique2Length,
                        int32_t baseOffset2T) {
...
template <typename T>
void FinalMatch(T *pArray1, int32_t *pOutput, int8_t *pBoolOutput,
                int32_t *pReIndex, int32_t array1Length, int32_t baseoffset) {
...
PyObject *IsMemberCategoricalFixup(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!iii", &PyArray_Type, &inArr1,
                        &PyArray_Type, &inArr2, &PyArray_Type, &isMemberUnique,
  ...
  default:
    PyErr_Format(
        PyExc_ValueError,
        "IsMemberCategoricalFixup third argument must be type int32_t not %s",
        NpyToString(uniqueType));
    return NULL;
  }

  LOGGING("IsMemberCategoricalFixup uniqlength:%d   baseoffset1:%d   "
          "baseoffset2:%d\n",
          unique2Length, baseoffset1, baseoffset2);

  // need to reindex this array...
  int32_t *reIndexArray =
      (int32_t *)WORKSPACE_ALLOC(arraySizeUnique * sizeof(int32_t));
  int32_t *reverseMapArray =
      (int32_t *)WORKSPACE_ALLOC(unique2Length * sizeof(int32_t));
  ...
  if (reIndexArray && indexArray && boolArray) {
    ...
    switch (array2Type) {
    case NPY_INT8:
      FindFirstOccurence<int8_t>((int8_t *)pArray2, pUnique, reIndexArray,
                                 reverseMapArray, arraySize2, arraySizeUnique,
                                 unique2Length, baseoffset2);
      break;
    case NPY_INT16:
      FindFirstOccurence<int16_t>((int16_t *)pArray2, pUnique, reIndexArray,
                                  reverseMapArray, arraySize2, arraySizeUnique,
                                  unique2Length, baseoffset2);
      break;
    CASE_NPY_INT32:
      FindFirstOccurence<int32_t>((int32_t *)pArray2, pUnique, reIndexArray,
                                  reverseMapArray, arraySize2, arraySizeUnique,
                                  unique2Length, baseoffset2);
      break;
    CASE_NPY_INT64:

      FindFirstOccurence<int64_t>((int64_t *)pArray2, pUnique, reIndexArray,
                                  reverseMapArray, arraySize2, arraySizeUnique,
                                  unique2Length, baseoffset2);
      break;
    default:
      PyErr_Format(
          PyExc_ValueError,
          "IsMemberCategoricalFixup second argument is not INT8/16/32/64");
      return NULL;
    }
    ...
    switch (array1Type) {
    case NPY_INT8:
      FinalMatch<int8_t>((int8_t *)pArray1, pIndexOut, pBoolOut, reIndexArray,
                         arraySize1, baseoffset1);
      break;
    case NPY_INT16:
      FinalMatch<int16_t>((int16_t *)pArray1, pIndexOut, pBoolOut, reIndexArray,
                          arraySize1, baseoffset1);
      break;
    CASE_NPY_INT32:
      FinalMatch<int32_t>((int32_t *)pArray1, pIndexOut, pBoolOut, reIndexArray,
                          arraySize1, baseoffset1);
      break;
    CASE_NPY_INT64:

      FinalMatch<int64_t>((int64_t *)pArray1, pIndexOut, pBoolOut, reIndexArray,
                          arraySize1, baseoffset1);
      break;
    default:
      PyErr_Format(
          PyExc_ValueError,
          "IsMemberCategoricalFixup first argument is not INT8/16/32/64");
      return NULL;
    }
  ...
  PyErr_Format(PyExc_ValueError,
               "IsMemberCategoricalFixup internal memory error");
  return NULL;
}
