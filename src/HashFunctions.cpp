...
//------------------------------------------------
// PCategoricalFixup
// Used when a categorical is multiday loaded
//
//    Used after multiday load
//    1st arg: p bin array A  (array1) (index in B)
//    2nd arg: partition lengths A (INT64)
//    3rd arg: p unique B unique strings (array2) (to be referenced)
//    4th arg: partition lengths B (INT64)
//    5th arg: unique C strings (in correct order - sorted or no) (not partitioned)
//
// Return A fixed up to C instead of B
// A is still partitioned, just reindexed
// NOTE: to use with non partitioned data, use a partition length of 1 (third arg)
//
PyObject *
PCategoricalFixup(PyObject *self, PyObject *args)
{
   PyArrayObject *inArrA = NULL;
   PyArrayObject *inArrB = NULL;
   PyArrayObject *inPLengthsA = NULL;
   PyArrayObject *inPLengthsB = NULL;
   PyArrayObject *inArrC = NULL;
   ...
   if (!PyArg_ParseTuple(args, "O!O!O!O!O!",
      &PyArray_Type, &inArrA,
      &PyArray_Type, &inPLengthsA,
      &PyArray_Type, &inArrB,
      &PyArray_Type, &inPLengthsB,
      &PyArray_Type, &inArrC)) return NULL;


   switch (PyArray_TYPE(inPLengthsA)) {
   CASE_NPY_INT64:
      break;
   default:
      PyErr_Format(PyExc_ValueError, "PCategoricalFixup 2nd argument must be type INT64");
      return NULL;
   }

   switch (PyArray_TYPE(inPLengthsB)) {
   CASE_NPY_INT64:
      break;
   default:
      PyErr_Format(PyExc_ValueError, "PCategoricalFixup 2nd argument must be type INT64");
   ...
   INT32 arraySizeA = (INT32)ArrayLength(inArrA);
   INT32 arraySizeB = (INT32)ArrayLength(inArrB);
   INT32 arraySizeC = (INT32)ArrayLength(inArrC);

   INT32 arrayAType = PyArray_TYPE(inArrA);
   INT32 arrayBType = PyArray_TYPE(inArrB);
   INT32 arrayCType = PyArray_TYPE(inArrC);

   // Step 1, hash on C
   // Step 2
   //    for each partition,
   //    find where B exists in C
   //       hash B and lookup in C (it must exists, else assign 0)
   //       in side array, replace old index A with the new index
