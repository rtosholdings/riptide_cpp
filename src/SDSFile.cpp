...
static long double gDefaultLongDouble =
...
size_t ZSTD_decompress_stackmode(void *dst, size_t dstCapacity, const void *src,
                                 size_t srcSize) {
  ///* stack mode */
  // ZSTD_DCtx dctx;
...
//------------------------------------------------------------
static size_t DecompressData(ZSTD_DCtx *pDecompContext, int32_t compMode,
...
static size_t DecompressDataPartial(int32_t core, int32_t compMode, void *dst,
...
static bool CompressIsError(int32_t compMode, size_t code) {
  if (ZSTD_isError(code)) {
    SetErr_Format(SDS_VALUE_ERROR, "Decompression error: %s",
...
void SDS_DESTROY_EVENTHANDLE(SDS_EVENT_HANDLE handle) { CloseHandle(handle); }
...
SDS_FILE_HANDLE SDSFileOpen(const char *fileName, bool writeOption,
                            bool overlapped, bool directIO, bool appendOption) {
  ...
  SDS_FILE_HANDLE Handle =
      CreateFile(fileName, GENERIC_READ | (WriteOption ? GENERIC_WRITE : 0),
                 FILE_SHARE_READ | FILE_SHARE_WRITE, 0, filemode,
...
//---------------------------------------------------
// Returns bytes read
int64_t SDSFileReadChunk(SDS_EVENT_HANDLE eventHandle, SDS_FILE_HANDLE Handle,
                         void *buffer, int64_t bufferSize, int64_t BufferPos) {
  ...
  if (bufferSize > MAX_READSIZE_ALLOWED) {
    ...
    while (bufferSize > MAX_READSIZE_ALLOWED) {
      totalRead += SDSFileReadChunk(eventHandle, Handle, cbuffer,
                                    MAX_READSIZE_ALLOWED, BufferPos);
    ...
    if (bufferSize) {
      totalRead +=
          SDSFileReadChunk(eventHandle, Handle, cbuffer, bufferSize, BufferPos);
    }
    ...
    if (!bReadDone && LastError == ERROR_IO_PENDING) {
      ...
      if (!bReadDone) {
        ...
        printf("!!Read failed in getoverlapped %p %p %p %d\n", (void *)Handle,
               (void *)eventHandle, buffer, LastError);
        return 0;
      } else {
...
//---------------------------------------------------
// Returns bytes read
int64_t SDSFileWriteChunk(SDS_EVENT_HANDLE eventHandle, SDS_FILE_HANDLE Handle,
                          void *buffer, int64_t bufferSize, int64_t BufferPos) {
  // LogError("!! Suspicious code path for async read %s %d", FileName,
  // LastError);
  OVERLAPPED OverlappedIO;
  ...
  if (bufferSize > MAX_READSIZE_ALLOWED) {
    ...
    while (bufferSize > MAX_READSIZE_ALLOWED) {
      totalWritten += SDSFileWriteChunk(eventHandle, Handle, cbuffer,
                                        MAX_READSIZE_ALLOWED, BufferPos);
...
const char *GetLastErrorMessage() { return (const char *)strerror(errno); }
...
//---------------------------------------------------
// Returns bytes read
int64_t SDSFileReadChunk(SDS_EVENT_HANDLE eventHandle,
                         SDS_FILE_HANDLE fileHandle, void *buffer,
                         int64_t bufferSize, int64_t bufferPos) {
  ...
  if (bufferSize > MAX_READSIZE_ALLOWED) {
    ...
    while (bufferSize > MAX_READSIZE_ALLOWED) {
      totalRead += SDSFileReadChunk(eventHandle, fileHandle, cbuffer,
                                    MAX_READSIZE_ALLOWED, bufferPos);
    ...
    ssize_t bytes_read =
        pread(fileHandle, buffer, (size_t)bufferSize, bufferPos);
...
//---------------------------------------------------
// Returns bytes read
int64_t SDSFileWriteChunk(SDS_EVENT_HANDLE eventHandle,
                          SDS_FILE_HANDLE fileHandle, void *buffer,
                          int64_t bufferSize, int64_t bufferPos) {
  ...
  if (bufferSize > MAX_READSIZE_ALLOWED) {
    ...
    while (bufferSize > MAX_READSIZE_ALLOWED) {
      totalWritten += SDSFileWriteChunk(eventHandle, fileHandle, cbuffer,
                                        MAX_READSIZE_ALLOWED, bufferPos);
    ...
    ssize_t bytes_written =
        pwrite(fileHandle, buffer, (size_t)bufferSize, bufferPos);
...
typedef std::unordered_map<std::string, PMAPPED_VIEW_STRUCT>
    SHARED_MEMORY_STDMAP;
static SHARED_MEMORY_STDMAP g_SMMap;

//------------------------------------------------------------------------------------------
//
void AddSharedMemory(const char *name, PMAPPED_VIEW_STRUCT pmvs,
                     void *pointer) {
...
typedef SDS_FILE_HANDLE (*SDS_FileOpen)(const char *fileName, bool writeOption,
                                        bool overlapped, bool directIO,
                                        bool appendOption);
typedef int64_t (*SDS_FileReadChunk)(SDS_EVENT_HANDLE eventHandle,
                                     SDS_FILE_HANDLE Handle, void *buffer,
                                     int64_t bufferSize, int64_t BufferPos);
typedef int64_t (*SDS_FileWriteChunk)(SDS_EVENT_HANDLE eventHandle,
                                      SDS_FILE_HANDLE fileHandle, void *buffer,
                                      int64_t bufferSize, int64_t bufferPos);
...
typedef HRESULT (*SDS_SharedMemoryBegin)(const char *pMappingName, int64_t Size,
                                         PMAPPED_VIEW_STRUCT *pReturnStruct);

typedef HRESULT (*SDS_SharedMemoryEnd)(PMAPPED_VIEW_STRUCT pMappedViewStruct);

typedef HRESULT (*SDS_SharedMemoryCopy)(const char *pMappingName,
                                        PMAPPED_VIEW_STRUCT *pReturnStruct,
                                        int bTest);
...
public:
  ...
  SDS_ARRAY_BLOCK *GetArrayBlock(int64_t arrayNum) {
    SDS_ARRAY_BLOCK *pArrayBlock =
        (SDS_ARRAY_BLOCK *)GetMemoryOffset(GetFileHeader()->ArrayBlockOffset);
...
public:
  ...
  void ClearLists() { InclusionList.clear(); }

  bool IsEmpty() { return InclusionList.empty(); }
...
int64_t GetBytesPerBand(SDSArrayInfo *pArrayInfo, int64_t bandSize,
                        int64_t *bandCount = NULL) {
...
static size_t DecompressWithFilter(
  ...
  if (rowOffset == 0 && lastRow <= lastPossibleRow &&
      lastRow == pFilter->BoolMaskTrueCount && lastData <= uncompressedSize) {
    int64_t firstBand = 0;
    ...
    if (compressedSize == uncompressedSize) {
      ...
      result = DefaultFileIO.FileReadChunk(eventHandle, fileHandle, destBuffer,
                                           lastData, arrayDataOffset);
    } else {
      ...
      result = DefaultFileIO.FileReadChunk(eventHandle, fileHandle, tempBuffer,
                                           compressedSize, arrayDataOffset);
      ...
      if (result == compressedSize) {
        ...
        int64_t dcSize =
            DecompressDataPartial(core, compMode, destBuffer, uncompressedSize,
                                  tempBuffer, compressedSize);

        if (dcSize != uncompressedSize) {
          printf("[%d][%lld] MTDecompression band error direct size %lld vs "
                 "%lld\n",
    ...
    LOGGING("Boolmask bpr:%lld  ai:%lld  rowOffset:%lld  fixup:%lld  "
            "masklength:%lld\n",
    ...
    if (pFilter->BoolMaskTrueCount > 0) {
      ...
      if (compressedSize == uncompressedSize) {
        ...
        // Read compressed chunk directly into our tempBuffer
        result =
            DefaultFileIO.FileReadChunk(eventHandle, fileHandle, tempBuffer,
                                        compressedSize, arrayDataOffset);
      }
      ...
      if (result == compressedSize) {
        ...
        if (pTempBuffer) {
          ...
          if (uncompressedRead) {
            dcSize = DefaultFileIO.FileReadChunk(eventHandle, fileHandle,
                                                 pTempBuffer, uncompressedSize,
                                                 arrayDataOffset);
          } else {
            dcSize = DecompressDataPartial(core, compMode, pTempBuffer,
                                           uncompressedSize, tempBuffer,
                                           compressedSize);
          }
          ...
          if (dcSize == uncompressedSize) {
            ...
            LOGGING("sifting through %lld bytes with fixup %lld  bpr:%lld  "
                    "rowOffset: %lld  to dest:%p\n",
...
// The data is banded.
int64_t ReadAndDecompressBandWithFilter(
  ...
  LOGGING("[%lld] seek to %lld  compsz:%lld  uncompsz:%lld  stackIndex:%lld  "
          "bytesPerRow:%lld  <-- ReadAndDecompressBandWithFilter\n",
          rowOffset, pBlockInfo->ArrayDataOffset,
          pBlockInfo->ArrayCompressedSize, pBlockInfo->ArrayUncompressedSize,
  ...
  for (int32_t i = 0; i < pBlockInfo->ArrayBandCount; i++) {
    ...
    if ((i + 1) == pBlockInfo->ArrayBandCount) {
      uncompressedSize = pBlockInfo->ArrayUncompressedSize -
                         (pBlockInfo->ArrayBandSize * bytesPerRow * i);
    ...
    if (trueCount) {
      ...
      if (pFilter->pFilterInfo) {
        sfi.RowOffset = pFilter->pFilterInfo[stackIndex].RowOffset +
                        runningTrueCount; // sum of all previous True Count
      } else {
      ...
      LOGGING(
          "want band start:%lld  masklength:%lld  compsize:%lld  decomp:%lld\n",
          rowOffset, trueCount, compressedSize, uncompressedSize);

      result = DecompressWithFilter(compressedSize, uncompressedSize,
                                    arrayDataOffset, bytesPerRow, eventHandle,
                                    fileHandle, tempBuffer, destBandBuffer,
                                    &tempFilter, rowOffset,
...
static size_t ReadAndDecompressArrayBlockWithFilter(
    SDS_ARRAY_BLOCK *pBlockInfo, // may contain banding information
    SDS_EVENT_HANDLE eventHandle, SDS_FILE_HANDLE fileHandle,
    void *tempBuffer,  // used to decompress
    void *destBuffer,  // the array buffer (final destination of data)
  ...
  if (tempBuffer) {
    ...
    if (pBlockInfo->ArrayBandCount > 0) {
      ...
      result = DefaultFileIO.FileReadChunk(eventHandle, fileHandle, pBands,
                                           bandDataSize,
                                           pBlockInfo->ArrayDataOffset);

      if (result == bandDataSize) {
        // read from banded decompressed into array
        result = ReadAndDecompressBandWithFilter(
            pBlockInfo, eventHandle, fileHandle, tempBuffer, destBuffer,
            pFilter,
      ...
      result = DecompressWithFilter(
          pBlockInfo->ArrayCompressedSize, pBlockInfo->ArrayUncompressedSize,
          pBlockInfo->ArrayDataOffset, GetBytesPerRow(pBlockInfo), eventHandle,
          fileHandle, tempBuffer, destBuffer, pFilter, rowOffset, stackIndex,
          core, compMode);
    }
  } else {
    printf("out of mem no temp buffer  bandcount:%d  bandsize:%d  uncomp size: "
           "%lld\n",
           pBlockInfo->ArrayBandCount, pBlockInfo->ArrayBandSize,
           pBlockInfo->ArrayUncompressedSize);
  }
...
// If a mask exists, data
// return size of bytes read or -1 on failure
static size_t ReadAndDecompressArrayBlock(
    SDS_ARRAY_BLOCK *pBlockInfo, // may contain banding information
    SDS_EVENT_HANDLE eventHandle, SDS_FILE_HANDLE fileHandle,
    void *tempBuffer,   // used to decompress
    void *destBuffer,   // the array buffer (final destination of data)
    ...
    if (pBlockInfo->ArrayBandCount > 0) {
      ...
      if (result == bandDataSize) {
        ...
        for (int32_t i = 0; i < pBlockInfo->ArrayBandCount; i++) {
          ...
          if ((i + 1) == pBlockInfo->ArrayBandCount) {
            uncompressedSize = pBlockInfo->ArrayUncompressedSize -
                               (destBandBuffer - (char *)destBuffer);
          }

          if (compressedSize == uncompressedSize) {
            result = DefaultFileIO.FileReadChunk(
                eventHandle, fileHandle, destBandBuffer, uncompressedSize,
                arrayDataOffset);
            if (result != uncompressedSize) {
              printf("[%d][%lld][%d] MTDecompression (uncompressed) band error "
                     "size %lld vs %lld\n",
            ...
            int64_t dcSize =
                DecompressData(NULL, compMode, destBandBuffer, uncompressedSize,
                               tempBuffer, compressedSize);
            if (dcSize != uncompressedSize) {
              printf("[%d][%lld][%d] MTDecompression band error size %lld vs "
                     "%lld vs %lld\n",
      ...
      if (result != compressedSize) {
        ...
        int64_t dcSize =
            DecompressData(NULL, compMode, destBuffer, uncompressedSize,
                           tempBuffer, compressedSize);
...
void FillFileHeader(SDS_FILE_HEADER *pFileHeader, int64_t fileOffset,
                    int16_t compMode, int16_t compType, int32_t compLevel,
                    int16_t fileType, int16_t stackType, int32_t authorId,
                    int64_t listNameLength, int64_t listNameCount,
                    int64_t metaBlockSize, int64_t arrayCount,
                    int64_t bandSize) {
  ...
  pFileHeader->MetaBlockOffset =
      pFileHeader->NameBlockOffset + SDS_PAD_NUMBER(listNameLength);
  ...
  pFileHeader->ArrayBlockOffset =
      pFileHeader->MetaBlockOffset + SDS_PAD_NUMBER(pFileHeader->MetaBlockSize);

  // As we write or read arrays, this value should be incremented
  pFileHeader->ArraysWritten = 0;
  pFileHeader->ArrayFirstOffset = pFileHeader->ArrayBlockOffset +
                                  SDS_PAD_NUMBER(pFileHeader->ArrayBlockSize);

  LOGGING("main offsets %lld  %lld  %lld\n", pFileHeader->MetaBlockOffset,
          pFileHeader->ArrayBlockOffset, pFileHeader->ArrayFirstOffset);
  ...
  pFileHeader->SectionBlockOffset =
      0; // points to section directory if it exists
...
int64_t ReadFileHeader(SDS_FILE_HANDLE fileHandle, SDS_FILE_HEADER *pFileHeader,
                       int64_t fileOffset, const char *fileName) {
  size_t bytesRead = DefaultFileIO.FileReadChunk(
      NULL, fileHandle, pFileHeader, sizeof(SDS_FILE_HEADER), fileOffset);

  if (bytesRead != sizeof(SDS_FILE_HEADER)) {
    SetErr_Format(
        SDS_VALUE_ERROR,
        "Decompression error cannot read header for file: %s.  Error: %s",
        fileName, GetLastErrorMessage());
    DefaultFileIO.FileClose(fileHandle);
    return -1;
  }

  if (pFileHeader->SDSHeaderMagic != SDS_MAGIC ||
      pFileHeader->VersionHigh != SDS_VERSION_HIGH) {
    if (pFileHeader->SDSHeaderMagic == SDS_MAGIC &&
        pFileHeader->VersionHigh != SDS_VERSION_HIGH) {
      ...
      SetErr_Format(SDS_VALUE_ERROR,
                    ...
                    fileName, (int)(pFileHeader->VersionHigh),
      ...
      SetErr_Format(
          ...
          fileName, (int)(pFileHeader->SDSHeaderMagic),
          pFileHeader->ArraysWritten, fileOffset);
    }
    DefaultFileIO.FileClose(fileHandle);
    return -1;
  }
...
int64_t SDSSectionName::BuildSectionNamesAndOffsets(
  ...
  int64_t allocSize =
      SDS_PAD_NUMBER((((SDS_MAX_SECTIONNAME + 8) * (SectionCount + 1)) + 1024));
...
//
void SDSSectionName::AllocateSectionData(int64_t sectionBlockCount,
                                         int64_t sectionSize) {
...
void SDSSectionName::MakeListSections(const int64_t sectionBlockCount,
                                      const int64_t sectionByteSize) {
...
char *SDSSectionName::ReadListSections(SDS_FILE_HANDLE SDSFile,
                                       SDS_FILE_HEADER *pFileHeader) {
  ...
  if (sectionSize) {
    ...
    if (!pSectionData) {
      SetErr_Format(SDS_VALUE_ERROR, "Decompression error in sectionSize: %lld",
                    sectionSize);
      return NULL;
    }

    int64_t bytesRead =
        DefaultFileIO.FileReadChunk(NULL, SDSFile, pSectionData, sectionSize,
                                    pFileHeader->SectionBlockOffset);

    if (bytesRead != sectionSize) {
      SetErr_Format(SDS_VALUE_ERROR, "Decompression error in bytesRead: %lld",
                    sectionSize);
      return NULL;
    }
...
SDSSectionName::~SDSSectionName() { DeleteSectionData(); };
...
SDS_FILE_HANDLE StartCompressedFile(
    const char *fileName, SDS_FILE_HEADER *pFileHeader,

    int16_t compType, int32_t compLevel, int16_t fileType, int16_t stackType,
    int32_t authorId,
    ...
    SDS_STRING_LIST *pFolderName, int64_t bandSize,
  ...
  SDS_FILE_HANDLE fileHandle =
      DefaultFileIO.FileOpen(fileName, true, true, false,
                             mode == COMPRESSION_MODE_COMPRESS_APPEND_FILE);

  if (!fileHandle) {
    SetErr_Format(SDS_VALUE_ERROR,
                  "Compression error cannot create/open file: %s.  Error: %s",
                  fileName, GetLastErrorMessage());
    return BAD_SDS_HANDLE;
  }
  ...
  if (compType == COMPRESSION_TYPE_ZSTD) {
    ...
    cSize = CompressData(compType, dest, dest_size, strMeta, strMetaLength,
                         compLevel);
  ...
  FillFileHeader(pFileHeader, fileOffset, COMPRESSION_MODE_COMPRESS_FILE,
                 compType, compLevel, fileType, stackType, authorId,
  ...
  DefaultFileIO.FileWriteChunk(NULL, fileHandle, pFileHeader,
                               sizeof(SDS_FILE_HEADER), fileOffset);

  LOGGING("meta compressed to %llu vs %lld  %llu\n", cSize, strMetaLength,
          sizeof(SDS_FILE_HEADER));

  if (listNameLength) {
    DefaultFileIO.FileWriteChunk(NULL, fileHandle, (void *)listNames,
                                 listNameLength, pFileHeader->NameBlockOffset);
  }

  if (cSize) {
    DefaultFileIO.FileWriteChunk(NULL, fileHandle, dest, cSize,
                                 pFileHeader->MetaBlockOffset);
  }
...
// set pointer to NULL
//-------------------------------------------------------
void EndCompressedFile(SDS_FILE_HANDLE sdsFile, SDS_FILE_HEADER *pFileHeader,
  ...
  if (pWriteInfo->sectionName) {
    ...
    if (pFileHeader->FileOffset == 0) {
      ...
      int64_t sectionSize = cSDSSectionName.BuildSectionNamesAndOffsets(
          &pListNames, pWriteInfo->sectionName, pFileHeader->FileOffset);
      ...
      int64_t result = DefaultFileIO.FileWriteChunk(
          NULL, sdsFile, pListNames, pFileHeader->SectionBlockReservedSize,
          pFileHeader->SectionBlockOffset);

      if (result != pFileHeader->SectionBlockReservedSize) {
        SetErr_Format(SDS_VALUE_ERROR,
                      "Compression error cannot append section %lld at %lld",
                      pFileHeader->SectionBlockReservedSize,
                      pFileHeader->SectionBlockOffset);
      }
    } else {
      ...
      if (result == 0) {
        ...
        char *pSectionNames =
            cSDSSectionName.ReadListSections(sdsFile, &fileHeader);
        ...
        LOGGING("SDS: Writing section %s with blockcount:%lld at %lld (%lld) "
                "at %lld\n",
                pWriteInfo->sectionName, blockCount, pFileHeader->FileOffset,
                fileHeader.FileOffset, LastFileOffset);
        int64_t sectionSize = cSDSSectionName.BuildSectionNamesAndOffsets(
            &pListNames, pWriteInfo->sectionName, pFileHeader->FileOffset);
        ...
        LOGGING("SDS: new section size %lld vs %lld\n", sectionSize,
                fileHeader.SectionBlockReservedSize);
      ...
      result = DefaultFileIO.FileWriteChunk(NULL, sdsFile, pListNames,
                                            fileHeader.SectionBlockReservedSize,
      ...
      result = DefaultFileIO.FileWriteChunk(NULL, sdsFile, &fileHeader,
                                            sizeof(SDS_FILE_HEADER), 0);
      if (result != sizeof(SDS_FILE_HEADER)) {
        SetErr_Format(SDS_VALUE_ERROR,
                      "Compression error cannot rewrite fileheader\n");
      }
    }
  ...
  int64_t result = DefaultFileIO.FileWriteChunk(NULL, sdsFile, pFileHeader,
                                                sizeof(SDS_FILE_HEADER),
                                                pFileHeader->FileOffset);
  if (result != sizeof(SDS_FILE_HEADER)) {
    SetErr_Format(SDS_VALUE_ERROR,
                  "Compression error cannot write fileheader at offset %lld\n",
                  pFileHeader->FileOffset);
  }
  LOGGING("Total arrays written %lld  --- %llu %lld\n",
          pFileHeader->ArraysWritten, sizeof(SDS_FILE_HEADER), result);

  result = DefaultFileIO.FileWriteChunk(NULL, sdsFile, pArrayBlocks,
...
bool DecompressFileArray(void *pstCompressArraysV, int32_t core, int64_t t) {
  ...
  SDS_READ_DECOMPRESS_ARRAYS *pstCompressArrays =
      (SDS_READ_DECOMPRESS_ARRAYS *)pstCompressArraysV;
  ...
  if (pstCompressArrays->compMode == COMPRESSION_MODE_SHAREDMEMORY) {
    SDS_ARRAY_BLOCK *pArrayBlock =
        pstCompressArrays->pMemoryIO->GetArrayBlock(t);
    destBuffer = pstCompressArrays->pMemoryIO->GetMemoryOffset(
        pArrayBlock->ArrayDataOffset);
  ...
  if (destBuffer) {
    ...
    if (source_size != pBlockInfo->ArrayUncompressedSize &&
    ...
    if (pBlockInfo->Flags & SDS_ARRAY_FILTERED) {
      ...
      int64_t result = ReadAndDecompressArrayBlockWithFilter(
          pBlockInfo, pstCompressArrays->eventHandles[core], sdsFile,
          tempBuffer, destBuffer, 0, pstCompressArrays->pFilter, t, core,
          COMPRESSION_TYPE_ZSTD);

    } else {
      // No stacking, no filtering
      int64_t result = ReadAndDecompressArrayBlock(
          pBlockInfo, pstCompressArrays->eventHandles[core], sdsFile,
          tempBuffer, destBuffer, t, core, COMPRESSION_TYPE_ZSTD);
    }
  }
...
bool CompressFileArray(void *pstCompressArraysV, int32_t core, int64_t t) {
  ...
  SDS_WRITE_COMPRESS_ARRAYS *pstCompressArrays =
      (SDS_WRITE_COMPRESS_ARRAYS *)pstCompressArraysV;
  ...
  if (pstCompressArrays->compType == COMPRESSION_TYPE_ZSTD) {
    ...
    wantedSize = dest_size =
        CompressGetBound(COMPRESSION_TYPE_ZSTD, source_size);
  ...
  if (pTempMemory) {
    ...
    if (pstCompressArrays->compType == COMPRESSION_TYPE_ZSTD) {
      ...
      if (bandCount > 0) {
        ...
        for (int64_t i = 0; i < bandCount;) {
          ...
          cSize = CompressData(COMPRESSION_TYPE_ZSTD, pWriteMemory,
                               bytesAvailable, pReadMemory, bytesPerBand,
        ...
        cSize = CompressData(COMPRESSION_TYPE_ZSTD, pTempMemory, dest_size,
                             pArrayInfo->pData, source_size,
                             pstCompressArrays->compLevel);
        // Failed to compress
    ...
    LOGGING("[%lld][%lld] seek to fileOffset %lld  sz: %lld\n", t, arrayNumber,
            fileOffset, pArrayBlock->ArrayCompressedSize);

    //===========================
    // Write compressed chunk
    int64_t result = DefaultFileIO.FileWriteChunk(
        pstCompressArrays->eventHandles[core], pstCompressArrays->fileHandle,
        pTempMemory, cSize, fileOffset);

    if ((size_t)result != cSize) {
      printf("[%lld] error while writing into compressed  offset:%lld  sz: %zu "
             " vs %lld\n",
...
//
int64_t CalculateSharedMemorySize(SDS_FILE_HEADER *pFileHeader,
                                  SDS_ARRAY_BLOCK *pArrayBlocks) {
...
bool SDSWriteFileInternal(const char *fileName,
  ...
  int64_t listNameSize =
      pWriteInfo->listNameSize; // total byte size (store in memory)
  ...
  if (aInfo || arrayCount == 0) {
    ...
    if (shareName) {
      ...
      FillFileHeader(pFileHeader, 0, COMPRESSION_MODE_SHAREDMEMORY,
                     COMPRESSION_TYPE_NONE, 0, sdsFileType, 0, authorId,
                     listNameSize, listNameCount, metaDataSize, arrayCount,
                     bandSize);
      ...
      if (hr >= 0) {
        ...
        try {
          ...
          // Step 2: copy names
          memcpy(DefaultMemoryIO.GetMemoryOffset(
                     pMemoryFileHeader->NameBlockOffset),
                 pListNames, listNameSize);

          LOGGING("Copying metadata  size:%lld \n", (long long)metaDataSize);

          // Step 3: copy metadata
          memcpy(DefaultMemoryIO.GetMemoryOffset(
                     pMemoryFileHeader->MetaBlockOffset),
          ...
          for (int32_t arrayNumber = 0; arrayNumber < arrayCount;
               arrayNumber++) {
            ...
            pDestArrayBlock[arrayNumber].ArrayCompressedSize =
                pDestArrayBlock[arrayNumber].ArrayUncompressedSize =
                    aInfo[arrayNumber].ArrayLength *
                    aInfo[arrayNumber].ItemSize;

            pDestArrayBlock[arrayNumber].ArrayDataOffset = startOffset;
            pDestArrayBlock[arrayNumber].CompressionType =
          ...
          // space
          printf("!!!Failed to write all of shared memory.  May be out of "
                 "shared memory.\n");
        }
#if defined(_WIN32)
        // For windows we cannot close it since that will free the memory back
          ...
          printf(
              "BUS Error while writing to shared memory, check swap space.\n");
        }
      ...
      SDS_FILE_HANDLE sdsFile = StartCompressedFile(
          fileName, pFileHeader, (int16_t)compType, level, sdsFileType,
          0, // stackType
      ...
      if (sdsFile) {
        ...
        SDS_ARRAY_BLOCK *pArrayBlocks =
            (SDS_ARRAY_BLOCK *)WORKSPACE_ALLOC(pFileHeader->ArrayBlockSize);
        ...
        SDS_WRITE_COMPRESS_ARRAYS *pstCompressArrays =
            (SDS_WRITE_COMPRESS_ARRAYS *)WORKSPACE_ALLOC(
                sizeof(SDS_WRITE_COMPRESS_ARRAYS));
        ...
        if (arrayCount > 0) {
          ...
          for (int32_t j = 0; j < numCores; j++) {
            ...
            pstCompressArrays->eventHandles[j] =
                DefaultFileIO.CreateEventHandle();
          }
          ...
          g_cMathWorker->DoMultiThreadedWork((int)arrayCount, CompressFileArray,
                                             pstCompressArrays);
          ...
          for (int32_t j = 0; j < numCores; j++) {
            ...
            DefaultFileIO.DestroyEventHandle(
                pstCompressArrays->eventHandles[j]);
          }
        }
...
struct SDS_MULTI_IO_PACKETS {
  ...
  static SDS_MULTI_IO_PACKETS *Allocate(int64_t tupleSize,
                                        SDS_FILTER *pFilter) {
    int64_t allocSize =
        sizeof(SDS_MULTI_IO_PACKETS) + ((sizeof(SDS_IO_PACKET) * tupleSize));
    SDS_MULTI_IO_PACKETS *pMultiIOPackets =
        (SDS_MULTI_IO_PACKETS *)WORKSPACE_ALLOC(allocSize);
    memset(pMultiIOPackets, 0, allocSize);

    pMultiIOPackets->pFilter = pFilter;

    int64_t allocSizeDestInfo = tupleSize * sizeof(SDSArrayInfo);
    pMultiIOPackets->pDestInfo =
        (SDSArrayInfo *)WORKSPACE_ALLOC(allocSizeDestInfo);
...
bool IsNameIncluded(SDSIncludeExclude *pInclude, SDSIncludeExclude *pFolderName,
                    const char *nameToCheck, bool isOneFile) {
...
bool PossiblyShrinkArray(SDS_ALLOCATE_ARRAY *pArrayCallback,
                         SDS_READ_CALLBACKS *pReadCallbacks, bool isStackable) {
  bool wasFiltered = false;

  // Categoricals will not be in original container
  int32_t mask = SDS_FLAGS_ORIGINAL_CONTAINER;
  if ((pArrayCallback->sdsFlags &
       (SDS_FLAGS_SCALAR | SDS_FLAGS_META | SDS_FLAGS_NESTED)) == 0) {
    if (pArrayCallback->sdsFlags & mask) {
      // Did they pass a fancy index or a bool index?
      if (pReadCallbacks->Filter.pBoolMask &&
          pReadCallbacks->Filter.BoolMaskTrueCount >= 0 && isStackable) {
        wasFiltered = true;
        ...
        LOGGING("-->Allocation reduction for %s filter: %lld  vs %lld  "
                "bool:%lld  flags:%d\n",
                pArrayCallback->pArrayName, newLength, dim0Length,
...
public:
  ...
  SDSDecompressFile(const char *fileName, SDSIncludeExclude *pInclude = NULL,
                    int64_t instanceIndex = 0, const char *shareName = NULL,
  ...
  void AllocateOneArray(
      ...
      SDS_READ_CALLBACKS *pReadCallbacks, SDSArrayInfo *pDestInfo,
      bool isSharedMemory, bool isOneFile, bool isStackable) {
    ...
    if (isSharedMemory) {
      pDestInfo->pData = sdsArrayCallback.data =
          DefaultMemoryIO.GetMemoryOffset(pBlock->ArrayDataOffset);
    ...
    if (!pArrayNames ||
        IsNameIncluded(pInclude, pFolderName, sdsArrayCallback.pArrayName,
                       isOneFile)) {
      ...
      if (pReadCallbacks->AllocateArrayCallback) {
        ...
        LOGGING("Allocating dims:%d -- %lld %lld %lld   flags:%d  itemsize:%d  "
                "wasFiltered: %d\n",
                (int)pBlock->NDim, pBlock->Dimensions[0], pBlock->Dimensions[1],
                pBlock->Dimensions[2], pBlock->Flags, pBlock->ItemSize,
                wasFiltered);
        LOGGING("Strides  %lld %lld %lld  \n", pBlock->Strides[0],
                pBlock->Strides[1], pBlock->Strides[2]);
      }
    }
  ...
  //
  void AllocMultiArrays(SDS_IO_PACKET *pIOPacket, SDSArrayInfo *pDestInfo,
                        SDS_READ_CALLBACKS *pReadCallbacks, int64_t tupleSize,
                        bool isSharedMemory) {
  ...
  SDS_READ_DECOMPRESS_ARRAYS *
  AllocDecompressArrays(SDS_READ_CALLBACKS *pReadCallbacks, int64_t tupleSize,
                        bool isSharedMemory, bool isOneFile, bool isStackable) {
    ...
    SDS_READ_DECOMPRESS_ARRAYS *pstDecompressArrays =
        (SDS_READ_DECOMPRESS_ARRAYS *)WORKSPACE_ALLOC(
            sizeof(SDS_READ_DECOMPRESS_ARRAYS) +
            (tupleSize * sizeof(SDSArrayInfo)));
    ...
    for (int32_t t = 0; t < tupleSize; t++) {
      ...
      AllocateOneArray(t, pReadCallbacks, pDestInfo, isSharedMemory, isOneFile,
                       isStackable);
    }
  ...
  SDS_ARRAY_BLOCK *AllocateArrayBlocks() {
    ...
    if (pArrayBlocks == NULL) {
      SetErr_Format(SDS_VALUE_ERROR,
                    "Allocation of size %lld for arrayblocks failed.\n",
                    pFileHeader->ArrayBlockSize);
    }
    return pArrayBlocks;
  }
  ...
  int64_t GetTotalArraysWritten() {
    ...
    LOGGING("Section offsets %p   psectiondata %p   sectioncount: %lld\n",
            cSectionName.pSectionOffsets, cSectionName.pSectionData,
            cSectionName.SectionCount);
  ...
  ~SDSDecompressFile() { EndDecompressedFile(); }
  ...
  StartDecompressedFile(const char *fileName, int64_t fileOffset) {
    ...
    if (!fileHandle) {
      SetErr_Format(
          SDS_VALUE_ERROR,
          "Decompression error cannot create/open file: %s.  Error: %s",
          fileName, GetLastErrorMessage());
      return BAD_SDS_HANDLE;
    }
  ...
  //
  //
  bool DecompressMetaData(SDS_FILE_HEADER *pFileHeader,
                          char *metaDataUncompressed, int32_t core) {
    ...
    LOGGING("in decompress meta %lld vs %lld  offset:%lld  handle:%p\n",
            metaCompressedSize, pFileHeader->TotalMetaUncompressedSize,
            pFileHeader->MetaBlockOffset, SDSFile);

    // Did we compress the meta data?
    // If the decompressed size is the same as uncompressed, then it was never
    // compressed
    if (metaCompressedSize &&
        metaCompressedSize != pFileHeader->TotalMetaUncompressedSize) {
      ...
      if (!metaDataCompressed) {
        SetErr_Format(SDS_VALUE_ERROR,
                      "Decompression error in metaDataCompressedsize: %lld",
                      metaCompressedSize);
        return false;
      }

      // Read in compressed data into our buffer: metaDataCompressed
      bytesRead = DefaultFileIO.FileReadChunk(NULL, SDSFile, metaDataCompressed,
                                              metaCompressedSize,
                                              pFileHeader->MetaBlockOffset);
      ...
      if (!metaDataUncompressed) {
        ...
        if (!metaDataUncompressed) {
          SetErr_Format(
              SDS_VALUE_ERROR,
              "Decompression error meta: could not allocate meta string %lld",
      ...
      int64_t cSize = DecompressData(
          NULL, COMPRESSION_TYPE_ZSTD, metaDataUncompressed,
          pFileHeader->TotalMetaUncompressedSize, metaDataCompressed,
          pFileHeader->TotalMetaCompressedSize);

      WORKSPACE_FREE(metaDataCompressed);
      metaDataCompressed = NULL;

      if (CompressIsError(COMPRESSION_TYPE_ZSTD, cSize) ||
          cSize != metaUncompressedSize) {
        SetErr_Format(SDS_VALUE_ERROR,
      ...
      bytesRead = DefaultFileIO.FileReadChunk(
          NULL, SDSFile, metaDataUncompressed, metaUncompressedSize,
          pFileHeader->MetaBlockOffset);
    }
  ...
  bool CopyIntoSharedMemoryInternal(SDS_READ_CALLBACKS *pReadCallbacks,
                                    int32_t core) {
    ...
    if (SDSFile) {
      ...
      if (!AllocateArrayBlocks()) {
        SetErr_Format(SDS_VALUE_ERROR,
                      "Decompression error in pArrayBlock: %lld",
                      pFileHeader->ArrayBlockSize);
        return false;
      }

      int64_t bytesRead = DefaultFileIO.FileReadChunk(
          NULL, SDSFile, pArrayBlocks, pFileHeader->ArrayBlockSize,
          pFileHeader->ArrayBlockOffset);
      if (bytesRead != pFileHeader->ArrayBlockSize) {
        SetErr_Format(SDS_VALUE_ERROR,
                      "Decompression error in ArrayBlockSize: %lld",
                      pFileHeader->ArrayBlockSize);
        return false;
      }
      ...
      if (hr >= 0) {
        ...
        LOGGING("Fill file header   namecount:%lld   arrayswritten:%lld  "
                "address %p\n",
                pFileHeader->NameBlockCount, pFileHeader->ArraysWritten,
                pMemoryFileHeader);

        // The memory header
        // Shared memory is stored uncompressed
        FillFileHeader(pMemoryFileHeader, 0, COMPRESSION_MODE_SHAREDMEMORY,
                       COMPRESSION_TYPE_NONE,
                       0, // comp level
                       pFileHeader->FileType,
                       0, // stack Type
                       pFileHeader->AuthorId, pFileHeader->NameBlockSize,
        ...
        bool bResult = DecompressMetaData(
            pFileHeader,
        ...
        if (bResult) {
          LOGGING("Meta string is %s\n",
                  DefaultMemoryIO.GetMemoryOffset(
                      pMemoryFileHeader->MetaBlockOffset));

          SDS_ARRAY_BLOCK *pDestArrayBlock = DefaultMemoryIO.GetArrayBlock(0);

          // Read array block
          DefaultFileIO.FileReadChunk(NULL, SDSFile, pDestArrayBlock,
                                      pFileHeader->ArrayBlockSize,
                                      pFileHeader->ArrayBlockOffset);
          ...
          for (int32_t i = 0; i < arrayCount; i++) {
            ...
            // same size since uncompressed
            pDestArrayBlock[i].ArrayCompressedSize =
                pDestArrayBlock[i].ArrayUncompressedSize;
            ...
            // Keep tallying
            startOffset +=
                SDS_PAD_NUMBER(pDestArrayBlock[i].ArrayUncompressedSize);
          }
          ...
          // MORE WORK TO DO
          SDS_READ_DECOMPRESS_ARRAYS *pstCompressArrays = AllocDecompressArrays(
              pReadCallbacks, arrayCount, true, oneFile, fileTypeStackable);
          ...
          g_cMathWorker->DoMultiThreadedWork(
              (int)arrayCount, DecompressFileArray, pstCompressArrays);
      ...
      SetErr_Format(SDS_VALUE_ERROR,
                    "SDS file was null when copying into shared memory\n");
    }
    return false;
  }
  ...
  bool CopyIntoSharedMemory(SDS_READ_CALLBACKS *pReadCallbacks,
                            const char *fileName,
                            SDSIncludeExclude *pFolderName, int32_t core) {
  ...
  char *ReadListNames() {
    ...
    if (nameSize) {
      ...
      if (!pNameData) {
        SetErr_Format(SDS_VALUE_ERROR, "Decompression error in nameSize: %lld",
                      nameSize);
        return NULL;
      }

      int64_t bytesRead = DefaultFileIO.FileReadChunk(
          NULL, SDSFile, pNameData, nameSize, pFileHeader->NameBlockOffset);
  ...
  bool DecompressFileInternal(SDS_READ_CALLBACKS *pReadCallbacks, int32_t core,
                              int64_t startOffset) {
    ...
    if (SDSFile) {
      ...
      // sections
      if (startOffset == 0) {
        if (pFileHeader->SectionBlockCount &&
            cSectionName.ReadListSections(SDSFile, pFileHeader) == NULL) {
          goto EXIT_DECOMPRESS;
        }
      }
      ...
      pstCompressArrays = AllocDecompressArrays(
          pReadCallbacks, tupleSize, false,
      ...
      void *saveState = pReadCallbacks->BeginAllowThreads();
      g_cMathWorker->DoMultiThreadedWork((int)tupleSize, DecompressFileArray,
                                         pstCompressArrays);
      pReadCallbacks->EndAllowThreads(saveState);
  ...
  EXIT_DECOMPRESS:
    ...
    if (g_errorbuffer[0] == 0) {
      SetErr_Format(SDS_VALUE_ERROR,
                    "ExitDecompress called for error when opening file %s\n",
                    FileName);
    }
    return false;
  }
  ...
  //
  void *DecompressFile(SDS_READ_CALLBACKS *pReadCallbacks, int32_t core,
                       int64_t fileOffset) {
    ...
    if (ShareName) {
      ...
      if (result < 0) {
        ...
        if (Mode == COMPRESSION_MODE_INFO) {
          ...
          bResult =
              CopyIntoSharedMemory(pReadCallbacks, FileName, pFolderName, core);
...
void UpgradeType(SDS_ARRAY_BLOCK *pMasterArrayBlock, int32_t newdtype,
                 int64_t itemsize) {
  ...
  if (pMasterArrayBlock->NDim > 1 &&
      pMasterArrayBlock->Flags & SDS_ARRAY_F_CONTIGUOUS) {
    printf("!!!likely internal error with fortran array > dim 1 and upgrading "
           "dtype!\n");
...
typedef void (*CONVERT_INPLACE)(void *pDataIn, void *pDataOut,
                                int64_t dataOutSize, int32_t dtypein,
                                int32_t dtypeout);

template <typename T, typename U>
static void ConvertInplace(void *pDataIn, void *pDataOut, int64_t dataInSize,
                           int32_t dtypein, int32_t dtypeout) {

  LOGGING("**conversion %p %p   inLenSize: %lld   inItemSize:%lld  "
          "outItemSize:%lld\n",
...
template <typename T, typename U>
static void ConvertInplaceFloat(void *pDataIn, void *pDataOut,
                                int64_t dataInSize, int32_t dtypein,
                                int32_t dtypeout) {

  LOGGING("**conversionf %p %p   inLenSize: %lld   inItemSize:%lld  "
          "outItemSize:%lld\n",
          pDataIn, pDataOut, dataInSize, sizeof(T), sizeof(U));
  T *pIn = (T *)pDataIn;
  U *pOut = (U *)pDataOut;
...
template <typename T, typename U>
static void ConvertInplaceString(void *pDataIn, void *pDataOut,
                                 int64_t dataInSize, int32_t dtypein,
                                 int32_t dtypeout) {

  LOGGING("**conversions %p %p   inLenSize: %lld   inItemSize:%lld  "
          "outItemSize:%lld\n",
          pDataIn, pDataOut, dataInSize, sizeof(T), sizeof(U));
  T *pIn = (T *)pDataIn;
  U *pOut = (U *)pDataOut;
...
static CONVERT_INPLACE GetInplaceConversionFunction(int32_t inputType,
                                                    int32_t outputType) {
...
void GapFill(void *destBuffer, SDSArrayInfo *pDestInfo) {
  LOGGING(
      ">>> gap fill in decompress multi array  length:%lld   dtype:%d  %p\n",
      pDestInfo->ArrayLength, pDestInfo->NumpyDType, destBuffer);
  LOGGING(">>> more gap dims: %d  itemsz:%d  dim0:%lld  strides0:%lld\n",
          pDestInfo->NDim, pDestInfo->ItemSize, pDestInfo->Dimensions[0],
          pDestInfo->Strides[0]);
...
void ConvertDType(char *destBuffer, SDS_ARRAY_BLOCK *pMasterBlock,
                  SDS_ARRAY_BLOCK *pSlaveBlock, int64_t slaveRows,
  ...
  LOGGING("Convert dtype from:%d  to: %d  buffer:%p  size: %lld   %d vs %d\n",
          ...
          pSlaveBlock->NDim, pMasterBlock->NDim);
...
void GapFillSpecial(void *pDestT,         // (upper left corner of array)
  ...
  LOG_THREAD("In gap fill special sizeof:%lld   %lld %lld %lld %lld  %lf\n",
             sizeof(T), totalRowsize, arrayOffset, arrayRowsize, colSize,
             (double)fill);
...
void MatlabStringFill(uint16_t *pDest, // 2 byte unicode (see note on location)
  ...
  LOG_THREAD("Matlab string fill  %p  %lld %lld %lld %lld\n", pDest,
             totalRowsize, arrayOffset, arrayRowsize, itemSizeMaster);
...
//
void GapFillAny(SDSArrayInfo *pDestInfo, void *destBuffer,
                SDS_IO_PACKET *pIOPacket) {

  SDS_ARRAY_BLOCK *pMasterBlock = pIOPacket->pMasterBlock;

  LOG_THREAD("gapfill any %p  ndim:%d  flags:%d  destBuffer:%p  "
             "arrayoffset:%lld  itemsize:%d  trowlength:%lld\n",
             pDestInfo, pMasterBlock->NDim, pMasterBlock->Flags, destBuffer,
             pIOPacket->ArrayOffset, pMasterBlock->ItemSize,
...
void RotationalFixup(char *pDest,          //
  ...
  LOG_THREAD("Rotational2 fix off:%lld  totlrow:%lld  filerow:%lld  col:%lld  "
             "itemsz:%lld  dest:%p  %p\n",
             arrayOffset, totalRowsize, arrayRowsize, arrayColsize, itemSize,
             pDest, pSrc);
...
void MatlabStringFixup(
  ...
  LOG_THREAD("Matlab string fixup  %p %p  %lld %lld %lld %lld\n", pDest, pSrc,
             totalRowsize, arrayOffset, arrayRowsize, itemSize);
...
void StringFixup(char *destBuffer, SDS_ARRAY_BLOCK *pMasterBlock,
                 SDS_ARRAY_BLOCK *pSlaveBlock, int64_t slaveRowLength,
                 int64_t slaveItemSize) {
  ...
  if (gap > 0 && rows > 0) {
    if (pMasterBlock->DType == SDS_STRING) {
      LOGGING(
          "string gap: %lld  rows: %lld  smallerSize:%lld  oneRowSize:%lld\n",
      ...
      LOGGING(
          "unicode gap: %lld  rows: %lld  smallerSize:%lld  oneRowSize:%lld\n",
          gap, rows, slaveItemSize, oneRowSize);
      // Unicode loop
      oneRowSize /= 4;
      slaveItemSize /= 4;
...
//
SDS_COMPATIBLE IsArrayCompatible(const char *colName,
                                 SDS_ARRAY_BLOCK *pMasterArrayBlock,
                                 SDS_ARRAY_BLOCK *pArrayBlock, bool doFixup) {

  // Fixup for int32 linux vs windows
  int32_t mdtype =
      FixupDType(pMasterArrayBlock->DType, pMasterArrayBlock->ItemSize);
  ...
  if (mdtype != odtype) {
    ...
    if (wmdtype.whole == wodtype.whole) {
      ...
      LOGGING("step1 conversion %s  %d  %d  masteritemsize:%d vs  %d  length: "
              "%lld %lld\n",
              colName, odtype, mdtype, pMasterArrayBlock->ItemSize,
              pArrayBlock->ItemSize, pMasterArrayBlock->Dimensions[0],
      ...
      if (cwhole.w.isFloat && (cwhole.w.isInt || cwhole.w.isUInt)) {
        c.NeedsConversion = true;
        LOGGING("Conversion: possible upgrade to float32/64 from int/uint32_t "
                "for col %s  %d to %d\n",
        ...
        c.NeedsConversion = true;
        LOGGING(
            "Conversion: int32_t /  uint32_t for col %s  %d to %d  fixup: %d\n",
        ...
        if (doFixup) {
          ...
          if (hightype == SDS_ULONGLONG && lowtype == SDS_LONGLONG) {
            ...
            if (handleInt64ToUInt64) {
              printf("Warning: ignoring sign loss going to from int/uint64 for "
                     "col: %s\n",
                     colName);
              // flip it back off
              c.NeedsConversion = false;
              ...
              printf("Warning: precision loss going to float64 from int/uint64 "
                     "for col: %s\n",
                     colName);
    ...
    if (pMasterArrayBlock->DType == SDS_STRING ||
        pMasterArrayBlock->DType == SDS_UNICODE) {
      if (pMasterArrayBlock->ItemSize != pArrayBlock->ItemSize) {
        LOGGING("Conversion: String width mismatch on col: %s   master "
                "itemsize:%d  vs  itemsize: %d\n",
                colName, pMasterArrayBlock->ItemSize, pArrayBlock->ItemSize);
        if (doFixup) {
          if (pMasterArrayBlock->ItemSize < pArrayBlock->ItemSize) {
            // auto expand string
            UpgradeType(pMasterArrayBlock, pMasterArrayBlock->DType,
                        pArrayBlock->ItemSize);
          }
        }
...
void *CheckRotationalFixup(SDS_IO_PACKET *pIOPacket) {
  ...
  if (pMasterBlock) {
    ...
    if (pMasterBlock->NDim >= 2 &&
        pMasterBlock->Flags & SDS_ARRAY_F_CONTIGUOUS) {
      if (pMasterBlock->NDim > 2) {
...
bool FinishRotationalFixup(SDS_IO_PACKET *pIOPacket, void *origBuffer,
                           void *tempBuffer) {
  ...
  if (pMasterBlock->NDim >= 2 && pMasterBlock->Flags & SDS_ARRAY_F_CONTIGUOUS) {
    ...
    if (pMasterBlock->DType == SDS_UNICODE) {
      ...
      MatlabStringFixup<uint32_t>(
          (uint16_t *)
              origBuffer, // original buffer (upper left of final destination)
                          ...
                          int64_t t)    // t=task count from 0 - # of iopackets
{
  ...
  SDS_MULTI_IO_PACKETS *pMultiIOPackets =
      (SDS_MULTI_IO_PACKETS *)pstCompressArraysV;
  ...
  if (pIOPacket->CompMode == COMPRESSION_MODE_SHAREDMEMORY) {
    ...
    destBuffer =
        pIOPacket->pMemoryIO->GetMemoryOffset(pArrayBlock->ArrayDataOffset);
  ...
  if (destBuffer) {
    ...
    LOG_THREAD("ALLOC %p %d %p  stackpos: %lld  bpr: %lld\n", pMultiIOPackets,
               core, pMultiIOPackets->pCoreMemory[core],
    ...
    if (pIOPacket->StackPosition >= 0) {
      ...
      if ((pMasterBlock && pMasterBlock->Flags & SDS_ARRAY_FILTERED) ||
          (pBlockInfo->Flags & SDS_ARRAY_FILTERED)) {
        ...
        LOGGING("***stacking  plus filter  stack position: %lld  buffer: %p  "
                "TrueCount: %lld\n",
                pIOPacket->StackPosition, destBuffer,
                pMultiIOPackets->pFilter->pFilterInfo
                    ? pMultiIOPackets->pFilter
                          ->pFilterInfo[pIOPacket->StackPosition]
                          .TrueCount
                    : 0);
        result = ReadAndDecompressArrayBlockWithFilter(
            pBlockInfo, pMultiIOPackets->eventHandles[core], sdsFile,
            tempFileBuffer, destBuffer,
            pIOPacket->OriginalArrayOffset, // array offset (start row)

            pMultiIOPackets->pFilter, pIOPacket->StackPosition, core,
            COMPRESSION_TYPE_ZSTD);
        ...
        if (pMultiIOPackets->pFilter->pFilterInfo) {
          slaveRowLength =
              pMultiIOPackets->pFilter->pFilterInfo[pIOPacket->StackPosition]
                  .TrueCount;
        } else {
        ...
        LOGGING("[%lld]***stacking no filter  stack position: %lld  buffer: %p "
                " rowlength: %lld  colname: %s\n",
                t, pIOPacket->StackPosition, destBuffer,
                pBlockInfo->Dimensions[0], pIOPacket->ColName);

        result = ReadAndDecompressArrayBlock(
            pBlockInfo, pMultiIOPackets->eventHandles[core], sdsFile,
            tempFileBuffer, destBuffer, pIOPacket->StackPosition, core,
            COMPRESSION_TYPE_ZSTD);
      ...
      if (result >= 0) {
        ...
        if (origBuffer) {
          ...
          if (pIOPacket->Compatible.NeedsStringFixup == 1) {
            ...
            StringFixup((char *)destBuffer, pMasterBlock, pBlockInfo,
                        slaveRowLength, slaveItemSize);
          }
          if (pIOPacket->Compatible.NeedsConversion) {
            // done inplace (no need for extra buffer)
            LOGGING("Needs conversion  mb dtype:%d  mb itemsize: %d    dim0: "
                    "%lld   slaveRowLength:%lld  needs stringfixup: %d  "
                    "filterflags: %d\n",
                    pIOPacket->pMasterBlock->DType,
                    pIOPacket->pMasterBlock->ItemSize,
                    pIOPacket->pMasterBlock->Dimensions[0], slaveRowLength,
                    pIOPacket->Compatible.NeedsStringFixup,
                    pMasterBlock->Flags);
            ConvertDType((char *)destBuffer, pMasterBlock, pBlockInfo,
                         slaveRowLength, slaveItemSize);
          }
        }
      }
...
public:
  ...
  void AllocateVectorList(int64_t validCount, int64_t *&pArrayOffsets,
  ...
  void AddColumnList(int64_t validPos, int64_t validCount,
                     const char *columnName,
                     int32_t arrayEnum, // DATASET, OTHER
                     int32_t fileRow, int64_t column,
                     SDS_ARRAY_BLOCK *pArrayBlock) {
    ...
    if (columnFind == ColumnExists.end()) {
      ...
      AllocateVectorList(validCount, pArrayOffsets, pOriginalArrayOffsets,
                         pOriginalLengths, ppArrayBlocks);

      // Add to the end (in this column order) the new column
      ColumnVector.push_back(
          {columnName, TotalUniqueColumns, fileRow, arrayEnum,
           *pArrayBlock, // copy of the king (determines shape of column)
           ppArrayBlocks, pArrayOffsets, pOriginalArrayOffsets,
           pOriginalLengths, 0});
      ...
      SDS_COMPATIBLE compat = IsArrayCompatible(
          columnName, &ColumnVector[colPos].KingBlock, pArrayBlock, true);

      if (compat.NeedsRotation) {
        // cannot handle
        printf("Column '%s' needs rotation from col or row major and currently "
               "this is not allowed\n",
               columnName);
        TotalDimensionProblems++;
      }
      if (!compat.IsCompatible) {
        printf("Warning: Column '%s' has both string and unicode. Support for "
  ...
  // constructor
  // shareName NULL is allowed
  SDSDecompressManyFiles(SDSDecompressFile **pSDSDecompressFile,
                         SDSIncludeExclude *pIncludeList,
                         SDSIncludeExclude *pFolderList,
                         SDSIncludeExclude *pSectionsList, int64_t fileCount,
                         SDS_READ_CALLBACKS *pReadCallbacks) {
  ...
  static bool DecompressManyFiles(void *pstManyV, int32_t core, int64_t t) {
    ...
    SDSDecompressManyFiles *pSDSDecompressMany =
        (SDSDecompressManyFiles *)pstManyV;
    SDSDecompressFile *pSDSDecompressFile =
        pSDSDecompressMany->pSDSDecompressFile[t];

    // isFileValid will be set if the file is good
    pSDSDecompressFile->DecompressFileInternal(
        pSDSDecompressMany->pReadCallbacks, core, 0);

    if (pSDSDecompressFile->IsFileValid &&
        pSDSDecompressFile->Mode == SDS_MULTI_MODE_CONCAT_MANY) {
      pSDSDecompressFile->FileSize =
          DefaultFileIO.FileSize(pSDSDecompressFile->FileName);
    }
    return true;
  }
  ...
  void GetFileInfo(int32_t multiMode) {
    ...
    // Multithreaded work and we tell caller when we started/stopped
    void *saveState = pReadCallbacks->BeginAllowThreads();
    g_cMathWorker->DoMultiThreadedWork((int)FileCount, &DecompressManyFiles,
                                       this);
    pReadCallbacks->EndAllowThreads(saveState);
  }
  ...
  //
  void *ReadIOPackets(SDS_FINAL_CALLBACK *pReadFinalCallback,
                      SDS_READ_CALLBACKS *pReadCallbacks) {
    // Build IO LIST
    ...
    SDS_MULTI_IO_PACKETS *pMultiIOPackets =
        SDS_MULTI_IO_PACKETS::Allocate(totalIOPackets, &pReadCallbacks->Filter);
    ...
    for (int64_t f = 0; f < FileCount; f++) {
      ...
      if (pSDSDecompress->IsFileValid) {
        ...
        pSDSDecompress->AllocMultiArrays(pIOPacket, pArrayInfo, pReadCallbacks,
                                         tupleSize, false);
    ...
    if (totalIOPackets) {
      ...
      // Multithreaded work and we tell caller when we started/stopped
      void *saveState = pReadCallbacks->BeginAllowThreads();
      g_cMathWorker->DoMultiThreadedWork((int)totalIOPackets,
                                         DecompressMultiArray, pMultiIOPackets);
      pReadCallbacks->EndAllowThreads(saveState);
    }

    void *result =
        pReadCallbacks->ReadFinalCallback(pReadFinalCallback, FileCount);
  ...
  static void UpdateSectionData(char *pSectionData, int64_t currentSection,
  ...
  static int64_t AppendToFile(SDS_FILE_HANDLE outFileHandle,
                              SDSDecompressFile *pSDSDecompress,
                              int64_t fileOffset, int64_t fileSize,
                              char *pSectionData, int64_t &currentSection) {
    ...
    LOGGING("sds_concat %lld  vs  %lld   fileoffset:%lld\n", fileSize,
            pFileHeader->GetEndOfFileOffset(), fileOffset);
    ...
    int64_t bytesXfer = DefaultFileIO.FileWriteChunk(
        NULL, outFileHandle, pFileHeader, sizeof(SDS_FILE_HEADER), fileOffset);
    ...
    while (fileSize > 0) {
      ...
      // read and write
      int64_t sizeRead = DefaultFileIO.FileReadChunk(NULL, inFile, pBuffer,
                                                     copySize, localOffset);
      int64_t sizeWritten = DefaultFileIO.FileWriteChunk(
          NULL, outFileHandle, pBuffer, copySize, currentOffset);
    ...
    SDS_ARRAY_BLOCK *pDestArrayBlock =
        (SDS_ARRAY_BLOCK *)WORKSPACE_ALLOC(pFileHeader->ArrayBlockSize);

    bytesXfer = DefaultFileIO.FileReadChunk(NULL, inFile, pDestArrayBlock,
                                            pFileHeader->ArrayBlockSize,
    ...
    DefaultFileIO.FileWriteChunk(NULL, outFileHandle, pDestArrayBlock,
                                 pFileHeader->ArrayBlockSize,
    ...
    if (hasSections) {
      ...
      for (int64_t section = 1; section < sections; section++) {
        int64_t sectionOffset =
            pSDSDecompress->cSectionName.pSectionOffsets[section];
        // section within a section
        // read in a new fileheader
        SDS_FILE_HEADER tempFileHeader;
        int64_t bytesRead =
            DefaultFileIO.FileReadChunk(NULL, inFile, &tempFileHeader,
                                        sizeof(SDS_FILE_HEADER), sectionOffset);

        LOGGING("concat: reading section at %lld for output fileoffset %lld\n",
                sectionOffset, fileOffset);

        if (bytesRead != sizeof(SDS_FILE_HEADER)) {
          printf("!!warning file %s failed to read section header at offset "
                 "%lld\n",
        ...
        LOGGING("concat: Some offsets %lld %lld %lld  sbo:%lld  fo:%lld\n",
                tempFileHeader.NameBlockOffset, tempFileHeader.MetaBlockOffset,
                tempFileHeader.ArrayBlockOffset,
                tempFileHeader.SectionBlockOffset, tempFileHeader.FileOffset);
        // Fixup header offsets
        ...
        int64_t sizeWritten =
            DefaultFileIO.FileWriteChunk(NULL, outFileHandle, &tempFileHeader,
        ...
        //-----------------------------------------
        // Read array block and fix that up
        SDS_ARRAY_BLOCK *pDestArrayBlock2 =
            (SDS_ARRAY_BLOCK *)WORKSPACE_ALLOC(tempFileHeader.ArrayBlockSize);

        bytesXfer = DefaultFileIO.FileReadChunk(NULL, inFile, pDestArrayBlock2,
                                                tempFileHeader.ArrayBlockSize,
                                                origArrayBlockOffset);
        if (bytesXfer != tempFileHeader.ArrayBlockSize) {
          printf(
              "!!warning file %s failed to read array block at offset %lld\n",
              pSDSDecompress->FileName, origArrayBlockOffset);
        }
        ...
        // Write the array block with offsets fixed up
        DefaultFileIO.FileWriteChunk(NULL, outFileHandle, pDestArrayBlock2,
                                     tempFileHeader.ArrayBlockSize,
                                     fileOffset + origArrayBlockOffset);
  ...
  void *SDSConcatFiles(const char *strOutputFilename, int64_t validCount) {
    ...
    if (!fileHandle) {
      SetErr_Format(SDS_VALUE_ERROR,
                    "Concat error cannot create/open file: %s.  Error: %s",
                    strOutputFilename, "none");
      return BAD_SDS_HANDLE;
    }
    ...
    if (pFileHeader) {
      ...
      for (int64_t t = 0; t < FileCount; t++) {
        ...
        if (pSDSDecompress->IsFileValid) {
          ...
          int64_t nextOffset = AppendToFile(
              fileHandle, pSDSDecompress, fileOffset, pSDSDecompress->FileSize,
      ...
      int64_t result = DefaultFileIO.FileWriteChunk(
          NULL, fileHandle, pSectionData, sectionTotalSize, fileOffset);
  ...
  void *ReadManyFiles(SDS_READ_CALLBACKS *pReadCallbacks, int32_t multiMode) {
    ...
    if (pReadCallbacks->MustExist && missingfile >= 0) {
      ...
      SetErr_Format(SDS_VALUE_ERROR,
                    "Not all files found : Expected %lld files.  Missing %s\n",
    ...
    LOGGING("GetInfo ReadManyFiles complete.  FileCount %lld. valid %lld.  "
            "mode:%d \n",
    ...
    if (multiMode == SDS_MULTI_MODE_CONCAT_MANY) {
      ...
      SDS_FINAL_CALLBACK *pReadFinalCallback =
          (SDS_FINAL_CALLBACK *)WORKSPACE_ALLOC(sizeof(SDS_FINAL_CALLBACK) *
                                                FileCount);
      ...
      for (int64_t t = 0; t < FileCount; t++) {
        ...
        if (pSDSDecompress->IsFileValid) {
          ...
          pReadFinalCallback[t].arraysWritten =
              pSDSDecompress->pFileHeader->ArraysWritten;
  ...
  //========================================
  SDSDecompressFile *CopyDecompressFileFrom(SDSDecompressFile *pSDSDecompress,
                                            int64_t instance) {
    return new SDSDecompressFile(
        pSDSDecompress->FileName, pSDSDecompress->pInclude,
        instance, // instanceindex
        pSDSDecompress->ShareName, pSDSDecompress->pFolderName,
        pSDSDecompress->pSectionsName, pSDSDecompress->Mode);
  }
  ...
  SDSDecompressFile **ScanForSections() {
    ...
    for (int64_t t = 0; t < FileCount; t++) {
      ...
      if (pSDSDecompress->IsFileValid &&
          pSDSDecompress->pFileHeader->SectionBlockCount > 0) {
    ...
    if (FileWithSectionsCount > FileCount) {
      ...
      pSDSDecompressFileExtra = (SDSDecompressFile **)WORKSPACE_ALLOC(
          sizeof(void *) * FileWithSectionsCount);
      ...
      for (int64_t t = 0; t < FileCount; t++) {
        ...
        if (pSDSDecompress->IsFileValid &&
          ...
          for (int64_t section = 0; section < blockcount; section++) {
            ...
            SDSDecompressFile *pSDSDecompressExtra =
                CopyDecompressFileFrom(pSDSDecompress, instance);
            ...
            int64_t fileoffset =
                pSDSDecompress->cSectionName.pSectionOffsets[section];
            pSDSDecompressExtra->DecompressFileInternal(pReadCallbacks, 0,
                                                        fileoffset);
          }
          FileWithSectionsCount += blockcount;
        } else {
          pSDSDecompressFileExtra[FileWithSectionsCount] =
              CopyDecompressFileFrom(pSDSDecompress, FileWithSectionsCount);

          // TJD: Check to make sure no memory leaks, also, future optimization
          // - we do not have to read it again
          pSDSDecompressFileExtra[FileWithSectionsCount]
              ->DecompressFileInternal(pReadCallbacks, 0, 0);
          FileWithSectionsCount += 1;
        }
      }
  ...
  void *ReadAndStackFiles(SDS_READ_CALLBACKS *pReadCallbacks,
                          int32_t multiMode) {
    // Open what might be 100+ files
    GetFileInfo(multiMode);
    ...
    LOGGING("GetInfo ReadAndStackFiles complete.  FileCount %lld. valid %lld. "
            "reserve space set to %lf\n",
    ...
    for (int32_t t = 0; t < FileCount; t++) {
      ...
      if (pSDSDecompress->IsFileValid) {
        ...
        for (int64_t c = 0; c < pSDSDecompress->NameCount; c++) {
          ...
          if (IsNameIncluded(pIncludeList, pFolderList, columnName,
                             isOneFile)) {
                ...
                if (pArrayBlock && pArrayBlock->NDim > 0) {
                  ...
                  if (pDatasets[validPos].Length != 0) {
                    if (pDatasets[validPos].Length != dlength) {
                      printf("WARNING: datasets not same length %lld v. %lld "
                             "for column %s\n",
                  ...
                  LOGGING("Thomas band %lld %lld\n",
                          pSDSDecompress->FileHeader.BandBlockCount,
                          pSDSDecompress->FileHeader.BandSize);
                }
              }
            }
          }
    ...
    LOGGING("\nTotal column stats  uniq:%d, first:%d, conv:%d, strfix:%d, "
            "dim:%d, colgaps:%d, valid: %lld\n",
            TotalUniqueColumns, TotalFirstColumns, TotalConversions,
            TotalStringFixups, TotalDimensionProblems, TotalColumnGaps,
            validCount);
    ...
    if (validCount == 0) {
      SetErr_Format(
          SDS_VALUE_ERROR,
          "MultiDecompress error -- none of the files were valid: %s\n",
          pFirstFileName);
    } else
      ...
      SetErr_Format(
          SDS_VALUE_ERROR,
          "MultiDecompress error -- detected a dimension fixup problem %s\n",
          pFirstFileName);
    } else {
      isGood = true;
    }
    ...
    if (isGood) {
      ...
      SDS_MULTI_IO_PACKETS *pMultiIOPackets = SDS_MULTI_IO_PACKETS::Allocate(
          totalIOPackets, &pReadCallbacks->Filter);

      // Also for each column we create SDSArrayInfo
      SDSArrayInfo *pManyDestInfo = (SDSArrayInfo *)WORKSPACE_ALLOC(
          sizeof(SDSArrayInfo) * TotalUniqueColumns);
      ...
      if (pReadCallbacks->Filter.pBoolMask) {
        ...
        pFilterInfo =
            (SDSFilterInfo *)WORKSPACE_ALLOC(sizeof(SDSFilterInfo) * FileCount);
      ...
      for (int64_t col = 0; col < TotalUniqueColumns; col++) {
        ...
        // int32_t mask = SDS_FLAGS_ORIGINAL_CONTAINER;
        LOGGING("[%lld]Col: %s -- ColPos:%lld  FileRow:%d pBlock:%p Enum:%d  "
                "ftype:%d\n",
                col, pKing->ColName, pKing->ColPos, pKing->FileRow,
                pKing->ppArrayBlocks, pKing->ArrayEnum, fileTypeStackable);
        ...
        for (int32_t f = 0; f < FileCount; f++) {
          ...
          if (pSDSDecompress->IsFileValid) {
            ...
            if (pArrayBlock) {
              isCompatible = IsArrayCompatible(pKing->ColName, pMasterBlock,
                                               pArrayBlock, false);
            }
            ...
            if (isFilterable) {
              ...
              if (currentUnfilteredOffset < boolMaskLength) {
                ...
                if (hasFilter) {
                  ...
                  filterTrueCount = SumBooleanMask(
                      (int8_t *)pReadCallbacks->Filter.pBoolMask +
                          currentUnfilteredOffset,
                      bLength);
                }
              }
              ...
              pFilterInfo[row].RowOffset =
                  row == 0 ? 0
                           : pFilterInfo[row - 1].RowOffset +
                                 pFilterInfo[row - 1].TrueCount;
              ...
              LOGGING("ROW %lld has true: %lld  ufo: %lld  for offset: %lld vs "
                      "%lld\n",
                      row, pFilterInfo[row].TrueCount, currentUnfilteredOffset,
                      currentOffset, pFilterInfo[row].RowOffset);
            } else {
        ...
        LOGGING("stack: %d   totalplusreseve:%lld  totalrowlength: %lld  rows: "
                "%lld\n",
        ...
        sdsArrayCallback.numpyType =
            FixupDType(pMasterBlock->DType, pMasterBlock->ItemSize);
        ...
        if (pReadCallbacks->AllocateArrayCallback) {
          ...
          // FILTERING check...
          wasFiltered |= PossiblyShrinkArray(&sdsArrayCallback, pReadCallbacks,
                                             fileTypeStackable);

          LOGGING(
              "Allocating col: %s  lengthplusr: %lld  length: %lld   itemsize: "
              "%lld  strides: %lld  dims:%d  dtype:%d  wasFiltered:%d\n",
              sdsArrayCallback.pArrayName, rowLengthToAlloc, currentOffset,
              sdsArrayCallback.itemsize, strides[0], (int)sdsArrayCallback.ndim,
              sdsArrayCallback.numpyType, wasFiltered);
          // callback into python or matlab to allocate memory
          // this will fill in pData and pArrayObject
          pReadCallbacks->AllocateArrayCallback(&sdsArrayCallback);
          LOGGING("Array allocated at %p for object %p\n",
                  sdsArrayCallback.pDestInfo->pData,
        ...
        for (int32_t f = 0; f < FileCount; f++) {
          ...
          if (pSDSDecompress->IsFileValid) {
            ...
            if (pArrayBlock) {
              pIOPacket->Compatible = IsArrayCompatible(
                  pKing->ColName, pMasterBlock, pArrayBlock, false);
            }
            ...
            if (!pArrayBlock || pOriginalLengths[row] == 0) {
              ...
              // NO IO PACKET (GAP)
              LOGGING(">>> gap fill %lld  for row: %lld   col: %lld   "
                      "orig:%lld  ds:%lld  mdtype:%d\n",
                      gapLength, row, col, pOriginalLengths[row],
                      pDatasets[row].Length, pMasterBlock->DType);
            ...
            if ((pIOPacket->pMasterBlock->NDim >= 2 &&
                 pIOPacket->pMasterBlock->Flags & SDS_ARRAY_F_CONTIGUOUS) ||
              ...
              // into
              pDestInfo->pData = sdsArrayCallback.pDestInfo->pData +
                                 (oneRowSize * pArrayOffsets[row]);
              LOGGING(
                  "Fixing up pData for stackposition: %lld  itemoffset:%lld  "
                  "onerowsize:%lld  mbitemsize:%d  dest:%p\n",
                  row, pArrayOffsets[row], oneRowSize, pMasterBlock->ItemSize,
                  pDestInfo->pData);
            }
      ...
      if (validPos) {
        ...
        // Multithreaded work and we tell caller when we started/stopped
        void *saveState = pReadCallbacks->BeginAllowThreads();
        g_cMathWorker->DoMultiThreadedWork((int)validPos, DecompressMultiArray,
                                           pMultiIOPackets);
        pReadCallbacks->EndAllowThreads(saveState);
      }
      ...
      SDS_STACK_CALLBACK *pMultiFinalCallback =
          (SDS_STACK_CALLBACK *)WORKSPACE_ALLOC(sizeof(SDS_STACK_CALLBACK) *
                                                TotalUniqueColumns);

      SDS_STACK_CALLBACK_FILES *pMultiCallbackFileInfo =
          (SDS_STACK_CALLBACK_FILES *)WORKSPACE_ALLOC(
              sizeof(SDS_STACK_CALLBACK_FILES) * validCount);
      ...
      for (int32_t f = 0; f < FileCount; f++) {
        ...
        if (pSDSDecompress->IsFileValid) {
          ...
          pMultiCallbackFileInfo[vrow].pFileHeader =
              pSDSDecompress->pFileHeader;
          vrow++;
        }
      }

      result = pReadCallbacks->StackFinalCallback(
          pMultiFinalCallback, TotalUniqueColumns, pMultiCallbackFileInfo,
...
DllExport int32_t SDSWriteFile(const char *fileName,
  ...
  return SDSWriteFileInternal(fileName, shareName, folderName, pWriteInfo,
                              pWriteCallbacks);
}
...
DllExport void *SDSReadFile(
  ...
  SDSDecompressFile decompress(fileName, &includeList, 0, shareName,
                               &folderList, &sectionsList, pReadInfo->mode);
...
// Returns what the user specified in ReadFinal
DllExport void *
SDSReadManyFiles(SDS_MULTI_READ *pMultiRead,
                 ...
                 int64_t fileCount, int32_t multiMode,
                 SDS_READ_CALLBACKS *pReadCallbacks) {
  void *result = NULL;
  ClearErrors();
  ...
  if (multiMode == SDS_MULTI_MODE_STACK_OR_READMANY) {
    ...
    if (fileCount > 0) {
      SDS_FILE_HANDLE fileHandle = DefaultFileIO.FileOpen(
          pMultiRead[0].pFileName, false, true, false, false);
      ...
      if (!fileHandle) {
        ...
        // Read file header
        SDS_FILE_HEADER tempFileHeader;
        int64_t result = ReadFileHeader(fileHandle, &tempFileHeader, 0,
                                        pMultiRead[0].pFileName);
  ...
  if (multiMode != SDS_MULTI_MODE_ERROR) {
    ...
    // Allocate an array of all the files we need to open
    SDSDecompressFile **pSDSDecompressFile =
        (SDSDecompressFile **)WORKSPACE_ALLOC(sizeof(void *) * fileCount);

    for (int64_t i = 0; i < fileCount; i++) {
      LOGGING("...%s\n", pMultiRead[i].pFileName);
      pSDSDecompressFile[i] = new SDSDecompressFile(
          pMultiRead[i].pFileName, &includeList, i, NULL, &folderList,
          &sectionsList,
          multiMode == SDS_MULTI_MODE_CONCAT_MANY ? SDS_MULTI_MODE_CONCAT_MANY
                                                  : COMPRESSION_MODE_INFO);
    }

    SDSDecompressManyFiles manyFiles(pSDSDecompressFile, &includeList,
                                     &folderList, &sectionsList, fileCount,
                                     pReadCallbacks);

    if (multiMode == SDS_MULTI_MODE_READ_MANY ||
...
DllExport char *SDSGetLastError() { return g_errorbuffer; }
