                    ...
                    int64_t metaBlockSize, int64_t arrayCount, int64_t bandSize)
{
    ...
    LOGGING("main offsets %lld  %lld  %lld\n", pFileHeader->MetaBlockOffset, pFileHeader->ArrayBlockOffset,
    ...
    if (pFileHeader->SDSHeaderMagic != SDS_MAGIC || pFileHeader->VersionHigh != SDS_VERSION_HIGH)
    {
        ...
        else
        {
            SetErr_Format(SDS_VALUE_ERROR,
                          "Decompression error cannot understand header for file (corrupt or different filetype): %s  %d  arrays: "
                          "%lld  fileoffset: %lld",
        ...
        else
        {
            ...
            if (result == 0)
            {
                ...
                LOGGING("SDS: Writing section %s with blockcount:%lld at %lld (%lld) at %lld\n", pWriteInfo->sectionName,
                        blockCount, pFileHeader->FileOffset, fileHeader.FileOffset, LastFileOffset);
                int64_t sectionSize =
                    cSDSSectionName.BuildSectionNamesAndOffsets(&pListNames, pWriteInfo->sectionName, pFileHeader->FileOffset);
    ...
    if (aInfo || arrayCount == 0)
    {
        ...
        else
        {
            ...
            if (sdsFile)
            {
                ...
                // (SDS_WRITE_COMPRESS_ARRAYS*)WORKSPACE_ALLOC(sizeof(SDS_WRITE_COMPRESS_ARRAYS) + (arrayCount *
                // sizeof(SDSArrayInfo)));
                SDS_WRITE_COMPRESS_ARRAYS * pstCompressArrays =
                ...
                if (arrayCount > 0)
                {
                    ...
                    // This will kick off the workerthread and call CompressFileArray passing pstCompressArrays as argument with
                    // counter and core
    ...
    if ((pArrayCallback->sdsFlags & (SDS_FLAGS_SCALAR | SDS_FLAGS_META | SDS_FLAGS_NESTED)) == 0)
    {
        if (pArrayCallback->sdsFlags & mask)
        {
            ...
            if (pReadCallbacks->Filter.pBoolMask && pReadCallbacks->Filter.BoolMaskTrueCount >= 0 && isStackable)
            {
                ...
                LOGGING("-->Allocation reduction for %s filter: %lld  vs %lld  bool:%lld  flags:%d\n", pArrayCallback->pArrayName,
                        newLength, dim0Length, pReadCallbacks->Filter.BoolMaskTrueCount, pArrayCallback->sdsFlags);
        ...
        else
        {
            ...
            //   pArrayCallback->pArrayName, dim0Length, pReadCallbacks->Filter.FancyLength,
        ...
        if (! pArrayNames || IsNameIncluded(pInclude, pFolderName, sdsArrayCallback.pArrayName, isOneFile))
        {
            ...
            if (pReadCallbacks->AllocateArrayCallback)
            {
                ...
                LOGGING("Allocating dims:%d -- %lld %lld %lld   flags:%d  itemsize:%d  wasFiltered: %d\n", (int)pBlock->NDim,
                        pBlock->Dimensions[0], pBlock->Dimensions[1], pBlock->Dimensions[2], pBlock->Flags, pBlock->ItemSize,
                        wasFiltered);
        ...
        if (SDSFile)
        {
            ...
            if (hr >= 0)
            {
                ...
                LOGGING("Fill file header   namecount:%lld   arrayswritten:%lld  address %p\n", pFileHeader->NameBlockCount,
                        pFileHeader->ArraysWritten, pMemoryFileHeader);
...
void GapFillAny(SDSArrayInfo * pDestInfo, void * destBuffer, SDS_IO_PACKET * pIOPacket)
{
    ...
    LOG_THREAD("gapfill any %p  ndim:%d  flags:%d  destBuffer:%p  arrayoffset:%lld  itemsize:%d  trowlength:%lld\n", pDestInfo,
               pMasterBlock->NDim, pMasterBlock->Flags, destBuffer, pIOPacket->ArrayOffset, pMasterBlock->ItemSize,
    ...
    if (pMasterBlock->NDim >= 2 && pMasterBlock->Flags & SDS_ARRAY_F_CONTIGUOUS)
    {
        ...
        // printf("Multidim gapfill dims: %d   rowlen:%lld  colsize:%lld  startrow:%lld   dtype: %d\n", (int)pMasterBlock->NDim,
        // rowLength, colSize, pIOPacket->ArrayOffset, (int)pMasterBlock->DType);
        ...
        if (pIOPacket->StackPosition >= 0)
        {
            ...
            else
            {
                LOGGING("[%lld]***stacking no filter  stack position: %lld  buffer: %p  rowlength: %lld  colname: %s\n", t,
                        pIOPacket->StackPosition, destBuffer, pBlockInfo->Dimensions[0], pIOPacket->ColName);
...
public:
    ...
    void * ReadManyFiles(SDS_READ_CALLBACKS * pReadCallbacks, int32_t multiMode)
    {
        ...
        else
        {
            ...
            //      if (!pSDSDecompress->IsFileValid) {
            //         SetErr_Format(SDS_VALUE_ERROR, "Not all files found : Expected %lld files.  Missing %s\n", FileCount,
            //         pSDSDecompress->FileName); printf("Not all files found: Expected %lld files.\n", FileCount); return NULL;
            //      }
            //   }
            //}
            ...
            for (int64_t t = 0; t < FileCount; t++)
            {
                ...
                if (pSDSDecompress->IsFileValid && pSDSDecompress->pFileHeader->SectionBlockCount > 1)
                {
                    ...
                    for (int64_t section = 0; section < blockcount; section++)
                    {
                        ...
                        // read in header for section (note if this is the first header (i.e. section ==0), we are reading it
                        // again)
    ...
    void * ReadAndStackFiles(SDS_READ_CALLBACKS * pReadCallbacks, int32_t multiMode)
    {
        ...
        LOGGING("\nTotal column stats  uniq:%d, first:%d, conv:%d, strfix:%d, dim:%d, colgaps:%d, valid: %lld\n",
                TotalUniqueColumns, TotalFirstColumns, TotalConversions, TotalStringFixups, TotalDimensionProblems,
        ...
        //   SetErr_Format(SDS_VALUE_ERROR, "MultiDecompress error -- all the filetypes must be the same type (struct or dataset or
        ...
        if (isGood)
        {
            ...
            for (int64_t col = 0; col < TotalUniqueColumns; col++)
            {
                ...
                LOGGING("[%lld]Col: %s -- ColPos:%lld  FileRow:%d pBlock:%p Enum:%d  ftype:%d\n", col, pKing->ColName,
                        pKing->ColPos, pKing->FileRow, pKing->ppArrayBlocks, pKing->ArrayEnum, fileTypeStackable);
                ...
                for (int32_t f = 0; f < FileCount; f++)
                {
                    ...
                    if (pSDSDecompress->IsFileValid)
                    {
                        ...
                        else
                        {
                            ...
                            // row, col, pKing->ColName, pOriginalLengths[row], pKing->ArrayEnum,
                        ...
                        if (isFilterable)
                        {
                            ...
                            LOGGING("ROW %lld has true: %lld  ufo: %lld  for offset: %lld vs %lld\n", row,
                                    pFilterInfo[row].TrueCount, currentUnfilteredOffset, currentOffset,
                                    pFilterInfo[row].RowOffset);
                        }
                        else
                        {
                ...
                if (pReadCallbacks->AllocateArrayCallback)
                {
                    ...
                    LOGGING(
                        "Allocating col: %s  lengthplusr: %lld  length: %lld   itemsize: %lld  strides: %lld  dims:%d  dtype:%d  "
                        "wasFiltered:%d\n",
                    ...
                    if (pSDSDecompress->IsFileValid)
                    {
                        ...
                        if (! pArrayBlock || pOriginalLengths[row] == 0)
                        {
                            ...
                            LOGGING(">>> gap fill %lld  for row: %lld   col: %lld   orig:%lld  ds:%lld  mdtype:%d\n", gapLength,
                                    row, col, pOriginalLengths[row], pDatasets[row].Length, pMasterBlock->DType);
