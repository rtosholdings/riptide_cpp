...
#include "platform_detect.h"
    ...
    (defined(RT_COMPILER_MSVC) || defined(RT_COMPILER_CLANG))
...
bool CheckWindowsSharedMemoryPrerequisites(const char *pMappingName) {
  ...
  if (strstr(pMappingName, "Global") &&
      !CheckWindowsPrivilege(pPrivilegeName)) {
    printf("CheckWindowsSharedMemoryPrerequisites: privilege %s needs to be "
...
//
HRESULT
UtilSharedMemoryBegin(const char *pMappingName, INT64 Size,
  ...
  *pReturnStruct = NULL;
  ...
  //
  pMappedViewStruct = static_cast<PMAPPED_VIEW_STRUCT>(
      WORKSPACE_ALLOC(sizeof(MAPPED_VIEW_STRUCT)));
  ...
  pMappedViewStruct->BaseAddress = static_cast<PVOID>(MapViewOfFile(
      pMappedViewStruct->MapHandle, FILE_MAP_ALL_ACCESS, 0, 0, Size));
...
UtilSharedNumaMemoryBegin(const char *pMappingName, INT64 Size,
  ...
  *pReturnStruct = NULL;
  ...
  //
  pMappedViewStruct->MapHandle =
      CreateFileMappingNuma(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, HiWord,
                            LowWord, pMappingName, nndPreferred);
  ...
  pMappedViewStruct->BaseAddress = static_cast<PVOID>(
      MapViewOfFileExNuma(pMappedViewStruct->MapHandle, FILE_MAP_ALL_ACCESS, 0,
                          0, Size, lpBaseAddress, nndPreferred));
...
HRESULT
UtilSharedMemoryCopy(const char *pMappingName,
                     PMAPPED_VIEW_STRUCT *pReturnStruct, BOOL bTest) {
  ...
  *pReturnStruct = NULL;
  ...
  //
  pMappedViewStruct->MapHandle =
      OpenFileMapping(FILE_MAP_ALL_ACCESS, false, pMappingName);
  ...
  pMappedViewStruct->BaseAddress = static_cast<PVOID>(MapViewOfFile(
      pMappedViewStruct->MapHandle, FILE_MAP_ALL_ACCESS, 0, 0, 0));
...
//
HRESULT
UtilMappedViewReadBegin(const char *pszFilename,
                        PMAPPED_VIEW_STRUCT *pReturnStruct) {
  ...
  *pReturnStruct = NULL;
  ...
  //
  pMappedViewStruct->FileHandle = CreateFile(
      pszFilename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING,
  ...
  if (pMappedViewStruct->FileHandle == (void *)HFILE_ERROR) {
    ...
    //
    pMappedViewStruct->FileHandle = CreateFile(
        pszFilename, GENERIC_READ, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL,
  ...
  //
  pMappedViewStruct->MapHandle = CreateFileMapping(
      (HANDLE)pMappedViewStruct->FileHandle, NULL, PAGE_READONLY, 0, 0,
      0); // pMappingName);
  ...
  pMappedViewStruct->BaseAddress = pulFile = static_cast<PULONG>(
      MapViewOfFile(pMappedViewStruct->MapHandle, FILE_MAP_READ, 0, 0, 0));
  ...
  // Get File Size
  //
  pMappedViewStruct->FileSize = pMappedViewStruct->RealFileSize =
      GetFileSize(pMappedViewStruct->FileHandle, NULL);
...
//
HRESULT
UtilMappedViewWriteBegin(const char *pszFilename,
                         PMAPPED_VIEW_STRUCT *pReturnStruct,
  ...
  *pReturnStruct = NULL;
  ...
  //
  pMappedViewStruct->FileHandle = CreateFile(
      pszFilename, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, NULL,
      OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL | FILE_FLAG_SEQUENTIAL_SCAN, NULL);
  ...
  if (pMappedViewStruct->FileHandle == (void *)HFILE_ERROR) {
    ...
    // The "AutoCAD has the file as well" bug
    //
    pMappedViewStruct->FileHandle =
        CreateFile(pszFilename, GENERIC_READ | GENERIC_WRITE,
  ...
  //
  pMappedViewStruct->MapHandle =
      CreateFileMapping((HANDLE)pMappedViewStruct->FileHandle, NULL,
                        PAGE_READWRITE, dwMaxSizeHigh, dwMaxSizeLow,
                        0); // pMappingName);
  ...
  pMappedViewStruct->BaseAddress = pulFile = static_cast<PULONG>(MapViewOfFile(
      pMappedViewStruct->MapHandle, FILE_MAP_READ | FILE_MAP_WRITE, 0, 0, 0));
...
HRESULT
UtilSharedMemoryBegin(const char *pMappingName, int64_t size,
                      PMAPPED_VIEW_STRUCT *pReturnStruct) {
  ...
  *pReturnStruct = NULL;
  ...
  LOGGING("Linux: shared memory %s at %p  with size:%lld\n", pMappingName,
          memaddress, (long long)size);

  //
  // Allocate memory
  //
  PMAPPED_VIEW_STRUCT pMappedViewStruct = static_cast<PMAPPED_VIEW_STRUCT>(
      WORKSPACE_ALLOC(sizeof(MAPPED_VIEW_STRUCT)));
...
HRESULT
UtilSharedMemoryCopy(const char *pMappingName,
                     PMAPPED_VIEW_STRUCT *pReturnStruct, int bTest) {
