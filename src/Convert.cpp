...
PyObject * ConvertSafeInternal(PyArrayObject * const inArr1, const int64_t out_dtype)
{
    ...
    if (directionIn != 0 || directionOut != 0)
    {
        ...
        else
        {
            ...
            // return PyErr_Format(PyExc_RuntimeError, "ConvertSafe allocated an output array whose *_CONTIGUOUS flags were not set
            // even though the input array was contiguous. %d %d   out:%d %d  strideIn:%lld  strideOut:%lld", contigIn, ndim,
            // contigOut, ndimOut, strideIn, strideOut);
        }
    }
    else
    {
...
PyObject * CombineFilter(PyObject * self, PyObject * args)
{
    ...
    switch (numpyOutType)
    {
    case NPY_INT8: pFunction = GetCombineFunction(numpyOutType); break;
...
PyObject * CombineAccum2Filter(PyObject * self, PyObject * args)
{
    ...
    PyErr_Format(
        PyExc_ValueError,
        "Dont know how to combine filter these types %d.  Please make sure all bins are int8_t, int16_t, int32_t, or int64_t.",
        numpyOutType);
    return NULL;
}
...
PyObject * CombineAccum1Filter(PyObject * self, PyObject * args)
{
    ...
    if (hashSize < 0 || hashSize > 2000000000)
    {
        PyErr_Format(PyExc_ValueError,
                     "CombineAccum1Filter: Index sizes are either 0, negative, or produce more than 2 billion results %lld",
                     hashSize);
        return NULL;
    }
    ...
    switch (dtype)
    {
    case NPY_INT8: pFunction = Combine1Filter<int8_t>; break;
    case NPY_INT16:
    ...
    if (pFunction != NULL)
    {
        ...
        PyArrayObject * firstArray =
            AllocateNumpyArray(1, (npy_intp *)&arraySize1, NPY_INT32); // TODO: bump up to int64_t for large arrays
        ...
        if (outArray && firstArray)
        {
            ...
            if (uniqueCount < arraySize1)
            {
                ...
                PyArrayObject * firstArrayReduced =
                    AllocateNumpyArray(1, (npy_intp *)&uniqueCount, NPY_INT32); // TODO: bump up to int64_t for large arrays
...
PyObject * MakeiFirst(PyObject * self, PyObject * args)
{
    ...
    {
        PyErr_Format(PyExc_ValueError,
                     "MakeiFirst: Index sizes are either 0, negative, or produce more than 20 billion results %lld", hashSize);
        return NULL;
    }
    ...
    if (isLast)
    {
        switch (dtype)
        {
        case NPY_INT8: pFunction = iLastFilter<int8_t>; break;
        case NPY_INT16:
    ...
    else
    {
        switch (dtype)
        {
        case NPY_INT8: pFunction = iFirstFilter<int8_t>; break;
        case NPY_INT16:
...
PyObject * HStack(PyObject * self, PyObject * args)
{
    ...
    if (dtype != -1)
    {
        ...
        {
            PyErr_Format(PyExc_ValueError,
                         "Dont know how to convert dtype num %d.  Please make sure all arrays are ints or floats.", dtype);
            return NULL;
        }
    }
        ...
        else
        {
            ...
            for (int t = 0; t < tupleSize; t++)
            {
                ...
                if (a_dtype > NPY_LONGDOUBLE || aInfo[t].NDim != 1)
                {
                    FreeArrayInfo(aInfo);
                    PyErr_Format(PyExc_ValueError,
                                 "Dont know how to convert dtype num %d or more than 1 dimension.  Please make sure all arrays "
                                 "are ints or floats.",
                                 a_dtype);
                    return NULL;
                }
    ...
    if (aInfo)
    {
        ...
        if (dtype != -1)
        {
            if (dtype < 0 || dtype > NPY_LONGDOUBLE)
            {
                PyErr_Format(
                    PyExc_ValueError,
                    "HomogenizeArrays: Dont know how to convert dtype num %d.  Please make sure all arrays are ints or floats.",
                    dtype);
                return NULL;
            }
        }
