...
//-----------------------------------
//
void ConvertRecArray(char *pStartOffset, int64_t startRow, int64_t totalRows,
                     stOffsets *pstOffset, int64_t *pOffsets, int64_t numArrays,
                     int64_t itemSize) {
  ...
  __m256i vindex = _mm256_mullo_epi32(_mm256_set1_epi32((int32_t)itemSize),
                                      _mm256_loadu_si256(&__vindex8_strides.m));
    ...
    for (int64_t i = 0; i < numArrays; i++) {
      ...
      case 4:
        // ??? use _mm256_i32gather_epi32 to speed up
        {
          ...
          while (startRow < endSubRow) {
            __m256i m0 = _mm256_i32gather_epi32(
                reinterpret_cast<int32_t *>(pRead + (startRow * itemSize)),
                vindex, 1);
            _mm256_storeu_si256((__m256i *)(pWrite + (startRow * arrItemSize)),
                                m0);
            startRow += 8;
          }
          while (startRow < endRow) {
      ...
      case 8: {
        ...
        while (startRow < endSubRow) {
          ...
          __m256i m0 =
              _mm256_i32gather_epi64(reinterpret_cast<long long const *>(
                                         pRead + (startRow * itemSize)),
                                     vindex128, 1);
#else
          __m256i m0 = _mm256_i32gather_epi64(
              reinterpret_cast<int64_t const *>(pRead + (startRow * itemSize)),
              vindex128, 1);
#endif
          _mm256_storeu_si256((__m256i *)(pWrite + (startRow * arrItemSize)),
                              m0);
...
PyObject *RecordArrayToColMajor(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!:RecordArrayToColMajor", &PyArray_Type,
                        &inArr, &PyArray_Type, &offsetArr, &PyArray_Type,
                        &arrArr)) {
    return NULL;
  }
  ...
  if (itemSize != PyArray_STRIDE(inArr, 0)) {
    PyErr_Format(PyExc_ValueError,
                 "RecordArrayToColMajor cannot handle strides");
    return NULL;
  }
  ...
  if (totalRows > 16384) {
    ...
    auto lambdaConvertRecCallback = [](void *callbackArgT, int core,
      ...
      ConvertRecArray(callbackArg->pStartOffset, startRow, totalRows,
                      callbackArg->pstOffset, callbackArg->pOffsets,
    ...
    g_cMathWorker->DoMultiThreadedWork((int)items, lambdaConvertRecCallback,
                                       &stConvert);

  } else {
    ConvertRecArray(pStartOffset, startRow, totalRows, pstOffset, pOffsets,
                    numArrays, itemSize);
  }
