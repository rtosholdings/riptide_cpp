 ...
 * @tparam T A C++ primitive type.
 */
template <typename T> struct numpy_ctype_code {};

template <> struct numpy_ctype_code<bool> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_BOOL;
};

template <> struct numpy_ctype_code<int8_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_INT8;
};

template <> struct numpy_ctype_code<int16_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_INT16;
};

template <> struct numpy_ctype_code<int32_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_INT32;
};

template <> struct numpy_ctype_code<int64_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_INT64;
};

template <> struct numpy_ctype_code<uint8_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_UINT8;
};

template <> struct numpy_ctype_code<uint16_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_UINT16;
};

template <> struct numpy_ctype_code<uint32_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_UINT32;
};

template <> struct numpy_ctype_code<uint64_t> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_UINT64;
};

template <> struct numpy_ctype_code<float> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_FLOAT32;
};

template <> struct numpy_ctype_code<double> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_FLOAT64;
};

#if NPY_SIZEOF_LONGDOUBLE != NPY_SIZEOF_DOUBLE

template <> struct numpy_ctype_code<long double> {
  static constexpr NPY_TYPES value = NPY_TYPES::NPY_LONGDOUBLE;
};
 ...
 */
template <NPY_TYPES ndarraytype> struct numpy_cpp_type {};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_BOOL> { using type = bool; };

template <> struct numpy_cpp_type<NPY_TYPES::NPY_INT8> {
  using type = npy_int8;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_INT16> {
  using type = npy_int16;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_INT32> {
  using type = npy_int32;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_INT64> {
  using type = npy_int64;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_UINT8> {
  using type = npy_uint8;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_UINT16> {
  using type = npy_uint16;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_UINT32> {
  using type = npy_uint32;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_UINT64> {
  using type = npy_uint64;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_FLOAT32> {
  using type = npy_float32;
};

template <> struct numpy_cpp_type<NPY_TYPES::NPY_FLOAT64> {
  using type = npy_float64;
};

#if NPY_SIZEOF_LONGDOUBLE != NPY_SIZEOF_DOUBLE

template <> struct numpy_cpp_type<NPY_TYPES::NPY_LONGDOUBLE> {
 ...
 * @tparam ndarraytype
 */
template <NPY_TYPES ndarraytype> struct arrlen_index_cutoff {};

template <> struct arrlen_index_cutoff<NPY_TYPES::NPY_INT8> {
...
template <> struct arrlen_index_cutoff<NPY_TYPES::NPY_INT16> {
  static constexpr int64_t value = 30000;
};

template <> struct arrlen_index_cutoff<NPY_TYPES::NPY_INT32> {
...
static constexpr NPY_TYPES index_size_type(int64_t array_length) {
  ...
  // excluded here even when the language version checks pass.
    ...
    (defined(_MSVC_LANG) && _MSVC_LANG >= 201402L && _MSC_VER > 1900)
  if (array_length < 0) {
  ...
  return array_length < 0 ? NPY_TYPES::NPY_NOTYPE
         : array_length < arrlen_index_cutoff<NPY_TYPES::NPY_INT8>::value
             ? NPY_TYPES::NPY_INT8
         : array_length < arrlen_index_cutoff<NPY_TYPES::NPY_INT16>::value
             ? NPY_TYPES::NPY_INT16
         : array_length < arrlen_index_cutoff<NPY_TYPES::NPY_INT32>::value
             ? NPY_TYPES::NPY_INT32
             : NPY_TYPES::NPY_INT64;
#endif
}
...
static constexpr int64_t int32_index_cutoff =
    arrlen_index_cutoff<NPY_INT32>::value;
} // namespace riptide
