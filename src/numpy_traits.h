...
{
     /**
      * @brief Template-based, compile-time mapping between C++ types and numpy type codes (e.g. NPY_FLOAT64).
      *
     ...
     {
         // N.B. The 'value' field here should really use ssize_t as the type, but since we've
         // hard-coded int64_t throughout riptide, we need to use that for compatibility (at least for now).
     ...
     {
         // VS2015 says it supports C++14, but doesn't fully -- so it's explicitly excluded here even when the language version checks pass.
#if __cplusplus >= 201402L || (defined(_MSVC_LANG) && _MSVC_LANG >= 201402L && _MSC_VER > 1900)
        if (array_length < 0) { return NPY_TYPES::NPY_NOTYPE; }
        else if (array_length < arrlen_index_cutoff<NPY_TYPES::NPY_INT8>::value) { return NPY_TYPES::NPY_INT8; }
        else if (array_length < arrlen_index_cutoff<NPY_TYPES::NPY_INT16>::value) { return NPY_TYPES::NPY_INT16; }
        else if (array_length < arrlen_index_cutoff<NPY_TYPES::NPY_INT32>::value) { return NPY_TYPES::NPY_INT32; }
        else { return NPY_TYPES::NPY_INT64; }
#else
        // Fall back to the C++11 constexpr style.
        return
           array_length < 0 ? NPY_TYPES::NPY_NOTYPE :
     ...
     // The maximum number of rows (non-inclusive) for which we'll use a 32-bit integer array
     // to hold a fancy index.
