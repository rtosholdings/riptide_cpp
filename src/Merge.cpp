...
int64_t SumBooleanMask(const int8_t *const pData, const int64_t length,
  ...
  if (strideBoolean == 1) {
    ...
    for (size_t i = 0; i < vector_length;) {
      ...
      const auto byte_popcounts_8a =
          _mm256_unpacklo_epi8(byte_popcounts, zeros);
      const auto byte_popcounts_8b =
          _mm256_unpackhi_epi8(byte_popcounts, zeros);
...
int64_t BooleanCount(PyArrayObject *aIndex, int64_t **ppChunkCount,
  ...
  int64_t *const pChunkCount =
      (int64_t *)WORKSPACE_ALLOC(chunks * sizeof(int64_t));
  ...
  if (pChunkCount) {
    ...
    // int64_t length);
    auto lambdaBSCallback = [](void *callbackArgT, int core, int64_t start,
                               int64_t length) -> bool {
      ...
      int64_t total =
          SumBooleanMask(&pBooleanMask[start * strides], length, strides);
    ...
    bool didMtWork = g_cMathWorker->DoMultiThreadedChunkWork(
        lengthBool, lambdaBSCallback, &stBSCallback);
...
PyObject *BooleanIndexInternal(PyArrayObject *aValues, PyArrayObject *aIndex) {
  ...
  if (lengthBool != lengthValue) {
    PyErr_Format(PyExc_ValueError, "Array lengths must match %lld vs %lld",
                 lengthBool, lengthValue);
    return NULL;
  }
  ...
  if (pReturnArray) {
    ...
    if (totalTrue > 0) {
      ...
      //-----------------------------------------------
      // This is the routine that will be called back from multiple threads
      auto lambdaBICallback2 = [](void *callbackArgT, int core, int64_t start,
                                  int64_t length) -> bool {
        BICallbackStruct *callbackArg = (BICallbackStruct *)callbackArgT;

        int8_t *pBooleanMask = callbackArg->pBooleanMask;
        int64_t chunkCount =
            callbackArg->pChunkCount[start / g_cMathWorker->WORK_ITEM_CHUNK];
      ...
      g_cMathWorker->DoMultiThreadedChunkWork(lengthBool, lambdaBICallback2,
                                              &stBICallback);
    }
  } else {
    // ran out of memory
...
PyObject *BooleanIndex(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!:BooleanIndex", &PyArray_Type, &aValues,
                        &PyArray_Type, &aIndex)) {
...
static void GetItemInt(void *aValues, void *aIndex, void *aDataOut,
                       int64_t valLength, int64_t itemSize, int64_t len,
                       int64_t strideIndex, int64_t strideValue,
                       void *pDefault) {
  ...
  LOGGING("getitem sizes %lld  len: %lld   def: %I64d  or  %lf\n", valLength,
          len, (int64_t)defaultVal, (double)defaultVal);
...
// NOTE: The output CANNOT be strided
template <typename VALUE, typename INDEX>
static void GetItemUInt(void *aValues, void *aIndex, void *aDataOut,
                        int64_t valLength, int64_t itemSize, int64_t len,
                        int64_t strideIndex, int64_t strideValue,
                        void *pDefault) {
  const VALUE *pValues = (VALUE *)aValues;
...
static void GetItemIntVariable(void *aValues, void *aIndex, void *aDataOut,
                               int64_t valLength, int64_t itemSize, int64_t len,
                               int64_t strideIndex, int64_t strideValue,
                               void *pDefault) {
...
template <typename INDEX>
static void GetItemUIntVariable(void *aValues, void *aIndex, void *aDataOut,
                                int64_t valLength, int64_t itemSize,
                                int64_t len, int64_t strideIndex,
                                int64_t strideValue, void *pDefault) {
  const char *pValues = (char *)aValues;
  const INDEX *pIndex = (INDEX *)aIndex;
  char *pDataOut = (char *)aDataOut;
...
typedef void (*GETITEM_FUNC)(void *pDataIn, void *pDataIn2, void *pDataOut,
                             int64_t valLength, int64_t itemSize, int64_t len,
                             int64_t strideIndex, int64_t strideValue,
                             void *pDefault);
...
static bool GetItemCallback(struct stMATH_WORKER_ITEM *pstWorkerItem, int core,
                            int64_t workIndex) {

  int64_t didSomeWork = 0;
  MBGET_CALLBACK *Callback =
      &stMBGCallback; // (MBGET_CALLBACK*)&pstWorkerItem->WorkCallbackArg;
  ...
  while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
    ...
    LOGGING("%d : workBlock %lld   blocksize: %lld    lenx: %lld  %lld  %lld  "
            "%lld %lld\n",
            core, workBlock, pstWorkerItem->BlockSize, lenX, typeSizeValues,
            typeSizeIndex, valueAdj, indexAdj);

    Callback->GetItemCallback(aValues, aIndex + indexAdj,
                              (char *)Callback->pDataOut + valueAdj,
                              Callback->aValueLength, valueItemSize, lenX,
                              strideIndex, strideValue, Callback->pDefault);
...
PyObject *MBGet(PyObject *self, PyObject *args) {
  ...
  if (PyTuple_Size(args) == 2) {
    if (!PyArg_ParseTuple(args, "O!O!:getitem", &PyArray_Type, &aValues,
                          &PyArray_Type, &aIndex)) {
  ...
  } else if (!PyArg_ParseTuple(args, "O!O!O:getitem", &PyArray_Type, &aValues,
                               &PyArray_Type, &aIndex, &defaultValue)) {
  ...
  if (numpyIndexType > NPY_LONGDOUBLE) {
    PyErr_Format(
        PyExc_ValueError,
        "Dont know how to convert these types %d using index dtype: %d",
  ...
  if (pFunction != NULL || aIndexLength == 0) {
    ...
    if (outArray) {
      ...
      if (aIndexLength != 0) {
        ...
        if (defaultValue != Py_None) {
          ...
          result = ConvertScalarObject(defaultValue, &tempDefault,
                                       numpyValuesType, &pTempData, &itemSize);
          if (result) {
        ...
        stMATH_WORKER_ITEM *pWorkItem =
            g_cMathWorker->GetWorkItem(aIndexLength);
        ...
        if (pWorkItem == NULL) {
          ...
          pFunction(pValues, pIndex, pDataOut, aValueLength, aValueItemSize,
                    aIndexLength, strideIndex, strideValue, pDefault);
    ...
    PyErr_Format(PyExc_ValueError, "GetItem ran out of memory %d %d",
                 numpyValuesType, numpyIndexType);
    return NULL;
  }

  PyErr_Format(PyExc_ValueError,
               "Dont know how to convert these types %d %d.  itemsize: %lld",
               numpyValuesType, numpyIndexType, aValueItemSize);
  return NULL;
}
...
PyObject *BooleanToFancy(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (returnArray) {
    ...
    // This is the routine that will be called back from multiple threads
    auto lambdaCallback = [](void *callbackArgT, int core, int64_t start,
                             int64_t length) -> bool {
      ...
      if (bothMode) {
        int64_t chunkCountFalse =
            callbackArg
                ->pChunkCountFalse[start / g_cMathWorker->WORK_ITEM_CHUNK];
    ...
    g_cMathWorker->DoMultiThreadedChunkWork(indexLength, lambdaCallback,
                                            &stBTFCallback);
  }
...
PyObject *ReIndexGroups(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!O!O!", &PyArray_Type, &ikey, &PyArray_Type,
                        &uikey, &PyArray_Type, &u_cutoffs, &PyArray_Type,
                        &i_cutoffs)) {
  ...
  case 1:
    g_cMathWorker->DoMultiThreadedWork((int)u_cutoffs_length,
                                       ReIndexGroupsMT<int8_t>, &preindex);
    break;
  case 2:
    g_cMathWorker->DoMultiThreadedWork((int)u_cutoffs_length,
                                       ReIndexGroupsMT<int16_t>, &preindex);
    break;
  case 4:
    g_cMathWorker->DoMultiThreadedWork((int)u_cutoffs_length,
                                       ReIndexGroupsMT<int32_t>, &preindex);
    break;
  case 8:
    g_cMathWorker->DoMultiThreadedWork((int)u_cutoffs_length,
                                       ReIndexGroupsMT<int64_t>, &preindex);
    break;
  default:
...
// Algo: out[in[i]] = i
template <typename KEYTYPE>
bool ReverseShuffleMT(void *preindexV, int core, int64_t start,
                      int64_t length) {
...
PyObject *ReverseShuffle(PyObject *self, PyObject *args) {
  ...
  if (dtype >= 10 || (dtype & 1) == 0) {
    PyErr_Format(PyExc_ValueError,
                 "ReverseShuffle: ikey must be int8/16/32/64");
    return NULL;
  }
  ...
  if (pReturnArray) {
    ...
    case 1:
      g_cMathWorker->DoMultiThreadedChunkWork(
          arrlength, ReverseShuffleMT<int8_t>, &preindex);
      break;
    case 2:
      g_cMathWorker->DoMultiThreadedChunkWork(
          arrlength, ReverseShuffleMT<int16_t>, &preindex);
      break;
    case 4:
      g_cMathWorker->DoMultiThreadedChunkWork(
          arrlength, ReverseShuffleMT<int32_t>, &preindex);
      break;
    case 8:
      g_cMathWorker->DoMultiThreadedChunkWork(
          arrlength, ReverseShuffleMT<int64_t>, &preindex);
      break;
    default:
      PyErr_Format(PyExc_ValueError,
                   "ReverseShuffle: ikey must be int8/16/32/64");
