...
static bool GetItemCallback(struct stMATH_WORKER_ITEM * pstWorkerItem, int core, int64_t workIndex)
{
    ...
    while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0)
    {
        ...
        LOGGING("%d : workBlock %lld   blocksize: %lld    lenx: %lld  %lld  %lld  %lld %lld\n", core, workBlock,
                pstWorkerItem->BlockSize, lenX, typeSizeValues, typeSizeIndex, valueAdj, indexAdj);
        ...
        if (outArray)
        {
            if (aIndexLength != 0)
            {
                ...
                if (pWorkItem == NULL)
                {
                    ...
                    // typedef void(*GETITEM_FUNC)(void* pDataIn, void* pDataIn2, void* pDataOut, int64_t valSize, int64_t
                    // itemSize, int64_t len, int64_t strideIndex, int64_t strideValue, void* pDefault);
                ...
                else
                {
                    ...
                    // printf("**check %p %p %p %lld %lld\n", pValues, pIndex, pDataOut, stMBGCallback.TypeSizeValues,
                    // stMBGCallback.TypeSizeIndex);
