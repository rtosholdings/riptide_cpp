...
PyObject * process_one_input( PyArrayObject const* in_array, PyArrayObject * out_object_1, int32_t function_num, int32_t numpy_intype, int32_t numpy_outtype )
{
   ...
   if ( opt_op_trait && opt_type_trait )
   {
      ...
      {
         numpy_outtype = riptable_cpp::get_active_value_return( *opt_op_trait, std::make_index_sequence< std::variant_size_v< riptable_cpp::operation_t > >{} ) ? numpy_intype : NPY_BOOL;
         ...
         if ( result_array )
         {
            ...
            riptable_cpp::calculate_for_active_data_type( in_p, out_p, len, stride, *opt_op_trait, *opt_type_trait, std::make_index_sequence< std::variant_size_v< riptable_cpp::data_type_t > >{} );
         }
         else
         {
            Py_INCREF( Py_None );
      ...
      {
         int wanted_outtype = riptable_cpp::get_active_value_return( *opt_op_trait, std::make_index_sequence< std::variant_size_v< riptable_cpp::operation_t > >{} ) ? numpy_intype : NPY_BOOL;
