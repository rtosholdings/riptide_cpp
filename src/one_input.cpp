...
namespace internal
{
    ...
    void perform_operation(char const * in_p, char * out_p, npy_intp const len, int64_t const stride, operation_t * op_p,
                           data_t * data_type_p, int64_t const out_stride_as_items = 1)
    {
    ...
    void calculate_for_active_operation(char const * in_p, char * out_p, npy_intp const len, int64_t const stride,
    ...
    void calculate_for_active_data_type(char const * in_p, char * out_p, npy_intp const len, int64_t const stride,
                                        operation_t const & requested_op, type_variant const & in_type, std::index_sequence<Is...>)
    {
        (calculate_for_active_operation(in_p, out_p, len, stride, requested_op, std::get_if<Is>(&in_type),
    ...
    void walk_row_major(char const * in_p, char * out_p, int32_t ndim, PyArrayObject const * in_array, int64_t const stride_out,
    ...
    template <typename operation_trait, typename type_trait>
    void walk_column_major(char const * in_p, char * out_p, int32_t ndim, PyArrayObject const * in_array, int64_t const stride_out,
                           operation_trait const & requested_op, type_trait const & in_type)
    {
    ...
    if (opt_op_trait && opt_type_trait)
    {
        if (direction == 0 && numpy_outtype == -1)
        {
            ...
            if (result_array)
            {
                ...
                internal::calculate_for_active_data_type(in_p, out_p, len, stride, *opt_op_trait, *opt_type_trait,
                                                         std::make_index_sequence<std::variant_size_v<internal::data_type_t>>{});
            }
            else
            {
