...
PyObject *process_one_input(PyArrayObject const *in_array,
                            PyArrayObject *out_object_1, int32_t function_num,
                            int32_t numpy_intype, int32_t numpy_outtype) {
  int32_t ndim{};
  int64_t stride{};

  int32_t direction{GetStridesAndContig(in_array, ndim, stride)};
  npy_intp len{CALC_ARRAY_LENGTH(
      ndim, PyArray_DIMS(const_cast<PyArrayObject *>(in_array)))};

  auto [opt_op_trait, opt_type_trait] =
      riptable_cpp::set_traits(function_num, numpy_intype);
  ...
  if (opt_op_trait && opt_type_trait) {
    if (direction == 0 && numpy_outtype == -1) {
      numpy_outtype = riptable_cpp::get_active_value_return(
                          *opt_op_trait,
                          std::make_index_sequence<
                              std::variant_size_v<riptable_cpp::operation_t>>{})
                          ? numpy_intype
                          : NPY_BOOL;
      PyArrayObject *result_array{
          (ndim <= 1) ? AllocateNumpyArray(1, &len, numpy_outtype)
      ...
      if (result_array) {
        ...
        riptable_cpp::walk_data_array(1, len, stride, stride, in_p, out_p,
                                      *opt_op_trait, *opt_type_trait);
      } else {
        Py_INCREF(Py_None);
        return Py_None;
      }
      ...
      PyArrayObject *result_array{
          numpy_outtype == -1 ? AllocateLikeNumpyArray(in_array, wanted_outtype)
                              : out_object_1};

      if ((result_array == nullptr) || ((result_array == out_object_1) &&
                                        (len != ArrayLength(result_array)))) {
        Py_INCREF(Py_None);
        return Py_None;
      }
      ...
      int64_t stride_out{};
      int direction_out =
          GetStridesAndContig(result_array, num_dims_out, stride_out);

      if (direction_out == 0) {
        switch (direction) {
        case 0:
          riptable_cpp::walk_data_array(ndim, len, stride, stride_out, in_p,
                                        out_p, *opt_op_trait, *opt_type_trait);
          break;
        case 1:
          riptable_cpp::walk_row_major(in_p, out_p, ndim, in_array, stride_out,
                                       *opt_op_trait, *opt_type_trait);
          break;
        case -1:
          riptable_cpp::walk_column_major(in_p, out_p, ndim, in_array,
                                          stride_out, *opt_op_trait,
                                          *opt_type_trait);
          break;
        }
      }
...
chosen_traits_t set_traits(int32_t const function_num,
                           int32_t const numpy_intype) {
