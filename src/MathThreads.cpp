...
//   * libdispatch (GCD), using dispatch_semaphore_t (via dispatch_semaphore_create()) to control concurrency; include
//   <dispatch/semaphore.h>
//   * BSD syscalls like __psynch_cvwait (and other __psynch functions). These are not externally documented -- need to look in
        ...
        if ( ! didSomeWork )
        {
            ...
            else
            {
                ...
                g_WaitAddress(&pWorkerRing->WorkIndex, (PVOID)&workIndexCompleted,
                              8, // The size of the value being waited on (i.e. the number of bytes to read from the two pointers
                                 // then compare).
                              1000000L);
            }
