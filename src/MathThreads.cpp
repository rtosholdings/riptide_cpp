  ...
  while (pWorkerRing->Cancelled == 0) {
    ...
    if (!didSomeWork) {
      ...
      if (g_WaitAddress == NULL) {
        ...
        LOGGING("[%d] WaitAddress %llu  %llu  %d\n", core, workIndexCompleted,
                pWorkerRing->WorkIndex, (int)didSomeWork);

        // Otherwise wake up using conditional variable
        g_WaitAddress(
            &pWorkerRing->WorkIndex, (PVOID)&workIndexCompleted,
      ...
      //   int *uaddr2, int val3);
      futex((int *)&pWorkerRing->WorkIndex, FUTEX_WAIT, (int)workIndexCompleted,
            NULL, NULL, 0);
