...
extern "C"
{
    ...
    // HANDLE WINAPI CreateThread(VOID* lpThreadAttributes, SIZE_T dwStackSize, LPTHREAD_START_ROUTINE lpStartAddress, LPVOID
    // lpParameter, DWORD dwCreationFlags, LPDWORD lpThreadId); HMODULE WINAPI LoadLibraryW(const WCHAR* lpLibFileName); FARPROC
    // WINAPI GetProcAddress(HMODULE hModule, const char* lpProcName);
...
public:
    ...
    ////------------------------------------------------------------------------------
    ////
    // void WorkOneStubCall(FUNCTION_LIST* anyOneStubCall, void* pDataIn, void* pDataOut, int64_t len, int64_t strideIn, int64_t
    // strideOut) {

    //   // If it is a small work item, process it immediately
    //   if (len < WORK_ITEM_BIG || NoThreading) {
    //      anyOneStubCall->AnyOneStubCall(pDataIn, pDataOut, len, strideIn, strideOut);
    //      return;
    //   }
