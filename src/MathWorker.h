...
bool GetProcessAffinityMask(HANDLE hProcess, uint64_t *lpProcessAffinityMask,
...
public:
  ...
  int32_t GetFutexWakeup() { return pWorkerRing->FutexWakeCount; }
  ...
  static bool
  MultiThreadedCounterCallback(struct stMATH_WORKER_ITEM *pstWorkerItem,
    ...
    while ((index = pstWorkerItem->GetNextWorkIndex(&workBlock)) > 0) {
      ...
      pstWorkerItem->MTWorkCallback(pstWorkerItem->WorkCallbackArg, core,
                                    index);
  ...
  void DoMultiThreadedWork(int numItems, MTWORK_CALLBACK doMTWorkCallback,
                           void *workCallbackArg, int32_t threadWakeup = 0) {
  ...
  static bool
  MultiThreadedChunkCallback(struct stMATH_WORKER_ITEM *pstWorkerItem, int core,
                             int64_t workIndex) {
    // -1 is the first core
    core = core + 1;
    bool didSomeWork = false;
    ...
    while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
      ...
      pstWorkerItem->MTChunkCallback(pstWorkerItem->WorkCallbackArg, core,
                                     start, lenX);
  ...
  bool DoMultiThreadedChunkWork(int64_t lengthData,
                                MTCHUNK_CALLBACK doMTChunkCallback,
  ...
  ///
  int64_t SegmentBins(int64_t bins, int64_t maxCores,
                      stBinCount **ppstBinCount) {
    ...
    stBinCount *pstBinCount =
        (stBinCount *)WORKSPACE_ALLOC(cores * sizeof(stBinCount));
  ...
  void WorkMain(stMATH_WORKER_ITEM *pWorkItem, int64_t len,
                int32_t threadWakeup, int64_t BlockSize = WORK_ITEM_CHUNK,
  ...
  static bool AnyScatterGather(struct stMATH_WORKER_ITEM *pstWorkerItem,
                               int core, int64_t workIndex) {
    bool didSomeWork = false;
    ...
    stScatterGatherFunc *pstScatterGatherFunc =
        &((stScatterGatherFunc *)(OldCallback->pThreadWorkSpace))[core + 1];
    ...
    while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
      ...
      OldCallback->FunctionList->AnyScatterGatherCall(
          pDataInX + offsetAdj, lenX, pstScatterGatherFunc);
  ...
  static bool AnyGroupby(struct stMATH_WORKER_ITEM *pstWorkerItem, int core,
                         int64_t workIndex) {
    bool didSomeWork = false;
  ...
  static bool AnyTwoCallback(struct stMATH_WORKER_ITEM *pstWorkerItem, int core,
                             int64_t workIndex) {
    bool didSomeWork = false;
    OLD_CALLBACK *OldCallback = &pstWorkerItem->OldCallback;
    ...
    while ((lenX = pstWorkerItem->GetNextWorkBlock(&workBlock)) > 0) {
      ...
      case ANY_TWO: {
        ...
        case NO_SCALARS:
          ...
          OldCallback->FunctionList->AnyTwoStubCall(
              pDataInX + offsetAdj, pDataInX2 + offsetAdj,
        ...
        case FIRST_ARG_SCALAR:
          // Process this block of work
          OldCallback->FunctionList->AnyTwoStubCall(
              pDataInX, pDataInX2 + offsetAdj, pDataOutX + outputAdj, lenX,
        ...
        case SECOND_ARG_SCALAR:
          // Process this block of work
          OldCallback->FunctionList->AnyTwoStubCall(
              pDataInX + offsetAdj, pDataInX2, pDataOutX + outputAdj, lenX,
              OldCallback->ScalarMode);
          break;
      ...
      case ANY_ONE:
        // Process this block of work
        OldCallback->FunctionList->AnyOneStubCall(pDataInX + offsetAdj,
                                                  pDataOutX + outputAdj, lenX,
                                                  strideSizeIn, strideSizeOut);
        break;
      default:
  ...
  //------------------------------------------------------------------------------
  //
  void WorkGroupByCall(GROUPBY_FUNC groupByCall, void *pstData,
                       int64_t tupleSize) {
    // If it is a small work item, process it immediately
  ...
  void WorkScatterGatherCall(FUNCTION_LIST *anyScatterGatherCall, void *pDataIn,
                             int64_t len, int64_t func,
                             stScatterGatherFunc *pstScatterGatherFunc) {

    // If it is a small work item, process it immediately
    if (len < WORK_ITEM_BIG || NoThreading) {
      anyScatterGatherCall->AnyScatterGatherCall(pDataIn, len,
    ...
    if (pWorkSpace) {
      ...
      if (func == REDUCE_MIN || func == REDUCE_NANMIN || func == REDUCE_MAX ||
          func == REDUCE_NANMAX) {
        int32_t calcs = 0;
          ...
          if (pZeroArray[i].lenOut) {
            if (calcs == 0) {
              ...
              pstScatterGatherFunc->resultOutInt64 =
                  pZeroArray[i].resultOutInt64;
            } else {
              if (func == REDUCE_MIN || func == REDUCE_NANMIN) {
                pstScatterGatherFunc->resultOut = MINF(
                    pstScatterGatherFunc->resultOut, pZeroArray[i].resultOut);
                pstScatterGatherFunc->resultOutInt64 =
                    MINF(pstScatterGatherFunc->resultOutInt64,
                         pZeroArray[i].resultOutInt64);
              } else {

                pstScatterGatherFunc->resultOut = MAXF(
                    pstScatterGatherFunc->resultOut, pZeroArray[i].resultOut);
                pstScatterGatherFunc->resultOutInt64 =
                    MAXF(pstScatterGatherFunc->resultOutInt64,
                         pZeroArray[i].resultOutInt64);
              }
            }
          }
