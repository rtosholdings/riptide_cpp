...
static const uint64_t **slicing8TableECMA =
    makeSlicingBy8Table(makeTable(ECMA));

// Call with crc=0 to init or previous crc to chain
// et tab to slicing8TableECMA
static uint64_t CalculateCRC64(uint64_t crc, const uint64_t *tab, uint8_t *p,
                               int64_t Length) {
  crc = ~crc;
  ...
  while (Length >= 64) {
    ...
    while (Length >= 8) {
      ...
      crc ^= ((uint64_t)p[0]) | ((uint64_t)p[1]) << 8 | ((uint64_t)p[2]) << 16 |
             ((uint64_t)p[3]) << 24 | ((uint64_t)p[4]) << 32 |
             ((uint64_t)p[5]) << 40 | ((uint64_t)p[6]) << 48 |
             ((uint64_t)p[7]) << 56;
      crc = helperTable[7][crc & 0xff] ^ helperTable[6][(crc >> 8) & 0xff] ^
...
PyObject *CalculateCRC(PyObject *self, PyObject *args) {
  ...
  uint64_t crc_value =
      CalculateCRC64(0, slicing8TableECMA[0], (uint8_t *)pDataIn1, arraySize);
