...
static PyObject *ConvertInt32(PyObject *object) {
  ...
  for (int64_t i = 0; i < size; i++) {
    int overflow = 0;
    int64_t val =
        PyLong_AsLongLongAndOverflow(PyList_GET_ITEM(object, i), &overflow);
...
static PyObject *ConvertUnicode(PyObject *object) {
  ...
  PyArrayObject *pArray =
      AllocateNumpyArray(1, &size, NPY_UNICODE, maxsize * 4);
  ...
  if (kwargs && Py_SIZE(kwargs) > 0) {
    ...
    if (copykwarg) {
      if (PyBool_Check(copykwarg)) {
        ...
        PyErr_Format(PyExc_ValueError,
                     "The 'copy' argument must be either True or False.");
      }
    }
    ...
    if (orderkwarg) {
      if (PyUnicode_Check(orderkwarg)) {
        ...
        PyErr_Format(PyExc_ValueError,
                     "The 'order' argument must be either 'F' or 'C'.");
      }
    }
  }
  return flags;
...
PyObject *AsAnyArray(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (Py_SIZE(args) != 1) {
    ...
    if (Py_SIZE(args) != 2) {
      PyErr_Format(
          PyExc_TypeError,
          "AsArray takes 1 or 2 positional arguments but %lld were given",
          Py_SIZE(args));
      return NULL;
    } else {
    ...
    if (dtypekwarg) {
      if (descr) {
        PyErr_Format(PyExc_TypeError,
                     "AsArray got multiple values for 'dtype'");
        return NULL;
      } else {
  ...
  PyArrayObject *pArray =
      (PyArrayObject *)PyArray_FromAny(object, descr, 0, 0, flags, NULL);
  ...
  npy_intp size = 1;
  PyArrayObject *pNewArray = AllocateNumpyArray(1, &size, PyArray_TYPE(pArray),
                                                PyArray_ITEMSIZE(pArray));

  // copy over the data
  memcpy(PyArray_DATA(pNewArray), PyArray_DATA(pArray),
         PyArray_ITEMSIZE(pArray));
