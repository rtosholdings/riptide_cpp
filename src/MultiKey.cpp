...
char *RotateArrays(int64_t tupleSize, ArrayInfo *aInfo) {
  ...
  for (int64_t i = 0; i < tupleSize; i++) {
    if (aInfo[i].ArrayLength != totalRows) {
      PyErr_Format(PyExc_ValueError,
                   "MultiKeyHash all arrays must be same number of rows %llu",
                   totalRows);
      return NULL;
    }
  ...
  if (pSuperArray) {
    ...
    PyErr_Format(PyExc_ValueError, "MultiKeyHash out of memory    %llu",
                 totalRows);
  }
...
ArrayInfo *AllocArrayInfo(int64_t tupleSize) {
  int64_t *pRawAlloc = (int64_t *)WORKSPACE_ALLOC(
      (sizeof(ArrayInfo) * tupleSize) + sizeof(int64_t));
  if (pRawAlloc) {
...
ArrayInfo *BuildArrayInfo(PyObject *listObject, int64_t *pTupleSize,
                          int64_t *pTotalItemSize, bool checkrows,
                          bool convert) {
  ...
  if (PyArray_Check(listObject)) {
    ...
    PyErr_Format(PyExc_ValueError,
                 "BuildArrayInfo must pass in a list or tuple");
    return NULL;
  }
  ...
  for (int64_t i = 0; i < tupleSize; i++) {
    ...
    if (inObject == Py_None) {
      ...
      if (aInfo[i].NumpyDType == -1) {
        PyErr_Format(PyExc_ValueError,
                     "BuildArrayInfo array has bad dtype of %d",
      ...
      if (checkrows) {
        if (aInfo[i].NDim != 1) {
          PyErr_Format(PyExc_ValueError,
                       "BuildArrayInfo array must have ndim ==1 instead of %d",
                       aInfo[i].NDim);
          goto EXIT_ROUTINE;
        }
        if (itemSize != stride0) {
          PyErr_Format(
              PyExc_ValueError,
              "BuildArrayInfo array strides must match itemsize -- %lld %lld",
              itemSize, stride0);
          goto EXIT_ROUTINE;
        }
      } else {
      ...
      PyErr_Format(PyExc_ValueError,
                   "BuildArrayInfo only accepts numpy arrays");
      goto EXIT_ROUTINE;
    }
  }
  ...
  if (checkrows && tupleSize > 0) {
    ...
    for (int64_t i = 0; i < tupleSize; i++) {
      if (aInfo[i].ArrayLength != totalRows) {
        PyErr_Format(
            PyExc_ValueError,
            "BuildArrayInfo all arrays must be same number of rows %llu",
            totalRows);
        goto EXIT_ROUTINE;
      }
    }
  ...
  CMultiKeyPrepare(PyObject *args) {
    ...
    if (aInfo) {
      ...
      if (pBoolFilterObject) {
        if (PyArray_TYPE(pBoolFilterObject) != NPY_BOOL ||
            ArrayLength(pBoolFilterObject) != totalRows) {
          PyErr_Format(PyExc_ValueError,
...
void MakeNextKey(int64_t mode, int64_t numUnique, int64_t totalRows,
                 void *pIndexArrayK, void *pNextArrayK) {
...
PyObject *MakeiNext(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!LL", &PyArray_Type, &ikey, &unique_rows,
                        &mode)) {
    return NULL;
  }
...
template <typename K>
int32_t *GroupByPackFixup32(int64_t numUnique, int64_t totalRows,
                            void *pIndexArrayK, int32_t *pGroupArray) {
...
template <typename K>
bool GroupByPackFinal32(int64_t numUnique, int64_t totalRows,
                        void *pIndexArrayK, int32_t *pNextArray,
                        int32_t *pGroupArray, PyObject **ppSortArray,
                        PyObject **ppFirstArray, PyObject **ppCountArray)

{
  ...
  PyArrayObject *sortArray =
      AllocateNumpyArray(1, (npy_intp *)&totalRows, NPY_INT32);
  PyArrayObject *firstArray =
      AllocateNumpyArray(1, (npy_intp *)&numUnique, NPY_INT32);
  PyArrayObject *countArray =
      AllocateNumpyArray(1, (npy_intp *)&numUnique, NPY_INT32);
...
PyObject *GroupByPack32(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!OL", &PyArray_Type, &indexArray, &nextArray,
                        &numUnique)) {
  ...
  try {
    ...
    if (!PyArray_Check(nextArray)) {
      ...
      case NPY_INT8:
        pNextArray = GroupByPackFixup32<int8_t>(numUnique, totalRows,
                                                pIndexArray, pGroupArray);
        break;
      case NPY_INT16:
        pNextArray = GroupByPackFixup32<int16_t>(numUnique, totalRows,
                                                 pIndexArray, pGroupArray);
        break;
      CASE_NPY_INT32:
        pNextArray = GroupByPackFixup32<int32_t>(numUnique, totalRows,
                                                 pIndexArray, pGroupArray);
        break;
      CASE_NPY_INT64:

        pNextArray = GroupByPackFixup32<int64_t>(numUnique, totalRows,
                                                 pIndexArray, pGroupArray);
        break;
      default:
        PyErr_Format(PyExc_ValueError,
                     "GroupByPack32 index must be int8 int16, int32, int64");
        return NULL;
      }
    } else {
      if (totalRows != ArrayLength((PyArrayObject *)nextArray)) {
        PyErr_Format(PyExc_ValueError,
                     "GroupByPack32 array length does not match %llu",
                     totalRows);
        return NULL;
      }
    ...
    switch (numpyIndexType) {
    case NPY_INT8:
      bResult = GroupByPackFinal32<int8_t>(
          numUnique, totalRows, pIndexArray, pNextArray, pGroupArray,
          &sortGroupArray, &firstArray, &countArray);
      break;
    case NPY_INT16:
      bResult = GroupByPackFinal32<int16_t>(
          numUnique, totalRows, pIndexArray, pNextArray, pGroupArray,
          &sortGroupArray, &firstArray, &countArray);
      break;
    CASE_NPY_INT32:
      bResult = GroupByPackFinal32<int32_t>(
          numUnique, totalRows, pIndexArray, pNextArray, pGroupArray,
          &sortGroupArray, &firstArray, &countArray);
      break;
    CASE_NPY_INT64:

      bResult = GroupByPackFinal32<int64_t>(
          numUnique, totalRows, pIndexArray, pNextArray, pGroupArray,
          &sortGroupArray, &firstArray, &countArray);
      break;
    default:
    ...
    if (bResult) {
      ...
      PyObject *retObject =
          Py_BuildValue("(OOO)", sortGroupArray, firstArray, countArray);
...
PyObject *MultiKeyGroupBy32(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (!PyTuple_Check(args)) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyGroupBy32 arguments needs to be a tuple");
    return NULL;
  }
  ...
  if (tupleSize < 3) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyGroupBy32 only has %llu args but requires 3",
                 tupleSize);
    return NULL;
  }
  ...
  try {
    ...
    if (cutOffLength == -1) {
      PyErr_Format(
          PyExc_ValueError,
          "MultiKeyGroupBy32 'cutoffs' must be an array of type int64_t");
      return NULL;
    }
    ...
    if (mkp.pSuperArray && mkp.listSize > 0) {
      ...
      PyArrayObject *indexArray =
          AllocateLikeNumpyArray(mkp.aInfo[0].pObject, numpyIndexType);

      if (indexArray == NULL) {
        PyErr_Format(PyExc_ValueError,
                     "MultiKeyGroupBy32 out of memory    %llu", mkp.totalRows);
        return NULL;
      } else {
        ...
        LOGGING("Starting mkgp32  indxtype: %d  rows: %lld   itemsize: %lld  "
                "hintsize: %lld  filter: %p  coreType:%d\n",
                numpyIndexType, mkp.totalRows, mkp.totalItemSize, mkp.hintSize,
                mkp.pBoolFilter, coreType);
        ...
        numUnique = (int64_t)pGroupByCall(
            cutOffLength, pCutOffs, mkp.totalRows, mkp.totalItemSize,
            (const char *)mkp.pSuperArray,
            coreType, // set to -1 for unknown
            pIndexArray, &firstArray, HASH_MODE(hashMode), mkp.hintSize,
            mkp.pBoolFilter);

        PyObject *retObject =
            Py_BuildValue("(OOL)", indexArray, firstArray, numUnique);
  ...
  PyErr_Format(PyExc_ValueError,
               "MultiKeyGroupBy32 failed in multikey prepare:  %llu",
               tupleSize);
  return NULL;
}
...
PyObject *MultiKeyGroupBy32Super(PyObject *self, PyObject *args) {
  ...
  if (!PyTuple_Check(args)) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyHash arguments needs to be a tuple");
    return NULL;
  }
  ...
  if (tupleSize < 3) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyHash only has %llu args but requires 3", tupleSize);
    return NULL;
  }
  ...
  try {
    ...
    if (mkp.totalRows > 2100000000) {
      PyErr_Format(PyExc_ValueError, "MultiKeyHash exceeding 32bit limits %llu",
                   mkp.totalRows);
    }
    ...
    if (mkp.pSuperArray && mkp.listSize > 0) {
      ...
      if (nextArray == NULL) {
        ...
        // now one based, so zero reserved
        int32_t *pUniqueArray = (int32_t *)WORKSPACE_ALLOC(
            (mkp.aInfo[0].ArrayLength + 1) * sizeof(int32_t));
        int32_t *pUniqueCountArray = (int32_t *)WORKSPACE_ALLOC(
            (mkp.aInfo[0].ArrayLength + 1) * sizeof(int32_t));
        ...
        LOGGING("Starting hash  indxtype: %d  rows: %lld   itemsize: %lld  "
                "hintsize: %lld  filter: %p  coreType:%d\n",
                numpyIndexType, mkp.totalRows, mkp.totalItemSize, mkp.hintSize,
                mkp.pBoolFilter, coreType);
        ...
        Py_BEGIN_ALLOW_THREADS
            ...
            numUnique =
                (int64_t)GroupBy32Super(mkp.totalRows, mkp.totalItemSize,
                                        (const char *)mkp.pSuperArray, coreType,
                                        pIndexArray, pNextArray, pUniqueArray,
                                        pUniqueCountArray, HASH_MODE(hashMode),
                                        mkp.hintSize, mkp.pBoolFilter);
        ...
        Py_END_ALLOW_THREADS
            ...
            // array
                PyArrayObject *uniqueArray = AllocateNumpyArray(
                    1, (npy_intp *)&numUnique, numpyIndexType);
        PyArrayObject *uniqueCountArray =
            AllocateNumpyArray(1, (npy_intp *)&numUnique, numpyIndexType);
        ...
        if (uniqueArray != NULL && uniqueCountArray != NULL) {
          ...
          int32_t *pUniqueCountArrayDest =
              (int32_t *)PyArray_BYTES(uniqueCountArray);
          memcpy(pUniqueCountArrayDest, pUniqueCountArray,
                 numUnique * sizeof(int32_t));
        }
        ...
        // Build tuple to return
        PyObject *retObject = Py_BuildValue("(OOOO)", indexArray, nextArray,
                                            uniqueArray, uniqueCountArray);
        Py_DECREF((PyObject *)indexArray);
  ...
  PyErr_Format(PyExc_ValueError,
               "MultiKeySuper failed in multikey prepare:  %llu", tupleSize);
  return NULL;
}
...
PyObject *MultiKeyUnique32(PyObject *self, PyObject *args) {
  ...
  if (!PyTuple_Check(args)) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyUnique32 arguments needs to be a tuple");
    return NULL;
  }
  ...
  if (tupleSize < 1) {
    PyErr_Format(PyExc_ValueError, "MultiKeyUnique32 only has %llu args",
                 tupleSize);
    return NULL;
  }
  ...
  try {
    ...
    if (mkp.pSuperArray) {
      ...
      if (mkp.totalRows < 2100000000) {
        // worst case alloc
        int32_t *pIndexArray =
            (int32_t *)WORKSPACE_ALLOC(mkp.totalRows * sizeof(int32_t));
        ...
        if (pIndexArray == NULL || pCountArray == NULL) {
          PyErr_Format(PyExc_ValueError,
                       "MultiKeyUnique32 out of memory    %llu", mkp.totalRows);
          return NULL;
        } else {
          // printf("Size array1: %llu   array2: %llu\n", arraySize1,
          // arraySize2);
        ...
        // worst case alloc
        int64_t *pIndexArray =
            (int64_t *)WORKSPACE_ALLOC(mkp.totalRows * sizeof(int64_t));
        ...
        if (pIndexArray == NULL || pCountArray == NULL) {
          PyErr_Format(PyExc_ValueError,
                       "MultiKeyUnique64 out of memory    %llu", mkp.totalRows);
          return NULL;
        } else {
          // printf("Size array1: %llu   array2: %llu\n", arraySize1,
          // arraySize2);
          ...
          int64_t numUnique = (int64_t)Unique64(
              mkp.totalRows, mkp.totalItemSize, (const char *)mkp.pSuperArray,
              pIndexArray, pCountArray, HASH_MODE::HASH_MODE_MASK, mkp.hintSize,
              mkp.pBoolFilter);
          ...
          // We allocated for worst case, now copy over only the unique indexes
          PyArrayObject *indexArray2 =
              AllocateNumpyArray(1, (npy_intp *)&numUnique, NPY_INT64);
          CHECK_MEMORY_ERROR(indexArray2);
...
PyObject *MultiKeyRolling(PyObject *self, PyObject *args) {
  ...
  if (tupleSize != 2 && tupleSize != 3) {
    PyErr_Format(
        PyExc_ValueError,
        "MultiKeyRolling only %llu args, but requires exactly 2 or 3 args.",
        tupleSize);
    return NULL;
  }
  ...
  if (PyLong_Check(secondArg)) {
    ...
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyRolling second arg must be an integer.");
    return NULL;
  }
  if (tupleSize == 3) {
  ...
  if (mkp.pSuperArray) {
    ...
    PyArrayObject *runningCountArray =
        AllocateLikeNumpyArray(firstObject, NPY_INT64);
    ...
    void *pKeepRolling = MultiKeyRollingStep2(
        mkp.totalRows, mkp.totalItemSize, (const char *)mkp.pSuperArray,
        pIndexArray, pRunningCountArray, HASH_MODE::HASH_MODE_MASK, hintSize,
        &numUnique, (void *)memPointer);

    g_cMathWorker->NoCaching = prevValue;

    // Build tuple to return
    PyObject *retObject =
        Py_BuildValue("(OOLL)", indexArray, runningCountArray,
                      (int64_t)numUnique, (int64_t)pKeepRolling);
...
PyObject *MultiKeyHash(PyObject *self, PyObject *args) {
  ...
  if (mkp.pSuperArray) {
    ...
    PyArrayObject *runningCountArray =
        AllocateLikeNumpyArray(firstObject, NPY_INT32);
    ...
    if (lastArray == NULL) {
      ...
      MultiKeyHash32(mkp.totalRows, mkp.totalItemSize,
                     (const char *)mkp.pSuperArray, pIndexArray,
                     pRunningCountArray, pPrevArray, pNextArray, pFirstArray,
                     HASH_MODE::HASH_MODE_MASK, mkp.hintSize, mkp.pBoolFilter);
    ...
    // Build tuple to return
    PyObject *retObject =
        Py_BuildValue("(OOOOOOO)", indexArray, runningCountArray, bktSizeArray,
                      prevArray, nextArray, firstArray, lastArray);
...
PyObject *MultiKeyIsMember32(PyObject *self, PyObject *args) {
  ...
  if (tupleSize < 2) {
    PyErr_Format(PyExc_ValueError, "MultiKeyIsMember32 only %llu args",
                 tupleSize);
    return NULL;
  }
  ...
  if (!PyTuple_CheckExact(tupleObject1) || !PyTuple_CheckExact(tupleObject2)) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyIsMember32 first two args must be tuple");
    return NULL;
  }
  ...
  try {
    ...
    if (mkp1.totalRows == 0) {
      PyErr_Format(
          PyExc_ValueError,
          "MultiKeyIsMember32 first argument --  array lengths do not match");
      return NULL;
    }
    ...
    if (mkp2.totalRows == 0) {
      PyErr_Format(
          PyExc_ValueError,
          "MultiKeyIsMember32 second argument --  array lengths do not match");
      return NULL;
    }
    ...
    if (mkp1.totalItemSize != mkp2.totalItemSize) {
      PyErr_Format(
          PyExc_ValueError,
          "MultiKeyIsMember32 total itemsize is not equal %lld vs %lld",
          mkp1.totalItemSize, mkp2.totalItemSize);
      return NULL;
    }

    if (mkp1.pSuperArray && mkp2.pSuperArray) {

      PyArrayObject *boolArray =
          AllocateLikeNumpyArray(mkp1.aInfo[0].pObject, NPY_BOOL);
      ...
      if (boolArray) {
        ...
        IsMemberHashMKPre(&indexArray, mkp1.totalRows, mkp1.pSuperArray,
                          mkp2.totalRows, mkp2.pSuperArray, pDataOut1,
...
PyObject *MultiKeyAlign32(PyObject *self, PyObject *args) {
  if (!PyTuple_Check(args)) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyAlign32 arguments needs to be a tuple");
    return NULL;
  }
  ...
  if (!PyTuple_Check(tupleObject1) || !PyTuple_Check(tupleObject2)) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyAlign32 first two args must be tuples");
    return NULL;
  }
  ...
  if (PyArray_Check(valObject1) && PyArray_Check(valObject2)) {
    ...
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyAlign32 second two args must be arrays");
    return NULL;
  }
  ...
  if (mkp1.totalItemSize != mkp2.totalItemSize) {
    PyErr_Format(PyExc_ValueError,
                 "MultiKeyAlign32 keys are not the same itemsize");
    return NULL;
  }
  ...
  if (dtype1 < 0) {
    PyErr_Format(
        PyExc_ValueError,
        "MultiKeyAlign32 data types are not understood dtype.num: %d vs %d",
        dtype1, dtype2);
    return NULL;
  }
  ...
  if (dtype1 != dtype2) {
    ...
    if (!((dtype1 <= NPY_ULONGLONG && dtype2 <= NPY_ULONGLONG) &&
      ...
      PyErr_Format(
          PyExc_ValueError,
          "MultiKeyAlign32 data types are not the same dtype.num: %d vs %d",
          dtype1, dtype2);
      return NULL;
    }
  }
  ...
  if (ArrayLength(pvalArray1) != mkp1.totalRows) {
    PyErr_Format(
        PyExc_ValueError,
        "MultiKeyAlign32 val1 length does not match key input length of %lld",
        mkp1.totalRows);
    return NULL;
  }
  if (ArrayLength(pvalArray2) != mkp2.totalRows) {
    PyErr_Format(
        PyExc_ValueError,
        "MultiKeyAlign32 val2 length does not match key input length of %lld",
        mkp2.totalRows);
    return NULL;
  }
  ...
  if (mkp1.pSuperArray && mkp2.pSuperArray) {
    ...
    try {
      if (isIndex32) {
        ...
        success = AlignHashMK32(mkp1.totalRows, mkp1.pSuperArray, pVal1,
                                mkp2.totalRows, mkp2.pSuperArray, pVal2,
                                pDataOut2, mkp1.totalItemSize, HASH_MODE_MASK,
                                dtype1, isForward, allowExact);
      } else {
        indexArray = AllocateLikeNumpyArray(mkp1.aInfo[0].pObject, NPY_INT64);
        if (!indexArray)
          return PyErr_Format(PyExc_BufferError, "MultiKeyAlign32");
        int64_t *pDataOut2 = (int64_t *)PyArray_BYTES(indexArray);
        success = AlignHashMK64(mkp1.totalRows, mkp1.pSuperArray, pVal1,
                                mkp2.totalRows, mkp2.pSuperArray, pVal2,
                                pDataOut2, mkp1.totalItemSize, HASH_MODE_MASK,
                                dtype1, isForward, allowExact);
      }
      ...
      if (!success) {
        PyErr_Format(PyExc_ValueError, "MultiKeyAlign failed.  Only accepts "
...
//==================================================================
//------------------------------------------------------------------
typedef void (*MAKE_I_GROUP2)(void *piKeyT, void *piFirstGroupT, void *piGroupT,
                              int64_t totalRows, int64_t offset,
...
void MakeiGroup2(void *piKeyT, void *piFirstGroupT, void *piGroupT,
                 int64_t totalRows, int64_t offset, int64_t unique_count) {
...
//==================================================================
//------------------------------------------------------------------
typedef void (*MAKE_I_GROUP)(void *piKeyT, void *pnCountGroupT,
                             void *piFirstGroupT, void *piGroupT,
                             int64_t totalRows, int64_t binLow,
                             int64_t binHigh);
...
// piFirstGroup
template <typename KEYTYPE, typename OUTDTYPE>
void MakeiGroup(void *piKeyT, void *pnCountGroupT, void *piFirstGroupT,
                void *piGroupT, int64_t totalRows, int64_t binLow,
                int64_t binHigh) {
...
PyObject *GroupFromBinCount(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "O!O!", &PyArray_Type, &iKey, &PyArray_Type,
                        &nCountGroup)) {
  ...
  if (outdtype != PyArray_TYPE(nCountGroup)) {
    ...
    PyErr_Format(
        PyExc_ValueError,
        "GroupFromBinCount: nCountGroup dtype does not match expected dtype.");
    return NULL;
  }
  ...
  if (outiGroup && outiFirstGroup) {
    ...
    if (!g_cMathWorker->NoThreading && totalRows > 32000) {
      ...
      auto lambdaBinCountCallback = [](void *callbackArgT, int core,
        ...
        callbackArg->pMakeIGroup(callbackArg->piKey, callbackArg->pnCountGroup,
                                 callbackArg->piFirstGroup,
                                 callbackArg->piGroup, callbackArg->totalRows,
                                 callbackArg->pstBinCount[t].BinLow,
                                 callbackArg->pstBinCount[t].BinHigh);
      ...
      g_cMathWorker->DoMultiThreadedWork((int)cores, lambdaBinCountCallback,
                                         &stMaster);
      WORKSPACE_FREE(pstBinCount);
    } else {

      pMakeIGroup(piKey, pnCountGroup, piFirstGroup, piGroup, totalRows, 0,
                  totalUnique);
    }
...
typedef void (*BIN_COUNT)(void *pKeysT, void *pOutCount, int64_t startRow,
                          int64_t totalRows, int64_t highbin);
...
int64_t InternalBinCount(BIN_COUNT pBinFunc,
                         ...
                         void *piKey, int64_t unique_rows, int64_t totalRows,
                         int64_t coresRequested, int64_t itemSize) {
...
PyObject *BinCount(PyObject *self, PyObject *args, PyObject *kwargs) {
  ...
  if (unique_rows == 0 || totalRows <= 0) {
    PyErr_Format(
        PyExc_ValueError,
        "BinCount: unique or totalRows is zero, cannot calculate the bins.");
    return NULL;
  }
  ...
  if (outArray) {
    ...
    // multithreading
    //
    if (pack == 1 || (!g_cMathWorker->NoThreading && totalRows > 32000 &&
                      (totalRows / unique_rows) > 50)) {
      stBinCount *pstBinCount = NULL;
      char *pMemoryForBins = NULL;

      // Multithread count the bins and allocate
      int64_t cores =
          InternalBinCount(pBinFunc, &pstBinCount, &pMemoryForBins, piKey,
                           unique_rows, totalRows, 31, itemSize);

      if (pMemoryForBins && pstBinCount) {

        // Copy over the first column and then inplace add all the other cores
        memcpy(pnCountGroup, pstBinCount[0].pUserMemory,
               unique_rows * itemSize);
        ...
        if (pack) {
          outiGroup = AllocateNumpyArray(1, (npy_intp *)&totalRows, outdtype);
          outiFirstGroup =
              AllocateNumpyArray(1, (npy_intp *)&unique_rows, outdtype);
          ...
          if (outiGroup && outiFirstGroup) {
            ...
            memcpy(piFirstGroup, pstBinCount[0].pUserMemory,
                   unique_rows * itemSize);
            ...
            auto lambdaBinCountCallback = [](void *callbackArgT, int core,
              ...
              callbackArg->pMakeIGroup2(
                  callbackArg->piKey, callbackArg->pstBinCount[t].pUserMemory,
                  callbackArg->piGroup,
            ...
            LOGGING("multithread makeigroup2   %lld cores   %lld unique   %lld "
                    "rows\n",
                    cores, unique_rows, totalRows);

            g_cMathWorker->DoMultiThreadedWork(
                (int)cores, lambdaBinCountCallback, &stMaster);
  ...
  if (pack == 1 && outiGroup && outiFirstGroup) {
    ...
    PyObject *retObject =
        Py_BuildValue("(OOO)", outArray, outiGroup, outiFirstGroup);
