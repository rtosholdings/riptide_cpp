...
void *InternalAlloc(size_t allocSize) { return malloc(allocSize); }

void InternalFree(void *memory) { return free(memory); }
...
void *CArenaMemory::Allocate(INT64 length) {
  ...
  ArenaNode *pNode =
      (ArenaNode *)InterlockedPopEntrySList(&(pEntry->SListHead));
...
FAST_BUF_SHARED *CArenaMemory::AllocateFastBufferInternal(INT64 bufferSize) {
  ...
  FAST_BUF_SHARED *pFastBuf =
      (FAST_BUF_SHARED *)Allocate(sizeof(FAST_BUF_SHARED) + bufferSize);
...
void CArenaMemory::FreeFastBufferInternal(FAST_BUF_SHARED *pFastBuf) {
  ...
  if (result <= 0) {
    ...
    if (result < 0) {
      ...
      LOGERROR("!! reference count below 0.  This might be a shared memory "
               "buffer\n");
    } else {
      Free(pFastBuf);
    }
