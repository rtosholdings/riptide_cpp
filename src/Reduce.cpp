...
static inline __m256 FMIN_OP(float z, __m256 x, __m256 y) {
  ...
  const auto unord_cmp_mask =
      _mm256_cmp_ps(min_result, min_result, _CMP_UNORD_Q);
  return _mm256_blendv_ps(x, y, unord_cmp_mask);
}
...
static inline __m256d FMIN_OP(double z, __m256d x, __m256d y) {
  ...
  const auto unord_cmp_mask =
      _mm256_cmp_pd(min_result, min_result, _CMP_UNORD_Q);
  return _mm256_blendv_pd(x, y, unord_cmp_mask);
}
...
typedef int64_t (*ARG_SCATTER_GATHER_FUNC)(
    void *pDataIn, int64_t len, int64_t fixup,
...
class ReduceArgMax final {
  //--------------------------------------------------------------------------------------------
  template <typename T>
  static int64_t non_vector(void *pDataIn, int64_t len, int64_t fixup,
                            stArgScatterGatherFunc *pstScatterGatherFunc) {
...
public:
  ...
  static ARG_SCATTER_GATHER_FUNC
  GetScatterGatherFuncPtr(const NPY_TYPES inputType) {
    switch (inputType) {
    case NPY_FLOAT:
...
class ReduceMax final {
  //--------------------------------------------------------------------------------------------
  template <typename T>
  static double non_vector(void *pDataIn, int64_t len,
    ...
    if (pstScatterGatherFunc->lenOut == 0) {
      ...
      if (pstScatterGatherFunc->resultOut == pstScatterGatherFunc->resultOut) {
        pstScatterGatherFunc->resultOut =
            MAXF(pstScatterGatherFunc->resultOut, (double)result);
      }
  ...
  static double avx2(void *pDataIn, int64_t len,
                     stScatterGatherFunc *pstScatterGatherFunc) {
    T *pIn = (T *)pDataIn;
    T *pEnd;
...
public:
  // Given the numpy dtype number of the input array, returns a pointer to the
  // reduction function specialized for that array type.
  static ANY_SCATTER_GATHER_FUNC
  GetScatterGatherFuncPtr(const NPY_TYPES inputType) {
    switch (inputType) {
    case NPY_FLOAT:
...
class ReduceMin final {
  ...
  static double non_vector(void *pDataIn, int64_t len,
    ...
    if (pstScatterGatherFunc->lenOut == 0) {
      ...
      if (pstScatterGatherFunc->resultOut == pstScatterGatherFunc->resultOut) {
        pstScatterGatherFunc->resultOut =
            MINF(pstScatterGatherFunc->resultOut, (double)result);
      }
      T previous = (T)(pstScatterGatherFunc->resultOutInt64);
  ...
  static double avx2(void *pDataIn, int64_t len,
    ...
    if (len >= chunkSize) {
      ...
      if (false) {
        ...
        ym1 = CAST_TO128ME(
            ym1, _mm_shuffle_pd(CAST_TO128d(ym0), CAST_TO128d(ym1), 1));
        ym0 = MIN_OP128(result, ym0, ym1);
  ...
  template <typename T, typename U256, typename U128>
  static double avx2_nan_aware(void *pDataIn, int64_t len,
                               stScatterGatherFunc *pstScatterGatherFunc) {
    T *pIn = (T *)pDataIn;
    T *pEnd;
  ...
  static double non_vector(void *pDataIn, int64_t len,
    ...
    if (invalid_for_type<T>::is_valid(result)) {
      ...
      if (pstScatterGatherFunc->lenOut == 0) {
        ...
        pstScatterGatherFunc->resultOut =
            (std::min)(pstScatterGatherFunc->resultOut, (double)result);

        T previous = (T)(pstScatterGatherFunc->resultOutInt64);
        pstScatterGatherFunc->resultOutInt64 =
            (int64_t)((std::min)(previous, result));
      }
  ...
  static double non_vector(void *pDataIn, int64_t len,
    ...
    if (invalid_for_type<T>::is_valid(result)) {
      ...
      if (pstScatterGatherFunc->lenOut == 0) {
        ...
        pstScatterGatherFunc->resultOut =
            (std::max)(pstScatterGatherFunc->resultOut, (double)result);

        T previous = (T)(pstScatterGatherFunc->resultOutInt64);
        pstScatterGatherFunc->resultOutInt64 =
            (int64_t)((std::max)(previous, result));
      }
...
class ReduceSum final {
  //--------------------------------------------------------------------------------------------
  template <typename T>
  static double ReduceAddSlow(void *pDataIn, int64_t length,
                              stScatterGatherFunc *pstScatterGatherFunc) {
    T *pIn = (T *)pDataIn;
    T *pEnd;
    double result = 0;
  ...
  //=============================================================================================
  static double ReduceAddF32(void *pDataIn, int64_t len,
                             stScatterGatherFunc *pstScatterGatherFunc) {
    double result = 0;
  ...
  //--------------------------------------------------------------------------------------------
  static double ReduceAddI32(void *pDataIn, int64_t len,
                             stScatterGatherFunc *pstScatterGatherFunc) {
    double result = 0;
    ...
    if (len >= 32) {
      ...
      while (pIn < pEnd) {
        ...
        m0 = _mm256_add_pd(
            m0, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn))));
        m1 = _mm256_add_pd(
            m1, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn + 4))));
        m2 = _mm256_add_pd(
            m2, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn + 8))));
        m3 = _mm256_add_pd(
            m3, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn + 12))));
        m4 = _mm256_add_pd(
            m4, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn + 16))));
        m5 = _mm256_add_pd(
            m5, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn + 20))));
        m6 = _mm256_add_pd(
            m6, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn + 24))));
        m7 = _mm256_add_pd(
            m7, _mm256_cvtepi32_pd(_mm_loadu_si128((__m128i *)(pIn + 28))));
        pIn += 32;
      }
      m0 = _mm256_add_pd(m0, m1);
      m2 = _mm256_add_pd(m2, m3);
  ...
  //--------------------------------------------------------------------------------------------
  static double ReduceAddD64(void *pDataIn, int64_t len,
                             stScatterGatherFunc *pstScatterGatherFunc) {
    double result = 0;
  ...
  */
  ...
  static double ReduceAdd(void *pDataIn, int64_t length,
                          stScatterGatherFunc *pstScatterGatherFunc) {
    Output result = 0;
  ...
  static double ReduceAddI64(void *pDataIn, int64_t len,
                             stScatterGatherFunc *pstScatterGatherFunc) {
    double resultd = 0;
    int64_t result = 0;
  ...
  static double ReduceAddU64(void *pDataIn, int64_t len,
                             stScatterGatherFunc *pstScatterGatherFunc) {
    double resultd = 0;
    uint64_t result = 0;
...
class ReduceVariance final {
  ...
  template <typename T>
  static double ReduceVar_TODO(void *pDataIn, int64_t len,
                               stScatterGatherFunc *pstScatterGatherFunc) {
    ...
    if (len >= 4) {
      ...
      for (int64_t i = 0; i < len2; i += 4) {
        ...
        m0 =
            _mm256_sub_pd(_mm256_set_pd((double)pIn[i], (double)pIn[i + 1],
                                        (double)pIn[i + 2], (double)pIn[i + 3]),
                          m3);
  ...
  template <typename T>
  static double ReduceVar(void *pDataIn, int64_t len,
                          stScatterGatherFunc *pstScatterGatherFunc) {
  ...
  //--------------------------------------------------------------------------------------------
  static double ReduceVarF32(void *pDataIn, int64_t len,
                             stScatterGatherFunc *pstScatterGatherFunc) {
    double mean = (double)pstScatterGatherFunc->meanCalculation;
    double result = 0;
...
class ReduceMean final {
  ...
  static double wrapper(void *pDataIn, int64_t len,
                        stScatterGatherFunc *pstScatterGatherFunc) {

    ReduceSum::GetScatterGatherFuncPtr(
        (NPY_TYPES)pstScatterGatherFunc->inputType)(pDataIn, len,
                                                    pstScatterGatherFunc);
    ...
    if (pstScatterGatherFunc->lenOut > 1) {
      pstScatterGatherFunc->resultOut = pstScatterGatherFunc->resultOut /
...
class ReduceNanMean final {
  static double wrapper(void *pDataIn, int64_t len,
                        stScatterGatherFunc *pstScatterGatherFunc) {
    ReduceNanSum::GetScatterGatherFuncPtr(
        (NPY_TYPES)pstScatterGatherFunc->inputType)(pDataIn, len,
                                                    pstScatterGatherFunc);
...
class ReduceStdDev final {
  static double wrapper(void *pDataIn, int64_t len,
                        stScatterGatherFunc *pstScatterGatherFunc) {
    const double variance = ReduceVariance::GetScatterGatherFuncPtr(
        (NPY_TYPES)pstScatterGatherFunc->inputType)(pDataIn, len,
                                                    pstScatterGatherFunc);
...
class ReduceNanStdDev final {
  static double wrapper(void *pDataIn, int64_t len,
                        stScatterGatherFunc *pstScatterGatherFunc) {
    const double variance = ReduceNanVariance::GetScatterGatherFuncPtr(
        (NPY_TYPES)pstScatterGatherFunc->inputType)(pDataIn, len,
                                                    pstScatterGatherFunc);
    return sqrt(variance);
  }
...
//
static ARG_SCATTER_GATHER_FUNC
GetArgReduceFuncPtr(const NPY_TYPES inputType, const REDUCE_FUNCTIONS func) {
  switch (func) {
...
//
//
static ANY_SCATTER_GATHER_FUNC GetReduceFuncPtr(const NPY_TYPES inputType,
                                                const REDUCE_FUNCTIONS func) {
  switch (func) {
...
//
static PyObject *ReduceInternal(PyArrayObject *inArr1, REDUCE_FUNCTIONS func,
                                const int64_t ddof = 1) {

  const NPY_TYPES numpyInType = (NPY_TYPES)ObjectToDtype(inArr1);

  if (numpyInType < 0) {
    LOGGING("Reduce: Dont know how to convert these types %d  for func %llu",
            numpyInType, func);
    // punt to numpy
    Py_INCREF(Py_None);
    return Py_None;
  ...
  LOGGING("Reduce: numpy types %d --> %d   %d %d\n", numpyInType, numpyOutType,
          gNumpyTypeToSize[numpyInType], gNumpyTypeToSize[numpyOutType]);
  ...
  if (func >= REDUCE_FUNCTIONS::REDUCE_ARGMIN &&
      func <= REDUCE_FUNCTIONS::REDUCE_NANARGMAX) {
  ...
  if (func >= 100 && func < REDUCE_FUNCTIONS::REDUCE_MIN) {
    ...
    if (func == REDUCE_FUNCTIONS::REDUCE_SUM ||
        func == REDUCE_FUNCTIONS::REDUCE_NANSUM) {
      // Check for overflow
      switch (numpyInType) {
      CASE_NPY_UINT64:
      ...
      CASE_NPY_INT64:
        ...
        if (sgFunc.resultOut > 9223372036854775807.0 ||
            sgFunc.resultOut < -9223372036854775808.0) {
          LOGGING("Returning overflow %lf  for func %lld\n", sgFunc.resultOut,
                  func);
          return PyFloat_FromDouble(sgFunc.resultOut);
        }
    ...
    case NPY_LONGDOUBLE:
      ...
      case REDUCE_FUNCTIONS::REDUCE_NANMIN:
      case REDUCE_FUNCTIONS::REDUCE_NANMAX:
        return PyFloat_FromDouble(sgFunc.lenOut > 0 ? sgFunc.resultOut
                                                    : Py_NAN);
...
PyObject *Reduce(PyObject *self, PyObject *args) {
  ...
  if (tupleSize != 2) {
    return PyErr_Format(PyExc_ValueError,
                        "Reduce only takes two args instead of %lld args",
                        tupleSize);
  }
