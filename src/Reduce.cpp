...
static inline __m256d LOADU(__m256d* x) { return _mm256_loadu_pd((double const *)x); };
static inline __m256 LOADU(__m256* x) { return _mm256_loadu_ps((float const *)x); };
static inline __m256i LOADU(__m256i* x) { return _mm256_loadu_si256((__m256i const *)x); };

static inline void STOREU(__m256d* x, __m256d y) { _mm256_storeu_pd((double*)x, y); }
static inline void STOREU(__m256* x, __m256 y) { _mm256_storeu_ps((float*)x, y); }
...
/**
 * @brief AVX psuedo-intrinsic implementing the C 'fmax' function for a vector of packed flats.
 *
 * @param x
 * @param y
 * @return __m256
...
/**
 * @brief AVX psuedo-intrinsic implementing the C 'fmin' function for a vector of packed flats.
 *
 * @param x
 * @param y
 * @return __m256
...
/**
 * @brief AVX psuedo-intrinsic implementing the C 'fmin' function for a vector of packed doubles.
 *
 * @param x
 * @param y
 * @return __m256d
...
static inline __m256i CAST_TO256i(__m256d x) { return _mm256_castpd_si256(x); }
static inline __m256i CAST_TO256i(__m256 x) { return _mm256_castps_si256(x); }
static inline __m256i CAST_TO256i(__m256i x) { return x; }

static inline __m128i CAST_TO128i(__m128d x) {   return _mm_castpd_si128(x); }
static inline __m128i CAST_TO128i(__m128 x) { return _mm_castps_si128(x); }
static inline __m128i CAST_TO128i(__m128i x) { return x; }

static inline __m128d CAST_TO128d(__m128d x) { return x; }
static inline __m128d CAST_TO128d(__m128 x) { return _mm_castps_pd(x); }
static inline __m128d CAST_TO128d(__m128i x) { return _mm_castsi128_pd(x); }

static inline __m128d CAST_TO128ME(__m128d x, __m128i y) { return _mm_castsi128_pd(y); }
static inline __m128  CAST_TO128ME(__m128  x, __m128i y) { return _mm_castsi128_ps(y); }
static inline __m128i CAST_TO128ME(__m128i x, __m128i y) { return y; }

static inline __m128d CAST_TO128ME(__m128d x, __m128d y) { return y; }
static inline __m128  CAST_TO128ME(__m128  x, __m128d y) { return _mm_castpd_ps(y); }
static inline __m128i CAST_TO128ME(__m128i x, __m128d y) { return _mm_castpd_si128(y); }

static inline __m128d CAST_256_TO_128_LO(__m256d x) { return _mm_castsi128_pd(_mm256_extractf128_si256(_mm256_castpd_si256(x), 0)); }
static inline __m128  CAST_256_TO_128_LO(__m256  x) { return _mm_castsi128_ps(_mm256_extractf128_si256(_mm256_castps_si256(x), 0)); }
static inline __m128i CAST_256_TO_128_LO(__m256i x) { return _mm256_extractf128_si256(x, 0); }

static inline __m128d CAST_256_TO_128_HI(__m256d x) { return _mm_castsi128_pd(_mm256_extractf128_si256(_mm256_castpd_si256(x), 1)); }
static inline __m128  CAST_256_TO_128_HI(__m256  x) { return _mm_castsi128_ps(_mm256_extractf128_si256(_mm256_castps_si256(x), 1)); }
static inline __m128i CAST_256_TO_128_HI(__m256i x) { return _mm256_extractf128_si256(x, 1); }
...
public:
   ...
   // Given the numpy dtype number of the input array, returns a pointer to the reduction function
...
class ReduceMax final
{
   ...
   static double avx2(void* pDataIn, INT64 len, stScatterGatherFunc* pstScatterGatherFunc) {
      ...
      if (len >= chunkSize) {
         ...
         U256 m0 = LOADU(pIn256++); U256 m1 = LOADU(pIn256++);  U256 m2 = LOADU(pIn256++);  U256 m3 = LOADU(pIn256++);
         U256 m4 = LOADU(pIn256++); U256 m5 = LOADU(pIn256++);  U256 m6 = LOADU(pIn256++);  U256 m7 = LOADU(pIn256++);
         ...
         // Wind this calculation down from 8 256 bit registers to the data T MAX_OP
         m0 = MAX_OP(result, m0, m1);  m2 = MAX_OP(result, m2, m3);  m4 = MAX_OP(result, m4, m5);  m6 = MAX_OP(result, m6, m7);
         m0 = MAX_OP(result, m0, m2);  m4 = MAX_OP(result, m4, m6);  m0 = MAX_OP(result, m0, m4);
...
public:
   ...
   static ANY_SCATTER_GATHER_FUNC GetScatterGatherFuncPtr(const NPY_TYPES inputType) {
      ...
      case NPY_FLOAT:
         ...
         // Use the template parameter to select that version if specified.
...
class ReduceNanMin final
{
   ...
   static double non_vector(void* pDataIn, INT64 len, stScatterGatherFunc* pstScatterGatherFunc) {
      ...
      // Only store the result -- or reduce with an existing intermediate result from
      // another chunk in this same array -- if the result was a non-NaN value.
      ...
      if (invalid_for_type<T>::is_valid(result))
      {
         ...
         if (pstScatterGatherFunc->lenOut == 0) {
            ...
            // it only matters that we detect the case of an all-NaN array, and for that it's enough
            // to set this to any positive value.
   ...
   static double avx2(void* pDataIn, INT64 len, stScatterGatherFunc* pstScatterGatherFunc) {
      ...
      if (len >= chunkSize) {
         ...
         // Wind this calculation down from 8 256 bit registers to the data T FMIN_OP
         m0 = FMIN_OP(result, m0, m1);  m2 = FMIN_OP(result, m2, m3);  m4 = FMIN_OP(result, m4, m5);  m6 = FMIN_OP(result, m6, m7);
         m0 = FMIN_OP(result, m0, m2);  m4 = FMIN_OP(result, m4, m6);
...
class ReduceNanMax final
{
   ...
   static double avx2(void* pDataIn, INT64 len, stScatterGatherFunc* pstScatterGatherFunc) {
      ...
      if (len >= chunkSize) {
         ...
         // Wind this calculation down from 8 256 bit registers to the data T FMAX_OP
         m0 = FMAX_OP(result, m0, m1);  m2 = FMAX_OP(result, m2, m3);  m4 = FMAX_OP(result, m4, m5);  m6 = FMAX_OP(result, m6, m7);
         m0 = FMAX_OP(result, m0, m2);  m4 = FMAX_OP(result, m4, m6);
...
class ReduceSum final
{
   ...
   static double ReduceAddF32(void* pDataIn, INT64 len, stScatterGatherFunc* pstScatterGatherFunc) {
      ...
      if (len >= 32) {
         ...
         __m256d m0 = _mm256_setzero_pd();  __m256d m1 = _mm256_setzero_pd();  __m256d m2 = _mm256_setzero_pd();  __m256d m3 = _mm256_setzero_pd();
         __m256d m4 = _mm256_setzero_pd();  __m256d m5 = _mm256_setzero_pd();  __m256d m6 = _mm256_setzero_pd();  __m256d m7 = _mm256_setzero_pd();
...
//============================================================================================
/**
 * @brief Nan-ignoring arithmetic mean/average (nanmean) reduction implementation.
 */
...
static PyObject* ReduceInternal(PyArrayObject* inArr1, REDUCE_FUNCTIONS func, const INT64 ddof = 1) {
   ...
   LOGGING("Reduce: numpy types %d --> %d   %d %d\n", numpyInType, numpyOutType, gNumpyTypeToSize[numpyInType], gNumpyTypeToSize[numpyOutType]);
   ...
   if (func >= REDUCE_FUNCTIONS::REDUCE_ARGMIN && func <= REDUCE_FUNCTIONS::REDUCE_NANARGMAX) {
      ...
      if (sgFunc.resultOutArgInt64 == -1) {
         return PyErr_Format(PyExc_ValueError, "There were no valid values to return the index location of for argmin or argmax.");
      }
   ...
   if (func >= 100 && func < REDUCE_FUNCTIONS::REDUCE_MIN) {
      ...
      case NPY_DOUBLE:
      case NPY_LONGDOUBLE:
         // If the function called was a "nan___" reduction function, we need to check whether it found
...
Reduce(PyObject* self, PyObject* args)
{
   ...
   if (IsFastArrayOrNumpy(inArr1)) {
      ...
      if (PyArray_STRIDE(inArr1, 0) == PyArray_ITEMSIZE(inArr1)) {
         ...
         // TODO: future work -- we can make a copy of the array if strided (which removes the striding)
         //       then call our normal routine.
         //       Or, implement a special modified version of the routine which handles strided arrays.
         //       Could make this decision on a per-function basis if we modify GetConversionFunction()
         //       above to accept the array shape and a flag indicating whether the array is strided,
         //       then it can return a pointer to the appropriate function if one is available, or just
         //       return nullptr (as normal) in which case we'll punt to numpy.
      }
   }
