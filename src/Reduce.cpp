...
static inline __m256  MIN_OP(float z, __m256 x, __m256 y)
{
   ...
   const auto unord_cmp_mask = _mm256_cmp_ps(x, x, _CMP_UNORD_Q);
   ...
   const auto blended_nans = _mm256_blendv_ps(y, x, unord_cmp_mask);
...
{
   // Create a mask indicating which components of 'x', if any, are NaNs.
   const auto unord_cmp_mask = _mm256_cmp_pd(x, x, _CMP_UNORD_Q);

   // Use the blendv operation with our "x is nan" mask to create a new vector
   // containing the components of 'y' where 'x' was _not_ a NaN, and the components
   // of 'x' which were NaNs.
   const auto blended_nans = _mm256_blendv_pd(y, x, unord_cmp_mask);
...
// This function implements a 'max' operation which returns a NaN when either argument (per component)
// contains a NaN. This fixes the behavior of the _mm256_max_pd() intrinsic, which does not always propagate
// NaNs as expected.
...
// /**
//  * @brief AVX psuedo-intrinsic implementing the C 'fmax' function for a vector of packed doubles.
//  *
//  * @param x
//  * @param y
//  * @return __m256d
...
{
    //--------------------------------------------------------------------------------------------
    template <typename T, bool recognize_integer_invalids>
    static double non_vector(void * pDataIn, int64_t len, stScatterGatherFunc * pstScatterGatherFunc)
    {
        using namespace riptide::math;

        // TODO: Handle case where len <= 0 to avoid dereferencing an empty array below.

        const T* const pIn = (T*)pDataIn;

        // Initialize the result with the first (0th) element.
        T result = pIn[0];

        // Indicates when T is an integral type AND we're *NOT* recognizing / doing anything special
        // for integer invalid values.
        static constexpr bool skip_invalid_check = std::is_integral<T>::value && !recognize_integer_invalids;

        // get the maximum, making sure NaNs are propagated correctly.
        if (skip_invalid_check || invalid_for_type<T>::is_valid(result)) {
            for (int64_t i = 1; i < len; i++) {
                const auto temp = pIn[i];

                if (!skip_invalid_check && !invalid_for_type<T>::is_valid(temp)) {
                    // We've found a NaN value. NaNs taint the result, so we set
                    // the result value here then short-circuit since there's no
        ...
        else
        {
            ...
            {
                pstScatterGatherFunc->resultOut = max_with_nan_passthru(pstScatterGatherFunc->resultOut, (double)result);
            }

            T previous = (T)(pstScatterGatherFunc->resultOutInt64);
            pstScatterGatherFunc->resultOutInt64 = (int64_t)(max_with_nan_passthru(previous, result));
        }
        pstScatterGatherFunc->lenOut += len;
    ...
    static double avx2(void * pDataIn, int64_t len, stScatterGatherFunc * pstScatterGatherFunc)
    {
        ...
        static_assert(
            !std::is_integral<T>::value || !recognize_integer_invalids,
        ...
        static constexpr int64_t chunkSize = (sizeof(U256) * unroll_factor) / sizeof(T);
        ...
        if (len >= chunkSize)
        {
            ...
            {
                // TODO: Consider integrating a check into this loop to detect if we've seen a NaN,
                //       and if so, short-circuiting the loop -- there's no point in continuing the
                //       loop if e.g. the first element of the array is a NaN.
                //       We could also short-circuit for integer types if we've seen std::numeric_limits<T>::max()
                //       or std::numeric_limits<T>::infinity() for floating-point types.
                //       This can be done quickly with some comparisons against those values, then horizontally
                //       reducing that mask using e.g. _mm256_movemask_pd/ps() and checking if the result == 15 or 255 (respectively).
            ...
            // PERF: Replace this memory spill with a vector reduction (e.g. using unpackhi/unpacklo)
            __m256i temp;
            ...
            {
                result = max_with_nan_passthru(result, tempT[i]);
            }
        ...
        {
            // get the minimum
            result = max_with_nan_passthru(result, *pIn);
            pIn++;
        }
...
public:
    // Given the numpy dtype number of the input array, returns a pointer to the
    // reduction function specialized for that array type.
    static ANY_SCATTER_GATHER_FUNC GetScatterGatherFuncPtr(const NPY_TYPES inputType, const bool recognize_integer_invalids)
    {
        switch (inputType)
        {
        case NPY_FLOAT:
            return avx2<float, false>;
        case NPY_DOUBLE:
            return avx2<double, false>;
        case NPY_LONGDOUBLE:
            return non_vector<long double, false>;
        case NPY_BOOL:
            // TODO: The special case of bool (or bool as a view of int8/uint8) should use
            //       a vectorized any() kernel instead, because it doesn't have an invalid value
            //       and it can short-circuit.
        case NPY_INT8:
            return recognize_integer_invalids ? non_vector<int8_t, true> : avx2<int8_t, false>;
        case NPY_INT16:
            return recognize_integer_invalids ? non_vector<int16_t, true> : avx2<int16_t, false>;
        CASE_NPY_INT32:
            return recognize_integer_invalids ? non_vector<int32_t, true> : avx2<int32_t, false>;
        CASE_NPY_INT64:
            return recognize_integer_invalids ? non_vector<int64_t, true> : non_vector<int64_t, false>;
        case NPY_UINT8:
            return recognize_integer_invalids ? non_vector<uint8_t, true> : non_vector<uint8_t, false>;
        case NPY_UINT16:
            return recognize_integer_invalids ? non_vector<uint16_t, true> : non_vector<uint16_t, false>;
        CASE_NPY_UINT32:
            return recognize_integer_invalids ? non_vector<uint32_t, true> : avx2<uint32_t, false>;
        CASE_NPY_UINT64:
            return recognize_integer_invalids ? non_vector<uint64_t, true> : non_vector<uint64_t, false>;
        default:
            return nullptr;
        }
    }
...
class ReduceMin final
{
    ...
    static double non_vector(void * pDataIn, int64_t len, stScatterGatherFunc * pstScatterGatherFunc)
    {
        ...
        // get the minimum, making sure NaNs are propagated correctly.
        ...
        else
        {
            ...
            if (pstScatterGatherFunc->resultOut == pstScatterGatherFunc->resultOut)
            {
                pstScatterGatherFunc->resultOut = min_with_nan_passthru(pstScatterGatherFunc->resultOut, (double)result);
            }
            T previous = (T)(pstScatterGatherFunc->resultOutInt64);
            pstScatterGatherFunc->resultOutInt64 = (int64_t)(min_with_nan_passthru(previous, result));
        }
        pstScatterGatherFunc->lenOut += len;
        return (double)pstScatterGatherFunc->resultOutInt64;
    }
    ...
    static double avx2(void * pDataIn, int64_t len, stScatterGatherFunc * pstScatterGatherFunc)
    {
        ...
        if (len >= chunkSize)
        {
            ...
            while (pIn256 < pEnd256)
            {
                ...
                //       and if so, short-circuiting the loop -- there's no point in continuing the
                //       loop if e.g. the first element of the array is a NaN.
                //       We could also short-circuit for integer types if we've seen std::numeric_limits<T>::min()
                //       or -std::numeric_limits<T>::infinity() for floating-point types.
                //       This can be done quickly with some comparisons against those values, then horizontally
                //       reducing that mask using e.g. _mm256_movemask_pd/ps() and checking if the result == 15 or 255 (respectively).
            ...
            for (int i = 0; i < perReg; i++)
            {
                result = min_with_nan_passthru(result, tempT[i]);
            }
        ...
        {
            // get the minimum
            result = min_with_nan_passthru(result, *pIn);
            pIn++;
        }
...
public:
    ...
    static ANY_SCATTER_GATHER_FUNC GetScatterGatherFuncPtr(const NPY_TYPES inputType, const bool recognize_integer_invalids)
    {
        ...
        case NPY_BOOL:
            // TODO: The special case of bool (or bool as a view of int8/uint8) should use
            //       a vectorized all() kernel instead, because it doesn't have an invalid value
            //       and it can short-circuit.
        case NPY_INT8:
            return recognize_integer_invalids ? non_vector<int8_t, true> : avx2<int8_t, false>;
        case NPY_INT16:
...
class ReduceNanMin final
{
    ...
    static double non_vector(void * pDataIn, int64_t len, stScatterGatherFunc * pstScatterGatherFunc)
    {
        ...
        // Search for first non Nan
        if constexpr (!skip_invalid_check)
        {
            while (pIn < pEnd)
            {
                // break on first non nan
        ...
        if (pIn < pEnd)
        {
            ...
            while (pIn < pEnd)
            {
                ...
                if (skip_invalid_check || invalid_for_type<T>::is_valid(*pIn))
                {
        ...
        else
        {
            ...
            return invalid_for_type<double>::value;
        }
    }
...
public:
    ...
    static ANY_SCATTER_GATHER_FUNC GetScatterGatherFuncPtr(const NPY_TYPES inputType, const bool recognize_integer_invalids)
    {
        ...
        //       Initially, do this for `float` and `double`; the FMIN_OP function
        //       needs to have overloads implemented for integer types before the
        //       AVX2 implementation can be enabled for integral types.

        switch (inputType)
        {
        case NPY_FLOAT:
            return non_vector<float, false>;
        case NPY_DOUBLE:
            return non_vector<double, false>;
        case NPY_LONGDOUBLE:
            return non_vector<long double, false>;

        case NPY_INT8:
            return recognize_integer_invalids ? non_vector<int8_t, true> : non_vector<int8_t, false>;
        case NPY_INT16:
            return recognize_integer_invalids ? non_vector<int16_t, true> :  non_vector<int16_t, false>;
        CASE_NPY_INT32:
            return recognize_integer_invalids ? non_vector<int32_t, true> :  non_vector<int32_t, false>;
        CASE_NPY_INT64:
            return recognize_integer_invalids ? non_vector<int64_t, true> :  non_vector<int64_t, false>;
        case NPY_UINT8:
            return recognize_integer_invalids ? non_vector<uint8_t, true> :  non_vector<uint8_t, false>;
        case NPY_UINT16:
            return recognize_integer_invalids ? non_vector<uint16_t, true> :  non_vector<uint16_t, false>;
        CASE_NPY_UINT32:
            return recognize_integer_invalids ? non_vector<uint32_t, true> :  non_vector<uint32_t, false>;
        CASE_NPY_UINT64:
            return recognize_integer_invalids ? non_vector<uint64_t, true> :  non_vector<uint64_t, false>;
        ...
        case NPY_BOOL:
            return ReduceMin::GetScatterGatherFuncPtr(inputType, recognize_integer_invalids);
...
public:
    ...
    {
        // TODO: Enable avx2 implementation once it's been tested.
        //       Initially, do this for `float` and `double`; the FMAX_OP function
        //       needs to have overloads implemented for integer types before the
        //       AVX2 implementation can be enabled for integral types.
        ...
        // nan/invalid value so we utilize the normal reduction operation for it.
        case NPY_BOOL:
            return ReduceMax::GetScatterGatherFuncPtr(inputType, recognize_integer_invalids);
...
static ANY_SCATTER_GATHER_FUNC GetReduceFuncPtr(const NPY_TYPES inputType, const REDUCE_FUNCTIONS func)
{
    ...
    case REDUCE_FUNCTIONS::REDUCE_MIN:
        return ReduceMin::GetScatterGatherFuncPtr(inputType, false);
    case REDUCE_FUNCTIONS::REDUCE_MIN_NANAWARE:
        return ReduceMin::GetScatterGatherFuncPtr(inputType, true);
    case REDUCE_FUNCTIONS::REDUCE_NANMIN:
        return ReduceNanMin::GetScatterGatherFuncPtr(inputType, false);

    case REDUCE_FUNCTIONS::REDUCE_MAX:
        return ReduceMax::GetScatterGatherFuncPtr(inputType, false);
    case REDUCE_FUNCTIONS::REDUCE_NANMAX:
        return ReduceNanMax::GetScatterGatherFuncPtr(inputType, false);
