...
void *WorkSpaceAllocLarge(size_t HashTableAllocSize) {
  ...
  if (g_cMathWorker->NoThreading || g_cMathWorker->NoCaching) {
    ...
    if (g_pHashTableAny != NULL &&
        HashTableAllocSize <= g_CurrentAllocHashTable) {
...
void WorkSpaceFreeAllocLarge(void *&pHashTableAny, size_t HashTableAllocSize) {
  LOGRECYCLE("WorkSpaceFreeAllocLarge %p %p %lld %lld\n", pHashTableAny,
             g_pHashTableAny, g_CurrentAllocHashTable, HashTableAllocSize);
...
static void RefCountNumpyArray(stRecycleList *pItems, int32_t lzcount,
                               int32_t type, int32_t slot, bool bIncrement) {
  ...
  if (bIncrement) {
    ...
    LOGRECYCLE("Adding item to slot %d,%d,%d -- refcnt now is %llu\n", lzcount,
               type, slot, pItems->Item[slot].recycledArray->ob_base.ob_refcnt);
...
static void DumpItemStats(stRecycleList *pItems, int k) {
  ...
  printf("    delta: %lld   refcount: %d  now: %lld  addr: %p  type: %d \t %p "
         "\t %lld\n",
         delta / NANO_BILLION, pItems->Item[k].initRefCount,
         pItems->Item[k].recycledArray == NULL
             ? 0LL
             : (int64_t)(pItems->Item[k].recycledArray->ob_base.ob_refcnt),
         // too dangerous -- might be gone pItems->Item[k].recycledOrigArray ==
         // NULL ? 0 : pItems->Item[k].recycledOrigArray->ob_base.ob_refcnt,
         pItems->Item[k].recycledArray == NULL
             ? NULL
             : PyArray_BYTES(pItems->Item[k].recycledArray),
         pItems->Item[k].type, pItems->Item[k].recycledArray,
  ...
  for (int i = 0; i < RECYCLE_ENTRIES; i++) {
    for (int j = 0; j < RECYCLE_MAXIMUM_TYPE; j++) {
      ...
      if (deltaHead < 0 || deltaHead > RECYCLE_MAXIMUM_SEARCH) {
        LogError(
            "!!! critical error with recycler items %d,%d with deltahead %d\n",
            i, j, deltaHead);
      }
...
PyObject *RecycleDump(PyObject *self, PyObject *args) {
  printf("Recycled stats.   Hits: %lld    Misses: %lld    Heads: %lld    "
         "Tails: %lld\n",
...
static bool DeleteNumpyArray(PyArrayObject *inArr) {
  ...
  if ((flags & (NPY_ARRAY_WRITEABLE | NPY_ARRAY_OWNDATA |
                NPY_ARRAY_F_CONTIGUOUS | NPY_ARRAY_C_CONTIGUOUS)) !=
      (NPY_ARRAY_WRITEABLE | NPY_ARRAY_OWNDATA | NPY_ARRAY_F_CONTIGUOUS |
       NPY_ARRAY_C_CONTIGUOUS)) {
    LOGRECYCLE("Rejected recycling because object is not writeable or owned or "
               "contiguous.\n");
    return false;
  }
  ...
  if (g_GarbageCollectTimeSpan > 0) {
    ...
    if (totalSize >= RECYCLE_MIN_SIZE && type < RECYCLE_MAXIMUM_TYPE &&
      ...
      for (int i = 0; i < RECYCLE_MAXIMUM_SEARCH; i++) {
        ...
        if (pItems->Item[i].totalSize != 0 &&
            pItems->Item[i].memoryAddress == memAddress) {
      ...
      if (!abort) {
        ...
        if (slot == -1) {
          ...
          for (int i = 0; i < RECYCLE_MAXIMUM_SEARCH; i++) {
            ...
            // TODO: Check current ref count--- if not 1, remove?
            int64_t curRefCount =
                pItems->Item[i].recycledArray->ob_base.ob_refcnt;
            if (curRefCount != 1) {
        ...
        // sanity check head tail
        if (deltaHead < 0 || deltaHead > RECYCLE_MAXIMUM_SEARCH) {
          LogError("!!! inner critical error with recycler items %d,%d,%d with "
                   "deltahead %d  totalsize%lld\n",
                   log2, type, slot, deltaHead, pItems->Item[slot].totalSize);
        }

        LOGRECYCLE("-- keeping array with refcnt %llu   head: %d  tail: %d   "
                   "size: %llu  leadingz:%d   type:%d  %p %p\n",
                   refCount, pItems->Head, pItems->Tail, totalSize, log2, type,
                   inArr, PyArray_BYTES(inArr));
        ...
        LOGRECYCLE("inner setting slot %d,%d,%d to %lld from %lld\n", log2,
                   type, slot, totalSize, pItemSlot->totalSize);
        ...
        LOGRECYCLE("Rejected recycling due to mem address clash refcnt: %llu   "
                   "size: %llu   type: %d  %p %p\n",
                   refCount, totalSize, type, inArr, PyArray_BYTES(inArr));
      }
    } else {
      LOGRECYCLE(
          "Rejected recycling refcnt: %llu   size: %llu   type: %d  %p %p\n",
          refCount, totalSize, type, inArr, PyArray_BYTES(inArr));
    }
    ...
    LOGRECYCLE("Rejected recycling NOT FA refcnt: %llu   size: %llu   type: %d "
               " %p %p\n",
               refCount, totalSize, type, inArr, PyArray_BYTES(inArr));
  }
  gRecursion--;
...
PyObject *RecycleGarbageCollectNow(PyObject *self, PyObject *args) {
  ...
  PyDict_SetItemString(
      pDict, "Hits",
      (PyObject *)PyLong_FromLongLong((long long)(g_FoundRecycled)));
  PyDict_SetItemString(
      pDict, "Misses",
      (PyObject *)PyLong_FromLongLong((long long)(g_DidNotFindRecycled)));
  PyDict_SetItemString(
      pDict, "Heads",
      (PyObject *)PyLong_FromLongLong((long long)(g_TotalHeads)));
  PyDict_SetItemString(
      pDict, "Tails",
      (PyObject *)PyLong_FromLongLong((long long)(g_TotalTails)));
  PyDict_SetItemString(
      pDict, "BitSz", (PyObject *)PyLong_FromLongLong((g_CurrentAllocBitSize)));
  PyDict_SetItemString(
      pDict, "HashSz",
      (PyObject *)PyLong_FromLongLong((g_CurrentAllocHashTable)));

  PyDict_SetItemString(
      pDict, "Alloc",
      (PyObject *)PyLong_FromLongLong((long long)(g_TotalAllocs)));
  PyDict_SetItemString(
      pDict, "Free", (PyObject *)PyLong_FromLongLong((long long)(g_TotalFree)));
  PyDict_SetItemString(
      pDict, "AllocSz",
      (PyObject *)PyLong_FromLongLong((long long)(g_TotalMemoryAllocated)));
  PyDict_SetItemString(
      pDict, "FreeSz",
      (PyObject *)PyLong_FromLongLong((long long)(g_TotalMemoryFreed)));
  PyDict_SetItemString(
      pDict, "Delta",
      (PyObject *)PyLong_FromLongLong((long long)g_TotalMemoryAllocated -
                                      (long long)g_TotalMemoryFreed));
  ...
  PyDict_SetItemString(pDict, "TotalDeleted",
                       (PyObject *)PyLong_FromLongLong(totalDeleted));
