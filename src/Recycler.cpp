...
static void DumpItemStats(stRecycleList * pItems, int k)
{
    ...
    printf("    delta: %lld   refcount: %d  now: %lld  addr: %p  type: %d \t %p \t %lld\n", delta / NANO_BILLION,
           ...
           // too dangerous -- might be gone pItems->Item[k].recycledOrigArray == NULL ? 0 :
           // pItems->Item[k].recycledOrigArray->ob_base.ob_refcnt,
...
{
    printf("Recycled stats.   Hits: %lld    Misses: %lld    Heads: %lld    Tails: %lld\n", g_FoundRecycled, g_DidNotFindRecycled,
           g_TotalHeads, g_TotalTails);
    ...
    if (g_GarbageCollectTimeSpan > 0)
    {
            ...
            ndim == 1 && strides != NULL && itemSize == strides[0])
        {
            ...
            if (! abort)
            {
                ...
                //   // INSERT NEW OTEMS
                //}
                // printf("-- recycled with refcnt %llu   size: %llu  leadingz:%llu   type:%d\n", refCount, totalSize, leadingZero,
                // type); g_InitialCount[g_recyclecount] = refCount; g_RecycleList[g_recyclecount++] = inArr;
            }
            else
            {
