...
void* ReadMemoryCallback(SDS_SHARED_MEMORY_CALLBACK* pSMCB) {
   ...
   //------------- META DATA -------------------------------
   pReturnBack->strMeta = std::string((const char*)GetMemoryOffset(baseOffset, pFileHeader->MetaBlockOffset), pFileHeader->MetaBlockSize);
...
extern "C" {
   ...
   DllExport stReadSharedMemory* ReadFromSharedMemory(const char* fileName, const char* shareName) {
      ...
      //==============================================
      stReadSharedMemory* result = (stReadSharedMemory*)
         SDSReadFile(
            fileName,
            shareName,
            folderName,
   ...
   // Returns:
   //    true/false
   DllExport bool AppendSDSFile(
      const char* outFileName,

      const char* shareFileName,
      const char* shareName,
      int64_t       totalRows,
      int64_t       bandSize = 0)
   {
